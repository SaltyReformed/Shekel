...
**The Python files NAMING `due_date` in code** (census 51 code files `due_date` in `app/**/*.py`) -- a SUPERSET of those touching the column, since the identifier is also a local and a kwarg, and the narrower AST census this once stated as 20 is not reproducible by a pattern -- more naming it only in prose, and templates render
...
numbers this sentence carried had drifted by 2026-09-11 -- and with them the double sync at create, the
...
`nominal_day`. After R7d the opening bound has THREE writers: `params.py`'s, and `bind_rule_to_loan`'s two,
