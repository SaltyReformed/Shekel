...
**152 steps, 111 open.** The dependency graph holds 89 edges over 59 rows.
...
| bank_import | X-f6a-4 | -- | An import can be UNDONE -- it releases the matches naming its lines, forgets the source-account pairing with the last import from that source, and the database now refuses to orphan a match -- while a same-day group reconciles as a SET so the ordinal this app mints stops deciding whether a line is already held. Closed **N-302**, **N-317**, **N-324**, **N-327**; opened **N-328**, **N-329**. | SHIPPED | `9439a5ad` | -- |
...
| recurrence | R5 | -- | Split a generated row's dates into three -- `occurs_on` (the cadence), `pay_period_id` (the funding) and `due_on` (the installment) -- and delete `compute_due_date`. A value-splitting migration; own PR. | #19 | -- | after #3 / balance:X-f4 (R5 edits `cash_ledger/_events.py`, which is inside that step's deletion set) |
...
| balance | X-ai-s | -- | Migrate `journal_entries` to an EXCLUSIVE ARC of typed FKs with an AT-MOST-ONE check, plus the reversal linkage. Held until the cutover, which deletes the correction family it would buy attribution for. Closes **N-167**. | #40 | -- | after #1 / balance:X-f3c |
...
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #72 | -- | after #3 / balance:X-f (the container ticks at #4; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
