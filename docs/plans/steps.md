...
**149 steps, 107 open.** The dependency graph holds 99 edges over 65 rows.
...
| pay_calendar | C2-f2c | -- | Move `/investment`'s five readers onto the pass's calendar, retiring `_chart._build_chart_markers`' linear containment scan for the window type's own `containing`. Closes **P48**. | #14 | -- | NOW |
...
| balance | X-ai-s | -- | Migrate `journal_entries` to an EXCLUSIVE ARC of typed FKs with an AT-MOST-ONE check, plus the reversal linkage. Held until the cutover, which deletes the correction family it would buy attribution for. Closes **N-167**. | #45 | -- | after #2 / balance:X-f3c |
...
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #77 | -- | after #4 / balance:X-f (the container ticks at #5; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
