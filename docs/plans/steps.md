...
**130 steps, 99 open.** The dependency graph holds 104 edges over 63 rows.
...
| balance | X-au-c | -- | Make `transactions.estimated_amount` and `transfers.amount` NULLABLE under `ck_transactions_amount_ownership`, the CHECK that pairs an explicit `amount_source` with the presence of a figure, route every projected-amount reader through the resolver, and turn X-aq's settle refresh into the freeze where a derived amount becomes a fact. | #4 | -- | NOW / balance:X-au-b (shipped) / balance:X-ap (shipped) |
| balance | X-au-d | -- | Cut SALARY rows over: generation stops pricing them, the 51 live rows go NULL, and `income_service.live_projected_net`, `_freshest_amount` and `_reconcile_cached_amount` are deleted. Closes **N-224**, **N-228**. | #5 | -- | after #4 / balance:X-au-c |
| balance | X-au-e | -- | Cut TEMPLATE rows over to their template's amount series: generation stops pricing them, the 511 non-override rows go NULL, and regeneration's amount arm with the conflict chooser's keep-vs-use decision are both deleted. | #6 | -- | after #4 / balance:X-au-c / balance:X-au-a (shipped) |
| balance | X-au-g | -- | Cut LOAN-PAYMENT shadows over, which must first RULE that a shadow's P&I resolves on its own due date as ruling D5 already put its escrow, because a resolver may not read the wall clock. Closes **N-40**. | #7 | -- | after #4 / balance:X-au-c |
| balance | X-au-f | -- | Cut TRANSFER rows and their shadows over, deleting the amount copy in `update_transfer` and the drift corrector beside it, which makes Transfer Invariant 3 structural rather than maintained. | #8 | -- | after #4 / balance:X-au-c / balance:X-f2-c3 (shipped; it settles shadows through the same verb) |
| balance | X-au-i | -- | Cut the CC PAYBACK kind over, the sixth amount rule: a payback's figure is the credit entries it repays, so `sync_entry_payback`'s unconditional rewrite and `create_cc_payback_transaction`'s unrepaired copy both become one derivation. Closes **N-243**, **N-252**. | #9 | -- | after #4 / balance:X-au-c |
| balance | X-au-h | -- | Split the FOUR facts `is_override` carries, of which two were found by adversarial review: re-priced, moved, survives the regeneration sweep, and exempt from the partial unique index. Closes **N-238**. | #10 | -- | after #8 / balance:X-au-d / balance:X-au-e / balance:X-au-f / balance:X-au-g |
| pay_calendar | C2-c | -- | Retire `balance_at/_cash_periods._PeriodSpans` so the cash view answers from the one calendar, keeping `None` outside the reported window as a VIEW question. Closes **P14**. | #11 | -- | NOW / pay_calendar:C2-a (shipped) / balance:X-ad-a (shipped) / pay_calendar:C3 (shipped) |
...
| pay_calendar | C4 | -- | Drop `pay_periods.end_date` and `period_index` with their three constraints, once the ORM readers take their bounds from the calendar. Closes **P1**, **P4**, **P5**, **P8**, **P9**. | #14 | -- | after #13 / pay_calendar:C2 (the container ticks at #9) / pay_calendar:C3 (shipped) |
...
| recurrence | R7b | -- | Rebuild the recurrence form as interval plus unit plus anchor plus an optional due row, giving `max_occurrences` its first writer and deleting `offset_periods` and the "First paycheck" affordance. Closes **D1**, **D2**. | #17 | -- | NOW |
| recurrence | R7c | -- | THE CUTOVER: one migration adds the two-axis columns, backfills them, and drops `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id` and `offset_periods`. It must RULE row **D28** first. Closes **D10**, **D12**, **D21**, **D24**. | #18 | -- | after #17 / recurrence:R7b |
| recurrence | R8 | -- | Add the four ruled add-ons: the WEEK unit, `recurrence_weekday_anchors`, the business-day shift and the count-bounded end. | #19 | -- | after #18 / recurrence:R7a-2b / recurrence:R7c |
| recurrence | R9 | -- | Drop the `ref.recurrence_patterns` table and `pay_period_admin._repoint_recurrence_rules`, after re-checking the two premises ledger row **D6** names. | #20 | -- | after #18 / recurrence:R7c |
| pay_calendar | C6 | -- | Let a payday be inserted mid-schedule, refusing only where `classify_period_lock` says the split period is locked. Starts with the two rulings section 3 names. Closes **P10**. | #21 | -- | after #18 / pay_calendar:C4 / recurrence:R7c |
| recurrence | R5 | -- | Split a generated row's dates into three -- `occurs_on` (the cadence), `pay_period_id` (the funding) and `due_on` (the installment) -- and delete `compute_due_date`. A value-splitting migration; own PR. | #22 | -- | after #2 / balance:X-f4 (R5 edits `cash_ledger/_events.py`, which is inside that step's deletion set) |
...
| balance | X-y | -- | Move the fifteen surfaces that resolve the baseline scenario directly onto the seam, deciding what a WRITE may do without one. Closes **N-117**. | #32 | -- | after #31 / balance:X-x (the container ticks at #29) |
...
| balance | X-ak | -- | Rule the stored transfer-to-shadow copy first, then unify the THREE mirror implementations that already disagree. Closes **N-148**, **N-150**, **N-152**, **N-156**, **N-159**, **N-170**. | #35 | -- | after #34 / balance:X-aj (the container ticks at #32) |
...
| balance | X-ai-s | -- | Migrate `journal_entries` to an EXCLUSIVE ARC of typed FKs with an AT-MOST-ONE check, plus the reversal linkage. Held until the cutover, which deletes the correction family it would buy attribution for. Closes **N-167**. | #40 | -- | after #1 / balance:X-f3 |
| balance | X-d | -- | Make the posted account ledger a CHECKED PROJECTION: the posting writer consumes the walk instead of its own, with a per-visible-date assert. Carries **N-135**. | #41 | -- | after #40 / balance:X-ai (the container ticks at #38; N-155 parked this step and the assert's placement is the restructure's) |
| balance | X-i1 | -- | Give `BalanceContext` the input tier the loan derivations already have, through the same memoize mechanism: the calendar, the contribution feed, the override map, the standing extra, the contractual schedule. Additive and byte-identical. | #42 | -- | NOW |
...
| credit_card | CC1b | -- | Build `balance_at/_revolving.py`: a card is an event stream of anchor facts, settled rows and projected rows, unwired and additive. | #58 | -- | after #33 / balance:X-f3 / balance:X-f4 / balance:X-am |
...
| balance | X-f6 | -- | Replace the date GUESS with the bank's own record via OFX / CSV / Plaid import, consuming the outstanding set and the residual path. Its first act is a trace, not code. Closes **N-173**. | #72 | -- | after #3 / balance:X-f5 (legal from #6; block 5 SCHEDULES it after the card arc so ONE matching rule covers checking and card rows rather than being widened into them later) |
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #73 | -- | after #72 / balance:X-f (the container ticks at #70; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
...
| arc | id | also | what this step does | order | commit | starts |
|---|---|---|---|---|---|---|
| balance | X-f | -- | The DECOMPOSED parent of the whole "the app records when money moved" cluster, carrying **N-42**. | container | -- | ticks with #72 |
| balance | X-au | -- | The DECOMPOSED parent of the amount model (**R-FI**): a row's amount is either its OWN or DERIVED, and a derived amount is not stored at all. Supersedes **X-ar**. Carries **N-40**, **N-224**, **N-228**, **N-238**. | container | -- | ticks with #10 |
