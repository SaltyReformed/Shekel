...
**152 steps, 110 open.** The dependency graph holds 99 edges over 65 rows.
...
| recurrence | R7c-c | -- | Drop what the cutover orphans -- `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id`, `offset_periods`, `start_date` and the unwritten `budget.recurrence_month_anchors` -- re-point `interval_n` off the encoding, and delete the encode / decode pair with the closed pattern set behind it. Closes **D6**, **D32**. | #23 | -- | NOW / recurrence:R7c-b (shipped) |
