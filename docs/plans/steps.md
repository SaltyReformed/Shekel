...
**151 steps, 111 open.** The dependency graph holds 97 edges over 64 rows.
...
| pay_calendar | C2-f2d-3 | -- | Move `/savings`'s pay-period readers onto the pass's calendar, changing the type its two bundle dataclasses carry, and take all four of the package's public entries onto the read pass in the same commit. Closes **P58**. | #13 | -- | NOW / pay_calendar:C2-f2d-1 (shipped) |
| pay_calendar | C2-f2e | -- | Move the budget dashboard and `/accounts/<id>` detail, the last two `BalanceContext`-holding surfaces, onto the pass's calendar. | #14 | -- | NOW |
...
| pay_calendar | C6 | -- | Let a payday be inserted mid-schedule, refusing only where `classify_period_lock` says the split period is locked. Starts with the two rulings section 3 names. Closes **P10**. | #21 | -- | after #16 / pay_calendar:C4 / recurrence:R7c (shipped) |
...
| balance | E2-n | -- | Make the move and delete the registry, with `_FENCED_MODULE_RULINGS` as the LAST commit rather than the first. Its decomposition is decided from #75's trace. | #78 | -- | NOW |
...
| pay_calendar | C2-f2d | -- | The DECOMPOSED parent of `/savings` and `/retirement`, split into three leaves 2026-08-16 exactly as its own bullet predicted: the read pass, then `/retirement`'s double projection, then `/savings`'s readers. | container | -- | ticks with #13 |
