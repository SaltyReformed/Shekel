...
**138 steps, 99 open.** The dependency graph holds 101 edges over 63 rows.
...
| recurrence | R7c | -- | THE CUTOVER: one migration adds the two-axis columns, backfills them, and drops `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id` and `offset_periods`. It must RULE row **D28** first. Closes **D10**, **D12**, **D21**, **D24**. | #17 | -- | NOW / recurrence:R7b-4 (shipped) |
| recurrence | R8 | -- | Add the four ruled add-ons: the WEEK unit, `recurrence_weekday_anchors`, the business-day shift and the count-bounded end. | #18 | -- | after #17 / recurrence:R7a-2b / recurrence:R7c |
...
| recurrence | R5 | -- | Split a generated row's dates into three -- `occurs_on` (the cadence), `pay_period_id` (the funding) and `due_on` (the installment) -- and delete `compute_due_date`. A value-splitting migration; own PR. | #21 | -- | after #2 / balance:X-f4 (R5 edits `cash_ledger/_events.py`, which is inside that step's deletion set) |
...
| balance | X-ai-s | -- | Migrate `journal_entries` to an EXCLUSIVE ARC of typed FKs with an AT-MOST-ONE check, plus the reversal linkage. Held until the cutover, which deletes the correction family it would buy attribution for. Closes **N-167**. | #39 | -- | after #1 / balance:X-f3 |
...
| balance | X-f6 | -- | Replace the date GUESS with the bank's own record via OFX / CSV / Plaid import, consuming the outstanding set and the residual path. Its first act is a trace, not code. Closes **N-173**. | #71 | -- | after #3 / balance:X-f5 (legal from #4; block 5 SCHEDULES it after the card arc so ONE matching rule covers checking and card rows rather than being widened into them later) |
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #72 | -- | after #71 / balance:X-f (the container ticks at #70; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
...
| balance | E2-n | -- | Make the move and delete the registry, with `_FENCED_MODULE_RULINGS` as the LAST commit rather than the first. Its decomposition is decided from #77's trace. | #79 | -- | NOW |
...
| recurrence | R7b-4 | -- | The opening bound became a DATE: "First paycheck" is "Starts on", `start_period_id` folded into `start_date` under a MAXIMUM and is NULL on every row, and the `Every N Periods` PHASE became a derivation of that bound rather than a stored value. Closed **D2**, **D30**; opened **D34**. | SHIPPED | `67f013c8` | -- |
