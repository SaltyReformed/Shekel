...
**150 steps, 105 open.** The dependency graph holds 82 edges over 54 rows.

## The order

| arc | id | also | what this step does | order | commit | starts |
|---|---|---|---|---|---|---|
| bank_import | X-f6e | -- | Make a statement prove itself, asserting that closing minus the sum of its lines equals opening and that the app's own balance for that day equals the bank's, refusing the file when either fails. | #1 | -- | NOW |
...
| credit_card | CC1b | -- | Build `balance_at/_revolving.py`: a card is an event stream of anchor facts, settled rows and projected rows, unwired and additive. | #57 | -- | after #32 / balance:X-f3c / balance:X-f4 / balance:X-am |
...
| credit_card | CC4b | -- | Derive the projected card payment from the statement balance at last close minus redemptions since, as a CARD rule behind the amount resolver rather than a sixth entry in an override map. | #67 | -- | after #32 / balance:X-f4 / balance:X-am |
...
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #71 | -- | after #5 / balance:X-f (the container ticks at #3; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
...
| bank_import | X-f6g | -- | Stop the reconcile panel offering an account whose statements the owner imports, after a specification pass decides what statement coverage means for a day no import spans. | #92 | -- | after #1 / bank_import:X-f6e |
