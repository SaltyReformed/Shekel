...
**151 steps, 111 open.** The dependency graph holds 87 edges over 57 rows.
...
| recurrence | R7d | -- | Stop PERSISTING a loan payment's validity window: `loan_recurrence_sync` becomes a RESOLVER and generation applies the loan's window over the rule's own, deleting ten write chokepoints and the ordering hazard -- which lets `ck_recurrence_rules_valid_window` land, true by construction once every row is authored. Closes **D35**. | #82 | -- | NOW |
