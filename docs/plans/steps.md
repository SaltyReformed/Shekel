...
**151 steps, 110 open.** The dependency graph holds 98 edges over 64 rows.
...
| pay_calendar | C2-f2d-2 | -- | Make `/retirement` compute its retirement projection ONCE per render instead of twice, by loading the gap inputs and the projection batch at the route and handing them to both the verdict producer and the lever solver. Closes **P57**. | #13 | -- | NOW / pay_calendar:C2-f2d-1 (shipped) |
...
| balance | E2-n | -- | Make the move and delete the registry, with `_FENCED_MODULE_RULINGS` as the LAST commit rather than the first. Its decomposition is decided from #77's trace. | #78 | -- | NOW |
