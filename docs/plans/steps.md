...
**135 steps, 103 open.** The dependency graph holds 96 edges over 62 rows.
...
| recurrence | R-F2 | -- | Tighten the ref-seed parity scan's statement boundary. Closes **F-2**. | #74 | -- | NOW |
| recurrence | R-F3 | -- | Resolve the ref-table constraint-naming disagreement. Closes **F-3**. | #75 | -- | NOW |
| recurrence | R-F6 | -- | Close the recurrence-rule leak, then delete the orphaned rules it made. Closes **F-6**. | #76 | -- | NOW |
| recurrence | R-F7 | -- | Delete two unreachable branches in `_first_of_month_anchor`. Closes **D11**. | #77 | -- | NOW |
| recurrence | R-F13 | -- | Close the three holes in this arc's own gate. Closes **F-13**. | #78 | -- | NOW |
...
| balance | E2-n | -- | Make the move and delete the registry, with `_FENCED_MODULE_RULINGS` as the LAST commit rather than the first. Its decomposition is decided from #76's trace. | #80 | -- | NOW |
