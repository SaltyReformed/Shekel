...
**138 steps, 101 open.** The dependency graph holds 97 edges over 63 rows.
...
| recurrence | R7c-b | -- | Move every reader and the recurrence form onto the two-axis columns: `RecurrenceSpec` states one `starts_on`, the form's Day of Month and Month controls are deleted in favour of that one date, the four columns tighten to NOT NULL, and `end_date >= starts_on` lands with the mirror that refuses it at the door. Closes **D10**, **D21**, **D24**, **D28**, **D31**, **D32**. | #19 | -- | NOW / recurrence:R7c-a (shipped) |
| recurrence | R7c-c | -- | Drop what the cutover orphans -- `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id`, `offset_periods`, `start_date` and the unwritten `budget.recurrence_month_anchors` -- and delete the encode / decode pair with the closed pattern set behind it. Closes **D6**. | #20 | -- | after #19 / recurrence:R7c-b |
...
| recurrence | R5 | -- | Split a generated row's dates into three -- `occurs_on` (the cadence), `pay_period_id` (the funding) and `due_on` (the installment) -- and delete `compute_due_date`. A value-splitting migration; own PR. | #24 | -- | after #5 / balance:X-f4 (R5 edits `cash_ledger/_events.py`, which is inside that step's deletion set) |
...
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**. | #74 | -- | after #5 / balance:X-f (the container ticks at #5; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
