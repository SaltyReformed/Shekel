...
**152 steps, 112 open.** The dependency graph holds 88 edges over 58 rows.
...
| pay_calendar | C2-f3e | -- | Give the grid's two create fragments the pay-period ID they actually use instead of the whole ORM row, and make their routes' ownership check the owner's own calendar rather than a hand-written `period.user_id` comparison. Closes **P51**. | #13 | -- | NOW / pay_calendar:C2-f3b (shipped) |
...
| recurrence | R7d | -- | Stop PERSISTING a loan payment's validity window: `loan_recurrence_sync` becomes a RESOLVER and generation applies the loan's window over the rule's own, deleting ten write chokepoints and the ordering hazard -- which lets `ck_recurrence_rules_valid_window` land, true by construction once every row is authored. Closes **D35**. | #84 | -- | NOW |
...
| recurrence | R5 | -- | Split a generated row's dates into three -- `occurs_on` (the cadence), `pay_period_id` (the funding) and `due_on` (the installment) -- and delete `compute_due_date`. A value-splitting migration; own PR. | #19 | -- | after #4 / balance:X-f4 (R5 edits `cash_ledger/_events.py`, which is inside that step's deletion set) |
...
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #72 | -- | after #4 / balance:X-f (the container ticks at #4; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
...
| recurrence | R-F17 | -- | Derive the two period-INDEX horizon windows from the owner's cadence instead of a hardcoded 26, deciding first what a fractional period offset means. Closes **F-17**. | #82 | -- | NOW / recurrence:R7a-2a (shipped) |
| recurrence | R10-b | -- | Move the transfer recurrence engine onto ruling **R-R19**'s maintain shape, so a regeneration updates the transfers it already generated -- through `transfer_service.update_transfer`, which propagates to both shadows atomically -- instead of hard-deleting and recreating them with their shadow pairs. | #83 | -- | NOW / recurrence:R10-a (shipped) |
