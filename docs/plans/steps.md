...
| recurrence | R7b-4 | -- | Replace the "First paycheck" period picker with a "Starts on" DATE, folding `start_period_id` into `start_date` in a migration measured equal on every live rule and deleting the four readers that fold makes dead. Closes **D2**, **D30**. | #18 | -- | NOW / recurrence:R7b-3 (shipped) |
