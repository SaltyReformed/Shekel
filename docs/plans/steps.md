...
**152 steps, 109 open.** The dependency graph holds 97 edges over 63 rows.
...
| recurrence | R7c-c | -- | Drop what the cutover orphans -- `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id`, `offset_periods`, `start_date` and the unwritten `budget.recurrence_month_anchors` -- re-point `interval_n` off the encoding, and delete the encode / decode pair with the closed pattern set behind it. Closes **D6**, **D32**. | #20 | -- | NOW / recurrence:R7c-b (shipped) |
...
| balance | X-y | -- | Move the fifteen surfaces that resolve the baseline scenario directly onto the seam, deciding what a WRITE may do without one. Closes **N-117**. | #34 | -- | after #33 / balance:X-x (the container ticks at #34) |
...
| balance | X-ak | -- | Rule the stored transfer-to-shadow copy first, then unify the THREE mirror implementations that already disagree. Closes **N-148**, **N-150**, **N-152**, **N-156**, **N-159**, **N-170**. | #37 | -- | after #36 / balance:X-aj (the container ticks at #37) |
...
| balance | X-d | -- | Make the posted account ledger a CHECKED PROJECTION: the posting writer consumes the walk instead of its own, with a per-visible-date assert. Carries **N-135**. | #43 | -- | after #42 / balance:X-ai (the container ticks at #43; N-155 parked this step and the assert's placement is the restructure's) |
...
| balance | E2-n | -- | Make the move and delete the registry, with `_FENCED_MODULE_RULINGS` as the LAST commit rather than the first. Its decomposition is decided from #76's trace. | #77 | -- | NOW |
...
| recurrence | R7c | -- | The DECOMPOSED parent of THE CUTOVER, split into three leaves 2026-08-14 (R-R18) as an expand / migrate / contract: the columns land, the readers move, then the closed set dies. | container | -- | ticks with #20 |
