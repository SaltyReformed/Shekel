...
**151 steps, 111 open.** The dependency graph holds 99 edges over 65 rows.
...
| pay_calendar | C2-f2d-2 | -- | Make `/retirement` compute its retirement projection ONCE per render instead of twice, by loading the gap inputs and the projection batch at the route and handing them to both the verdict producer and the lever solver. Closes **P57**. | #14 | -- | NOW / pay_calendar:C2-f2d-1 (shipped) |
| pay_calendar | C2-f2d-3 | -- | Move `/savings`'s pay-period readers onto the pass's calendar, changing the type its two bundle dataclasses carry, and take all four of the package's public entries onto the read pass in the same commit. Closes **P58**. | #15 | -- | NOW / pay_calendar:C2-f2d-1 (shipped) |
...
| recurrence | R7c-c | -- | Drop what the cutover orphans -- `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id`, `offset_periods`, `start_date` and the unwritten `budget.recurrence_month_anchors` -- re-point `interval_n` off the encoding, and delete the encode / decode pair with the closed pattern set behind it. Closes **D6**, **D32**. | #22 | -- | NOW / recurrence:R7c-b (shipped) |
...
| balance | X-y | -- | Move the fifteen surfaces that resolve the baseline scenario directly onto the seam, deciding what a WRITE may do without one. Closes **N-117**. | #36 | -- | after #35 / balance:X-x (the container ticks at #36) |
...
| balance | X-ak | -- | Rule the stored transfer-to-shadow copy first, then unify the THREE mirror implementations that already disagree. Closes **N-148**, **N-150**, **N-152**, **N-156**, **N-159**, **N-170**. | #39 | -- | after #38 / balance:X-aj (the container ticks at #39) |
...
| balance | X-d | -- | Make the posted account ledger a CHECKED PROJECTION: the posting writer consumes the walk instead of its own, with a per-visible-date assert. Carries **N-135**. | #45 | -- | after #44 / balance:X-ai (the container ticks at #45; N-155 parked this step and the assert's placement is the restructure's) |
...
| pay_calendar | C2-f2d | -- | The DECOMPOSED parent of `/savings` and `/retirement`, split into three leaves 2026-08-16 exactly as its own bullet predicted: the read pass, then `/retirement`'s double projection, then `/savings`'s readers. | container | -- | ticks with #15 |
