...
**150 steps, 109 open.** The dependency graph holds 86 edges over 58 rows.
...
| bank_import | X-gd | -- | Promote a stated merchant answer from a suggestion to a standing RULE (**R-GI**) -- stored with its scope, revocable where it was stated, receipted on every application, and read at import time -- shipping the rule store one leaf before the door that acts on it. | #4 | -- | NOW |
| bank_import | X-ge | -- | Let a standing rule AUTO-APPLY at import for a NEW swipe line only (**R-GH**), **MOVES MONEY, OWN PR**: the line becomes a purchase in the rule's destination, dated by the bank and itemised on a receipt whose every item carries X-f6f's one-click undo, while every act that would modify a hand-made row keeps its tick. | #5 | -- | after #4 / bank_import:X-gd |
...
| balance | X-au-e | -- | Cut TEMPLATE rows over to their template's amount series: generation stops pricing them, the 511 non-override rows go NULL, and regeneration's amount arm with the conflict chooser's keep-vs-use decision are both deleted. | #8 | -- | NOW / balance:X-au-c3 / balance:X-au-a (shipped) |
...
| pay_calendar | C4 | -- | Drop `pay_periods.end_date` and `period_index` with their three constraints, once the ORM readers take their bounds from the calendar. Closes **P1**, **P4**, **P5**, **P8**, **P9**. | #15 | -- | NOW / pay_calendar:C2 (shipped) / pay_calendar:C3 (shipped) |
...
| recurrence | R5 | -- | Split a generated row's dates into three -- `occurs_on` (the cadence), `pay_period_id` (the funding) and `due_on` (the installment) -- and delete `compute_due_date`. A value-splitting migration; own PR. | #20 | -- | after #3 / balance:X-f4 (R5 edits `cash_ledger/_events.py`, which is inside that step's deletion set) |
...
| balance | X-y | -- | Move the fifteen surfaces that resolve the baseline scenario directly onto the seam, deciding what a WRITE may do without one. Closes **N-117**. | #33 | -- | after #32 / balance:X-x (the container ticks at #34) |
...
| balance | X-ak | -- | Rule the stored transfer-to-shadow copy first, then unify the THREE mirror implementations that already disagree. Closes **N-148**, **N-150**, **N-152**, **N-156**, **N-159**, **N-170**. | #36 | -- | after #35 / balance:X-aj (the container ticks at #37) |
...
| balance | X-ai-s | -- | Migrate `journal_entries` to an EXCLUSIVE ARC of typed FKs with an AT-MOST-ONE check, plus the reversal linkage. Held until the cutover, which deletes the correction family it would buy attribution for. Closes **N-167**. | #41 | -- | after #1 / balance:X-f3c |
| balance | X-d | -- | Make the posted account ledger a CHECKED PROJECTION: the posting writer consumes the walk instead of its own, with a per-visible-date assert. Carries **N-135**. | #42 | -- | after #41 / balance:X-ai (the container ticks at #43; N-155 parked this step and the assert's placement is the restructure's) |
...
| balance | X-ah | -- | Rule each of the 34 `request.args.get(..., type=int)` sites and parse them like every other id, with a second rule that admits a meaningful zero. Closes **N-142**. | #53 | -- | NOW |
...
| balance | X-ba | -- | Collapse `app/ref_cache`'s twenty-six near-identical id accessors into one generic lookup, delete the two that have no caller anywhere, and migrate the call sites, so the repetition that took the module past pylint's line ceiling stops being paid for by splitting it. Closes **N-341**. | #55 | -- | NOW |
| credit_card | CC0a | -- | Add `has_revolving_credit` to account types with the `REVOLVING` projection kind and its classifier branch, behind behavior-preserving shims. | #56 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC0b | -- | Add the `budget.credit_card_params` satellite model and its migration, inert by design. | #57 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC0c | -- | Add the card params setup flow: create and update routes, the Marshmallow schema, and the REVOLVING setup redirect. | #58 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC1a | -- | Make the card consume the shared instant-partition fold core rather than growing a second copy of it. | #59 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC1b | -- | Build `balance_at/_revolving.py`: a card is an event stream of anchor facts, settled rows and projected rows, unwired and additive. | #60 | -- | after #34 / balance:X-f3c / balance:X-f4 / balance:X-am |
| credit_card | CC1c | -- | Dispatch REVOLVING to the fold at four seam surfaces, deleting the CC0a shim and explaining every moved number. | #61 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC2a | -- | Derive the statement cycle purely: `cycle_window`, `statement_sequence`, `due_date_for`, `statement_balance`, `grace_kept` and `minimum_payment`. | #62 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC2b | -- | Fold the daily balance into a finance charge over effective-dated APR segments, with purchases joining the average daily balance on grace loss. | #63 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC2c | -- | Ride card APR history on `rate_history` through a card-gated write route, pinning that the loan loaders never see the card. | #64 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC3a | -- | Add `charge_to_card` and its undo as an additive action with row locking, guards and provenance, beside the still-live mark-credit. | #65 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC3b | -- | Cut mark-credit over to charge-to-card at the transaction level, with an in-migration backfill of every live pair and a provenance-restoring downgrade. | #66 | -- | after #34 / balance:X-f1 (shipped; absorbed the X-f1b leaf this once named) / balance:X-f4 / balance:X-am |
| credit_card | CC3c | -- | Rewrite `entry_credit_workflow.py` as `entry_card_charge.py` for split tender, rename `is_credit` to `is_card_tender`, and delete `credit_workflow.py` whole. | #67 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC3d | -- | Refuse what the card cannot model -- transfers OUT of the card -- and give `active_accounts_query` an orthogonal `revolving` filter. | #68 | -- | after #34 / balance:X-f4 / balance:X-am |
| bank_import | X-gg | -- | Run the ENVELOPE-SEMANTICS design loop with the developer (**R-GK**): how an envelope fills from rule-assigned swipes and stays open through its period, what closes it when statement coverage completes, what carry-forward then means, and which row identity the grid renders (the same-name double-render the review measured) -- the loop mints the build steps rather than building. | #69 | -- | after #67 / credit_card:CC3c (the loop designs over the card-tender entry shape CC3c rewrites; designing over the payback shape the card arc deletes is what withdrew balance:X-au-i) |
| credit_card | CC4a | -- | Add `card_payment_settings` with its payment-mode ref table and the creation flow mirroring the loan payment transfer. | #70 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC4b | -- | Derive the projected card payment from the statement balance at last close minus redemptions since, as a CARD rule behind the amount resolver rather than a sixth entry in an override map. | #71 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC4c | -- | Warn on underpayment with a one-click "pay statement balance", and maintain ONE projected finance-charge expense when grace fails. | #72 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC5a | -- | Accrue rewards as a derived figure over settled purchases minus redemptions, carrying the `system_origin_id` migration. | #73 | -- | after #34 / balance:X-f4 / balance:X-am |
| credit_card | CC5b | -- | Add manual redemptions and the auto-redeem threshold, holding the one-live-row invariant under the concurrency shape. | #74 | -- | after #34 / balance:X-f4 / balance:X-am |
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #75 | -- | after #3 / balance:X-f (the container ticks at #4; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
...
| balance | E2-n | -- | Make the move and delete the registry, with `_FENCED_MODULE_RULINGS` as the LAST commit rather than the first. Its decomposition is decided from `balance:E2-0`'s trace. | #77 | -- | NOW |
...
| recurrence | R7d | -- | Stop PERSISTING the CLOSING half of a loan payment's validity window -- `loan_recurrence_sync` becomes a resolver that generation applies over the rule's own bound, deleting nine of its ten write chokepoints -- so `ck_recurrence_rules_valid_window` lands true by construction while `starts_on` stays as the cadence anchor. Closes **D35**. | #85 | -- | NOW |
...
| bank_import | X-f6g | -- | Stop the reconcile panel offering an account whose statements the owner imports, after a specification pass decides what statement coverage means for a day no import spans. | #96 | -- | NOW |
...
| balance | X-x | -- | The DECOMPOSED parent of the one-pay-calendar-precondition work. Closes **N-116**, **N-125**, **N-126**, **N-129**. | container | -- | ticks with #32 |
...
| pay_calendar | C5 | -- | The DECOMPOSED parent of "the gap machinery goes, and a paycheck may owe one template twice", split 2026-08-09. | container | -- | ticks with #25 |
...
| bank_import | X-f6 | -- | The DECOMPOSED parent of the statement importer, whose first leaf moved AHEAD of the cutover because what the cutover needed was clearing facts rather than an import surface. Carries **N-173**. | container | -- | ticks with #96 |
