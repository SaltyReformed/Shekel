...
**148 steps, 107 open.** The dependency graph holds 83 edges over 55 rows.

## The order

| arc | id | also | what this step does | order | commit | starts |
|---|---|---|---|---|---|---|
| bank_import | X-f6e-1 | -- | Make a statement's stated balance an OBSERVATION with a solved effective day and a recorded basis, dropping the two balance columns derived from it, so a file whose figure no day explains is refused and one whose header merely lags is not. | #1 | -- | NOW |
...
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #72 | -- | after #6 / balance:X-f (the container ticks at #4; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
...
| bank_import | X-f6g | -- | Stop the reconcile panel offering an account whose statements the owner imports, after a specification pass decides what statement coverage means for a day no import spans. | #93 | -- | after #2 / bank_import:X-f6e |
