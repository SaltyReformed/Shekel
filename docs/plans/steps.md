...
**150 steps, 107 open.** The dependency graph holds 82 edges over 54 rows.

## The order

| arc | id | also | what this step does | order | commit | starts |
|---|---|---|---|---|---|---|
| bank_import | X-f6d-4 | -- | Make a group's residual an ordinary uncategorized row the owner accepts, rather than a difference the door refuses outright or an invisible plug to anchor equity. | #1 | -- | NOW |
| bank_import | X-f6e | -- | Make a statement prove itself, asserting that closing minus the sum of its lines equals opening and that the app's own balance for that day equals the bank's, refusing the file when either fails. | #2 | -- | NOW |
...
| arc | id | also | what this step does | order | commit | starts |
|---|---|---|---|---|---|---|
| bank_import | X-f6d | -- | The DECOMPOSED parent of the matcher's CORRECTION half, whose three leaves make a near miss visible, let a match disagree, and make matching the cheap act. | container | -- | ticks with #1 |
