...
**146 steps, 107 open.** The dependency graph holds 99 edges over 65 rows.
...
| balance | X-ai-s | -- | Migrate `journal_entries` to an EXCLUSIVE ARC of typed FKs with an AT-MOST-ONE check, plus the reversal linkage. Held until the cutover, which deletes the correction family it would buy attribution for. Closes **N-167**. | #46 | -- | after #3 / balance:X-f3c |
