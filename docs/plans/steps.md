...
**138 steps, 100 open.** The dependency graph holds 102 edges over 64 rows.
...
| recurrence | R7b-4 | -- | Replace the "First paycheck" period picker with a "Starts on" DATE, folding `start_period_id` into `start_date` in a migration measured equal on every live rule and deleting the four readers that fold makes dead. Closes **D2**, **D30**. | #17 | -- | NOW / recurrence:R7b-3 (shipped) |
| recurrence | R7c | -- | THE CUTOVER: one migration adds the two-axis columns, backfills them, and drops `pattern_id`, `day_of_month`, `month_of_year`, `start_period_id` and `offset_periods`. It must RULE row **D28** first. Closes **D10**, **D12**, **D21**, **D24**. | #18 | -- | after #17 / recurrence:R7b-4 |
...
| balance | X-f6 | -- | Replace the date GUESS with the bank's own record via OFX / CSV / Plaid import, consuming the outstanding set and the residual path. Its first act is a trace, not code. Closes **N-173**. | #72 | -- | after #3 / balance:X-f5 (legal from #4; block 5 SCHEDULES it after the card arc so ONE matching rule covers checking and card rows rather than being widened into them later) |
| balance | X-p | -- | Put the analytics calendar's day chips and its balance line on one clock, or render the row that explains the gap. Closes **N-58**, **N-97**. | #73 | -- | after #72 / balance:X-f (the container ticks at #71; the import shrinks the date noise at its source, so ruling before it decides against numbers that then change) |
