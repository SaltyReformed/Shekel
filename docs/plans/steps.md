...
**147 steps, 102 open.** The dependency graph holds 85 edges over 55 rows.
...
| balance | X-au-j | -- | Give the review pass ONE amount basis for the whole pass instead of one per offered row: `ReviewScope` derives it and threads it as it already threads the calendar, and both `settle_amount` twins take an optional one. Closed **N-295**, **N-309**. | SHIPPED | `340c2675` | -- |
