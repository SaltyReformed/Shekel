...
- [ ] **X-hf** `feat(import): the card on the feed` -- design 3.11 of `credit_card_from_scratch.md`:
      confirm SimpleFIN serves the card as it serves SECU, map the card account (the mapping form
      serves what `serves_cash_detail` admits, a Credit Card among them), land its lines in the one
      inbox on the card (purchase lines matching card movements or minting them, payment lines
      disposed TRANSFER against the card leg, the closing balance a level), the Capital One CSV
      adapter the manual fallback. After `X-f6b-2` and `credit_card:CC-5-4a-2`. Closes **BI-506**.
