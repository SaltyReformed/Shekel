...
- [ ] **X-f6** `feat(import): the bank says when money moved` -- the DECOMPOSED parent of the
  ...
  - [ ] **X-f6a** the DECOMPOSED parent of the importer's CORE (**R-FP**), carrying **N-173**, split
    ...
    - [ ] **X-f6a-3c** `feat(import): review a statement in one pass` -- the reviewed MULTI-SELECT
          finding **N-306** says the volume owes, and the bounds this screen still hides.
          **READ `balance:X-au-j` (#6) FIRST, or measure before building.** It is legal to start
          ...
          `candidates_for` -- finding **N-309** measures that at `4.7 s` on the developer's own
          data, and the existing-envelope arm pays a second full scan through `destinations_for`. A
          multi-select over 91 lines therefore costs roughly 91 times that unless N-309 is closed
          first or this leaf takes a BATCH rather than looping the single-line door.
          ...
          **It also owns what the screen does not admit**: `undated_pool_too_large()` returns 674
          and is rendered nowhere (**N-315**), and that pool is 89% forward projections the
          statement could never show (**N-316**) -- `_reads.review_set` already computes the covered
          span for its own panel and never passes it to the proposer, which is also the only bound
          available to **N-312**. **N-317** is the third: the parent/purchase guard is wider than a
          purchase born already posted needs.
