...
- [ ] **CC-5-4a-5** `fix(cards): the panel and carry-forward say what they free first` -- `R-CC76`:
      ...
      widening this step, is the developer's question. Checking's own list, ticking a reopened
      card-paid bill, re-points its kept payment (`status_seam._covering._re_point`) and withdraws a
      card match through `match_withdrawal.withdraw_for_moved_movement` with no caption
      (**CC-378**): its builder asks the developer whether that list captions such a row
      (`paid from <card>`) or omits it. After 4a-4 (`R-CC76`). Closes **CC-364**, **CC-378**.
- [x] **CC-5-4b** `30e7ddbb0` -- the "Paid from this account" list (`R-CC44`), the bill arm's second
      ...
      card predicate, where an id compare would be a second (`R-CC117`, rule 14). Checking's list
      still offers the row under a standing rule (`R-CC43`), never ruled for the panel (**CC-378**).
