...
- **Wave 1 - system fixes (Fable, CSS/tokens/templates; one session).** RC1 light `.card` skin; RC2
  component skins (+ P-ST1 active-state bug); RC4 muted token bump (+ light accent nudge if
  ratified); RC5 type floor sweep; RC7 chart theme completion; RC3 mechanical replacements where the
  correct token is unambiguous (`table-light`, `bg-info`, statement green/red, `code` color). Every
  page improves before any page is redesigned.
