...
| **Accounts** (third target; unifies `/savings` + `/accounts`) | IN PROGRESS -- audit + Gate A locked in `accounts_audit.md`. Loop A DONE 2026-06-24: direction **B** chart-forward "Net Worth Cockpit" chosen from 3 scratch directions; rulings + build refinements recorded in the audit. NEXT: the Property-asset mini-sprint (Opus; prerequisite that also fixes the analytics net-worth skew), then Loop B (net-worth producer + unified template + `accounts.css` + charts + `balanceChanged` wiring + `/accounts` redirect). |
| Remaining screens | PROVISIONAL ORDER from the original plan (confirm per screen at each start): salary, analytics, retirement, investment, loan, settings. The app-wide navbar/IA rework is its own area. Screens the original list omits (recurring, transfers, obligations, companion, calendar) slot in per developer call. |
