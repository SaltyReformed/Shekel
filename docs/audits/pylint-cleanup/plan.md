...
`duplicate-code` (R0801) clusters, zero `useless-suppression`, zero E/F; 126 visible messages.
Full suite 5769 passed.** Phase 3 (design smells) has NINETEEN files plus the form-mutation helper
family complete. The newest, **`services/budget_variance_service.py` DONE** (`b5a9d56`), cleared all 3
...
Next by live density (re-measured 2026-06-06 after `services/budget_variance_service.py` DONE
`b5a9d56`; 36 smell items remain [31 of the 8-symbol set + 5 `too-many-instance-attributes`], down
from 39): the band at 3 -- **`app/ref_cache.py`** (`init` tm-locals/branches/statements) and
**`services/calibration_service.py`** (`derive_effective_rates` tm-args/pos/locals). For `ref_cache`
the verified-clean approach is a genuine DRY registry refactor: `init` hand-writes 12 ref tables 4x
each (a `_load_rows` call, an `unavailable` tuple row, a `or {}` normalization, a near-identical
enum-sweep loop); grep-confirmed nothing outside `ref_cache.py` references the 13 module maps, so the
5 `global`-rebind disables can drop in favor of in-place `clear()`/`update()`. For `calibration` the
6 inputs are one cohesive pay-stub snapshot, so a `PayStubActuals`-style param object (clears
args/pos/locals together) is the likely move over a disable. Plan-first per the developer's cadence.
The remaining `too-many-instance-attributes` after this file: `calendar_service` (2),
