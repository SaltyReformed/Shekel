...
`duplicate-code` (R0801) clusters, zero `useless-suppression`, zero E/F; 99 visible messages.
Full suite 5771 passed.** Phase 3 (design smells) has THIRTY-FOUR files plus the form-mutation helper
family complete. **The low-fork batch is essentially done -- 10 remaining smell items: 5
`too-many-instance-attributes` (the "prefer restructure" group: `calendar_service`,
`spending_trend_service`, `carry_forward_service`) + 5 of the 8-symbol set (`balance_calculator`
tm-branches [CONSULT pending -- invariant-critical], the `carry_forward_service`/`schemas/validation`
module tm-lines [package splits], `transfer_service.update_transfer` tm-locals [critical] + the
auth/transfer_service/budget_variance disabled smells).** The newest, **`services/entry_service.py`
DONE** (`d641e06`), cleared `create_entry`'s `too-many-arguments` + `too-many-positional-arguments`
...
Next by live density (re-measured 2026-06-07 after **`services/entry_service.py` DONE** `d641e06`; 10
smell items remain [5 of the 8-symbol set + 5 `too-many-instance-attributes`], down from 12).
**The low-fork batch is complete.** What remains is the developer-gated FORK work: `balance_calculator`
(CONSULT pending), the instance-attr restructures, the package splits, and the 2 criticals.
