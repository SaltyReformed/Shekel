...
`duplicate-code` (R0801) clusters, zero `useless-suppression`, zero E/F; 98 visible messages.
Full suite 5771 passed.** Phase 3 (design smells) has THIRTY-FIVE files plus the form-mutation helper
family complete. **9 remaining smell items: 5 `too-many-instance-attributes` (the "prefer restructure"
group, NEXT: `calendar_service` DayEntry/MonthSummary, `spending_trend_service` ItemTrend/TrendReport,
`carry_forward_service` CarryForwardPlan) + 4 of the 8-symbol set: `account_service.create_account`
tm-args (a clean low-fork param-object missed from the batch), the `carry_forward_service` +
`schemas/validation` module tm-lines (package splits, decision #5), and `transfer_service.update_transfer`
tm-locals (CRITICAL).** (The `auth.py` + `transfer_service` + `budget_variance` smells live behind
documented disables -- a separate disposition.) The newest, **`services/balance_calculator.py` DONE**
...
Next by live density (re-measured 2026-06-07 after **`services/balance_calculator.py` DONE** `681fb04`;
9 smell items remain [4 of the 8-symbol set + 5 `too-many-instance-attributes`], down from 10). What
remains: **(NEXT, developer-chosen) the 5 instance-attr restructures** (`calendar_service`,
`spending_trend_service`, `carry_forward_service` -- prefer restructure, analyze + recommend per class);
`services/account_service.py` (`create_account` tm-args -- a clean low-fork param-object missed from the
batch); the package splits (`carry_forward_service` + `schemas/validation` module tm-lines); and the 2
criticals (`auth.py`, `transfer_service.update_transfer`) + the `budget_variance` tm-args disables.
...
The remaining low-fork files: `services/balance_calculator.py`
(`calculate_balances` tm-branches + the `:121` block disable -- CONSULT pending). The tm-args
param-object files (`tax_calculator`, `entry_service`) are DONE. (The instance-attr files
