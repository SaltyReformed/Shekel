...
percent-helper/envelope rule) + 16 domain modules mirroring the consuming route packages + `__init__`
...
- **`auth.py` package split** (decision #5; deferred from Tier-2 functions-first) -- the module
  ...
  STILL OPEN as of 2026-06-11 (`app/routes/auth.py` is 1358 lines, disable at line 21).
...
CarryForwardPlan (10/7) -- flat irreducible value aggregates (every field an irreducible column read flat
by a row-rendering surface; NO consumer reads any sub-group as a unit), so a scoped+named+commented
disable + Pylint-note docstring is correct, NOT a restructure: four of the five cannot reach 7 with one
nested object, and TrendReport (8/7) reads its window fields scattered (gate/banner/CSV header/filename),
never as a bundle. The developer LEANED restructure; the verified evidence (field counts, threshold
arithmetic, consumer-access shape) overrode it -- faithful to the "bundle only for a cohesive named
concept" rule; `AmortizationRow`/`PayoffScenarios`/`ProjectedBalance`/`RetirementGapAnalysis` precedent. A
separate FIRST commit `98fcb91` removed dead output: `MonthSummary.large_transactions` was computed +
stored but read by NO production code (only redundant test assertions; `is_large` stays fully tested via
`day_entries`), MonthSummary 10->9 attrs. Reviewer REFINE applied: the DayEntry note's "1:1
column-per-field" claim was false (CSV emits 9 cols, omits `transaction_id`, folds is_income/is_paid) --
corrected. visible 98->93; smell items 9->4 (4 8-symbol, 0 instance-attr); score 9.93 held; full suite
5771 passed. (Examined: `DayEntry.transaction_id` has no production reader -- but, unlike the derived
`large_transactions`, it is the row's identity/natural key, so the developer KEPT it as the DTO's identity
field [decided 2026-06-07], not a dead-field removal.) The previous, **`services/balance_calculator.py` DONE**
(`681fb04`), cleared `calculate_balances`'s `too-many-branches` (13/12) by extracting its self-contained
phase 2 (the stale-anchor detection scan, ~6 branches) into `_detect_stale_anchor(periods,
anchor_period_id, txn_by_period) -> bool`; phase 1 (the balance forward-roll) untouched, 13->~7 branches,
no disable. **Transfer Invariant #5 HOLDS** (reviewer-confirmed: neither function queries -- the helper
reads only the in-memory `txn_by_period`; the invariant binds the caller, and this adds zero queries).
The early `return True` proven equal to the old flag+double-break. The `:121` disable on the sibling
`calculate_balances_with_interest` stays (its 6 args ARE cohesive + irreducible -- the smell here was a
real second concern, not inherent). Independent quality-pass: behavior_equivalent=yes (line-by-line),
all ACCEPT, 0 REVERT-OVERREACH. 0 disables (82); 0 new R0801; visible 99->98; smell items 10->9 (4
8-symbol + 5 instance-attr); score 9.93 held; full suite 5771 passed. Before it,
**`services/entry_service.py` DONE** (`d641e06`), cleared `create_entry`'s `too-many-arguments` + `too-many-positional-arguments`
(6/5) by bundling the 4 CONTENT fields (amount/description/entry_date/is_credit) into the new frozen
`EntryDetails` -- the add-purchase form's inputs, "mirroring the FORM not the TransactionEntry table"
(the reviewer's framing) -- keeping `transaction_id`/`user_id` as explicit routing on a real seam (the
developer chose this over the all-6 entity-column-mirror, mindful of the `build_recurring` precedent;
none of a structural reduction was available, and a disable was rejected). tm-args 6->3. Behavior
bit-identical (all 5 guard paths untouched; only the `TransactionEntry` construction + log_event read
`details.X`); 41 call sites updated. Independent quality-pass: behavior_equivalent=yes (line-by-line),
all ACCEPT, 0 REVERT-OVERREACH. 0 disables (82); 0 new R0801; visible 101->99; smell items 12->10 (5
8-symbol + 5 instance-attr); score 9.93 held; full suite 5771 passed. Before it,
**`services/tax_calculator.py` DONE** (`2cdde52`), cleared BOTH smells on
`calculate_federal_withholding` (IRS Pub 15-T) -- `too-many-arguments` (9/5) + `too-many-locals`
(21/15) -- by bundling the 6 keyword-only W-4 inputs into the new frozen `W4Inputs` dataclass (a
genuine cohesive concept -- the employee's W-4 / pre-tax adjustments; the `feedback_tm_args` textbook
case) whose `__post_init__` does the `Decimal(str(...))`/`int(...)` coercion (`object.__setattr__`,
frozen), so the function reads `w4.X` directly (which is what clears tm-locals); `gross_pay`/
`pay_periods`/`bracket_set` stay out (withholding context, not W-4 data); Step 5 extracted to
`_dependent_credits`. tm-args 9->4, tm-locals 21->12; NO disable. Behavior verified BY EXECUTION (the
reviewer reproduced every coercion + the 6 IRS steps + the credit calc + the 5 validation errors
against HEAD); the immutable `w4=W4Inputs()` default preserves "all W-4 optional". 37 call sites
updated (paycheck_calculator wraps the profile W-4 in `W4Inputs(...)`; 9 W-4-arg test calls wrapped;
~27 no-W-4 calls unchanged). Independent quality-pass: behavior_equivalent=yes, all ACCEPT, 0
REVERT-OVERREACH. 0 disables (82); 0 new R0801; visible 103->101; smell items 14->12 (7 8-symbol + 5
instance-attr); score 9.92->9.93; full suite 5771 passed. Before it,
**`services/loan_payment_service.py` DONE** (`1cfbcdb`), cleared both
tm-locals smells by INTERNAL helper extraction (public signatures unchanged, external callers
unaffected; financial file): `prepare_payments_for_engine` (16/15) -> 8 by extracting Step 2 (the
biweekly same-due-month redistribution while-loop) into `_redistribute_to_distinct_months`, leaving the
P-1-adjacent escrow Step 1 INLINE; `live_loan_transfer_amounts` (17/15) -> 12 by extracting the
per-loan resolve-loop body into `_resolve_loan_piti(...) -> Decimal | None` (the two skip-loan
`continue`s -> `return None`; the deferred `loan_resolver` import + disable moved in). Behavior
bit-identical (reviewer byte-level-diff verified: both helper bodies identical, escrow Step 1 + the P-1
functions untouched, the import moved not lost). Independent quality-pass: behavior_equivalent=yes, all
ACCEPT, 0 REVERT-OVERREACH (leaving the P-1-adjacent escrow step inline ruled the correct conservative
call). 0 disables (82); 0 new R0801; visible 105->103; smell items 16->14 (9 8-symbol + 5
instance-attr); score 9.92 held; full suite 5771 passed. Before it,
...
5. **Module splits (locked 2026-06-04):** the `too-many-lines` modules (8 remaining: `auth.py`,
   ...
   packages**, NOT closed with a documented `too-many-lines` disable. (Status: `carry_forward_service`
   DONE `4f7737e`; **`schemas/validation.py` DONE `c3d05de`+`8cda673`** -- 16 domain modules mirroring
   the consuming route packages, the schema-package analogue of the `_bp.py`/`__init__` precedent;
   `auth.py` is the next remaining split.) Follow the
...
| 3 | Design-smell refactors | 158 visible smells + smells revealed by Phase 1 | IN PROGRESS (23 files + the form-helper family done: `carry_forward_service/` (pkg), `transfer_service.py`, `account_service.py`, `calibration_service.py`, `budget_variance_service.py`, `investment_projection.py` (+ `projection_inputs.py` wrapper), `debt_strategy.py` (route), `retirement_gap_calculator.py`, `loan_resolver/`, `growth_engine.py`, `templates.py`, `grid.py`, `salary/`, `amortization_engine.py`, `savings_dashboard_service/`, `year_end_summary_service/`, `transactions/`, `transfers/`, `loan/`, `debt_strategy_service.py`, `investment_dashboard_service.py`, `paycheck_calculator.py`, `retirement_dashboard_service.py`, `_recurrence_form_helpers.py` + `_commit_helpers.py` + `_transfer_creation_helpers.py`, `schemas/validation/` (pkg -- the last visible design smell, now ZERO)) |
...
| app/services/transfer_service.py:283 (create_transfer) | too-many-arguments, too-many-positional-arguments, too-many-locals | **DONE (`36a4fbb`)** -- 12 args bundled into the frozen `TransferSpec` (developer-chosen B); 3 fn-disables -> 1 documented `too-many-instance-attributes` on the 12-field object (`AmortizationRow`/`PayoffScenarios` precedent). `_build_shadow` DRYs the 2 shadow constructions. 65 call sites wrapped. TRANSFER INVARIANTS 1-4 panel-verified preserved. | TRANSFER INVARIANTS apply |
| app/services/transfer_service.py:445 (update_transfer) | too-many-branches, too-many-statements | **DONE (`36a4fbb`)** -- decomposed `_apply_status_change` (F-047/F-048 block) + `_apply_actual_amount`; cleared too-many-locals (18/15) AND the branches/statements disable **REMOVED entirely** (extraction cleared both; no useless-suppression). missing-param-doc on `**kwargs` fixed. | TRANSFER INVARIANTS apply |
...
- services/loan_payment_service.py: **DONE** (`1cfbcdb`; file now 10.00/10, zero messages). Both
  tm-locals cleared by INTERNAL helper extraction (public signatures UNCHANGED -> the external
  `balance_resolver` caller + the in-module `load_loan_context` caller unaffected); FINANCIAL file,
  separate from the OPEN P-1. `prepare_payments_for_engine` (16/15)->8: Step 2 (biweekly
  same-due-month redistribution -- the dense `while (y,m) in allocated_months` cascade + month-rollover
  arithmetic, ~9 locals) extracted VERBATIM into `_redistribute_to_distinct_months(payments,
  payment_day) -> list[PaymentRecord]` (collision-key rationale moved to its docstring); Step 1 (escrow
  subtraction -- P-1-adjacent) deliberately LEFT INLINE. `live_loan_transfer_amounts` (17/15)->12: the
  per-loan resolve-loop body (LoanParams -> `load_loan_context` -> LoanAnchorEvent -> `resolve_loan` ->
  `monthly_payment + monthly_escrow`) extracted into `_resolve_loan_piti(loan_account_id, scenario_id,
  as_of) -> Decimal | None` (the two skip-this-loan `continue`s -> `return None`, mapped by the caller's
  `if piti is not None`); the deferred `from app.services import loan_resolver` import + its
  `import-outside-toplevel` disable + the one-directional-dependency comment MOVED into the helper (net
  disable count unchanged). Behavior bit-identical (reviewer byte-level-diff verified: both helper
  bodies identical to the old inline blocks, the PITI sum + escrow Step 1 + the P-1 functions
  `compute_contractual_pi`/`compute_monthly_payment_baseline` untouched/not-in-diff, `Decimal`
  discipline intact, the import moved not duplicated/lost). Both helpers private + typed (the file IS
  typed). The extract-the-dense-self-contained-sub-step altitude (the
  `interest_projection._days_in_quarter` / `retirement._project_one_account` precedent). **Leaving the
  P-1-adjacent escrow Step 1 inline = the correct conservative call** (reviewer): extracting it for
  symmetry would entangle a behavior-preserving refactor with the open P-1 correctness question. P-1
  remains OPEN and untouched. Independent quality-pass: behavior_equivalent=yes, all ACCEPT, 0 REFINE, 0
  REVERT-OVERREACH. 0 disables added (82); 0 new R0801; visible 105->103; smell items 16->14; score 9.92
  held; full suite 5771 passed.
...
- services/account_service.py: **DONE** (`53809b5`; `create_account` tm-args 6/5 cleared by bundling
  the six cohesive creation fields into a new frozen `AccountSpec` param object -- the textbook
  public-fn move [[feedback_tm_args_param_object]], W4Inputs/PayStubActuals/EntryDetails precedent).
  `**extra_columns` kept as an open-ended passthrough (15 sites pass `is_active`/`sort_order`; folding
  them in would push the spec to 8 attrs and trip too-many-instance-attributes -- right altitude). 201
  call sites updated (the /accounts `**data`-splat route hand-edited; 200 via a deterministic AST pass
  preserving every value verbatim, referencing `account_service.AccountSpec`, zero new imports).
  Behavior bit-identical (full suite 5771); 10.00/10; two independent rubric reviewers ACCEPT /
  behavior_equivalent=yes, 0 REFINE, 0 REVERT-OVERREACH (run `wpprlm9ni`). See the quality-pass register.
- services/balance_calculator.py: **DONE** (`681fb04`; `calculate_balances` tm-branches cleared; the
  file's residual 9.91 score is a PRE-EXISTING `line-too-long`:11 in the module docstring [Phase-4] +
  the documented `:121` disable, not a smell). FINANCIAL + Transfer-Invariant-#5-critical, but the
  function QUERIES NOTHING (it takes `transactions` as a param). `calculate_balances` tm-branches
  (13/12) cleared by extracting its self-contained phase 2 -- the stale-anchor detection scan (~6
  branches: walk post-anchor periods for a settled txn) -- into `_detect_stale_anchor(periods,
  anchor_period_id, txn_by_period) -> bool`; phase 1 (anchor coercion + `txn_by_period` grouping + the
  forward-roll loop) UNTOUCHED. 13->~7 branches, no disable. The two phases are genuinely separate
  concerns (phase 1 produces the balances OrderedDict; phase 2 an independent informational boolean
  that does not affect the balances). The helper uses an early `return True` on the first settled
  post-anchor txn -- proven equal to the old flag + inner-break + outer-`if`-break (the loop had no
  work after it), a cleaner guard-clause idiom. **TRANSFER INVARIANT #5 HOLDS** (reviewer-confirmed):
  neither `calculate_balances` nor `_detect_stale_anchor` issues any query -- the helper reads only the
  in-memory `txn_by_period` + `periods`; no `db.session`/`.query`/`Transfer` except docstrings; the
  invariant binds the caller assembling `transactions`, and this adds zero queries. The deliberately
  retained `:121` `too-many-arguments` disable on `calculate_balances_with_interest` stays (those 6 args
  ARE cohesive + irreducible -- the author correctly distinguished extract-the-real-second-concern from
  keep-the-disable-where-inherent). Public contract unchanged; callers (`balance_resolver:488`/`:902`)
  unaffected. Behavior bit-identical (reviewer verified line-by-line; the `TestStaleAnchorWarning` class
  pins every branch: done/received->True, projected/credit/cancelled/no-txn->False,
  done-only-in-anchor->False, warning-does-not-change-balances). Independent quality-pass:
  behavior_equivalent=yes, all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH. 0 disables added (82); 0 new R0801;
  visible 99->98; smell items 10->9; score 9.93 held; full suite 5771 passed.
- services/entry_service.py: **DONE** (`d641e06`; file now 10.00/10, zero messages). `create_entry`
  `too-many-arguments` (6/5) + `too-many-positional-arguments` (6/5) cleared, NO disable. Its 6 args
  exactly mirror the `TransactionEntry` columns (the `build_recurring_transfer_template`
  entity-column-mirror situation, where a column-mirroring param object is the anti-pattern; no
  structural reduction available here, and the project avoids `too-many-arguments` disables), so --
  developer-chosen -- bundle ONLY the cohesive CONTENT sub-group: a new frozen
  `EntryDetails(amount, description, entry_date, is_credit=False)`, the add-purchase form's user inputs
  (submitted + validated together by `EntryCreateSchema`), "mirroring the FORM not the
  `TransactionEntry` table" (reviewer). `transaction_id`/`user_id` stay explicit -- routing/ownership
  context, a different lifecycle (URL + session, not the form) on a real seam. NO `__post_init__` (the
  fields are typed and used directly; the old function did no coercion). Signature
  `create_entry(transaction_id, user_id, details: EntryDetails)`. tm-args 6->3, tm-pos 6->3. Behavior
  bit-identical: all 5 validation/guard paths byte-identical (the diff touches nothing above the
  `TransactionEntry` construction); only the `TransactionEntry(...)` kwargs + the `log_event`
  amount/is_credit now read `details.X` (`amount=str(details.amount)` preserves `str(amount)`). **41
  call sites:** the production route `**data` -> `EntryDetails(**data)` (the schema loads exactly the 4
  keys; `amount` arrives as Decimal -- `as_string` governs dump not load; `is_credit` `load_default=False`
  always present, matching the dataclass default); 39 test calls wrapped (content kwargs into
  `details=EntryDetails(...)`, txn_id/user_id kept explicit -- script-wrapped 38 + 1 manual for a
  comment-bearing call; AST-verified 0 unwrapped). Independent quality-pass: behavior_equivalent=yes
  (line-by-line), all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH -- `EntryDetails` a genuine cohesive concept
  not a count-dodge; the 4-content split the correct altitude over the all-6 column-mirror (which would
  "blur the ownership boundary every guard depends on") and over a disable; Decimal discipline intact.
  0 disables added (82); 0 new R0801; visible 101->99; smell items 12->10; score 9.93 held; full suite
  5771 passed.
...
- services/tax_calculator.py: **DONE** (`2cdde52`; file now 10.00/10, zero messages).
  `calculate_federal_withholding` (IRS Pub 15-T) tripped BOTH `too-many-arguments` (9/5) and
  `too-many-locals` (21/15); both cleared, NO disable. The 6 keyword-only W-4 inputs (`additional_income`,
  `pre_tax_deductions`, `additional_deductions`, `qualifying_children`, `other_dependents`,
  `extra_withholding`) bundled into the new frozen `W4Inputs` dataclass -- a genuine cohesive named
  concept (the employee's W-4 / pre-tax adjustments, sourced + consumed together; the
  [[feedback_tm_args_param_object]] textbook public-fn R0913 case). `W4Inputs.__post_init__` does the
  coercion (`Decimal(str(...))` for the 4 money fields via `object.__setattr__` since frozen; `int(...)`
  for the 2 counts) so the function reads `w4.X` DIRECTLY -- this is what clears tm-locals (re-coercing
  in the function would reintroduce the 6 locals). `gross_pay`/`pay_periods`/`bracket_set` stay out (the
  withholding context, not W-4 data). Signature `(gross_pay, pay_periods, bracket_set, w4=W4Inputs())` --
  the immutable default preserves "all W-4 fields optional, default ZERO/0". Step 5 (dependent credits)
  extracted to `_dependent_credits`. tm-args 9->4, tm-locals 21->12. Validation STAYS in the function
  (reading `w4.X`) so the 5 errors raise at the same points. **Callers (37 sites):** `paycheck_calculator`
  wraps the profile-derived W-4 in `W4Inputs(...)` passing RAW values (W4Inputs coerces -- the
  construct-from-strings discipline now in one place); 9 W-4-arg test calls wrapped in `w4=W4Inputs(...)`;
  the ~27 no-W-4 test calls UNCHANGED (w4 defaults). Behavior bit-identical -- the reviewer verified BY
  EXECUTION on every branch (coercion across int/str/float, the 6 IRS steps byte-identical, `_dependent_credits`
  == the old child+other credit total incl. None/missing `getattr` defaults, the 5 validation errors, the
  immutable default hazard-free via `frozen=True`). `W4Inputs` is the new public surface, documented.
  Independent quality-pass: behavior_equivalent=yes, all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH (the
  coerce-in-`__post_init__` idiom canonical/sound; the lost keyword-only enforcement a non-issue -- now
  guarded inside W4Inputs). 0 disables added (82); 0 new R0801; visible 103->101; smell items 14->12;
  score 9.92->9.93; full suite 5771 passed.
- services/transfer_service.py: **DONE (`36a4fbb`)** -- `create_transfer` (developer-chosen `TransferSpec`
  bundle B + `_build_shadow` DRY) and `update_transfer` (decompose `_apply_status_change` /
  `_apply_actual_amount`, tm-locals cleared + branches/statements disable REMOVED) both cleared.
  TRANSFER INVARIANTS 1-4 panel-verified (3 reviewers: behavior-equivalence / invariants / design --
  all ACCEPT, behavior_equivalent=yes, invariants_hold=yes; lone REFINE [helper type hints] folded in).
...
| services/spending_trend_service.py:52 (`ItemTrend`) | 11/7 | **DISABLE (documented), `cf76c2a`** -- cohesive per-category trend row (11 fields produced in one pass by `_compute_item_trend`, read interleaved by the trends template + emitted column-per-field by `export_trends_csv`); no sub-group (identity/magnitude/trend/timing) read as a unit; `AmortizationRow`/`PayoffScenarios` precedent; scoped+named+commented |
| services/spending_trend_service.py:81 (`TrendReport`) | 8/7 | **DISABLE (documented), `cf76c2a`** -- cohesive trends-tab result aggregate (4 window fields + 4 collections); the 1-over count is NOT a single-nest restructure -- `data_sufficiency`/`window_months`/`threshold` are read scattered (render gate x3, banner, CSV header, route filename), never as a `window` bundle; `PayoffScenarios`/`ProjectedBalance` precedent; scoped+named+commented |
...
| services/calendar_service.py:71 (`DayEntry`) | 10/7 | **DISABLE (documented), `cf76c2a`** -- cohesive calendar-day transaction row read flat by the calendar surface (CSV month export = 9 cols folding is_income/is_paid/is_large/is_infrequent; month-detail table cells; route day-totals read amount/is_income); the two category fields are independent columns, never a unit; `AmortizationRow`/`PayoffScenarios` precedent; scoped+named+commented. (REFINE applied `cf76c2a`: the note's "1:1 column-per-field" claim was false -- CSV emits 9 cols, omits `transaction_id`, folds 2 booleans -- corrected.) |
| services/calendar_service.py:87 (`MonthSummary`) | 10/7 -> 9/7 | **DISABLE (documented), `cf76c2a`**; **dead-field removal FIRST, `98fcb91`** -- `large_transactions` was computed + stored but read by NO production code (only redundant test assertions; the `is_large` classification stays fully tested via `day_entries`), so it was removed (10->9 attrs). The remaining 9 are flat columns read individually by the month/year templates + the CSV year export, no sub-group as a unit; `PayoffScenarios` precedent; scoped+named+commented |
| services/carry_forward_service.py:87 (`CarryForwardPlan`) | 10/7 | **DISABLE (documented), `cf76c2a`** -- cohesive per-row carry-forward action read flat by its sole consumer (the preview modal iterates `preview.plans`, interleaving the rollover figures in one sentence + gating `blocked` styling apart from `block_reason`); `PayoffScenarios` precedent; scoped+named+commented. (Module tm-lines 1027->1038 by the 11-line note: a pre-existing, separately-tracked C0302, decision #5 package split.) |
...
| services/transfer_service.py (`TransferSpec`) | 12/7 | **DISABLE (documented), `36a4fbb`** -- NOT one of the original 13; introduced by the developer-chosen `create_transfer` param-object refactor (B). The 12 irreducible creation inputs of one "transfer to create" request, read as a flat unit by the sole consumer; no cohesive sub-group to nest. A 2-way split would be arbitrary stamp coupling on the invariant-critical path. `AmortizationRow`/`PayoffScenarios`/`AccountSpec`-vs-this (6 attrs needed no disable; 12 do) precedent; scoped+named+commented |
...
Next by live density (re-measured 2026-06-07 after the **5 `too-many-instance-attributes` classes DISPOSED**
`98fcb91`+`cf76c2a`; 4 smell items remain [4 of the 8-symbol set, 0 instance-attr], down from 9). The
instance-attr group is CLOSED -- all 5 (`calendar_service` DayEntry/MonthSummary, `spending_trend_service`
ItemTrend/TrendReport, `carry_forward_service` CarryForwardPlan) are documented disables: a 5-analyst
workflow + an independent reviewer found each a flat irreducible aggregate (no consumer reads a sub-group
as a unit; four cannot reach 7 with one nest; TrendReport's window fields read scattered), so the
developer's restructure lean was overridden by verified evidence -- faithful to the "bundle only for a
cohesive named concept" rule. `services/account_service.py` is now **DONE** (`53809b5`; `create_account`
tm-args cleared via the `AccountSpec` param object -- trust-but-verify corrected the plan's "low-fork"
label: 201 call sites, not low-fork, but the developer chose the param-object over a disable anyway).
`services/transfer_service.py` is now **DONE** (`36a4fbb`; `create_transfer` -> `TransferSpec` bundle
[developer-chosen B] + `_build_shadow` DRY; `update_transfer` decomposed, branches/statements disable
removed; TRANSFER INVARIANTS 1-4 panel-verified). `services/carry_forward_service.py` is now **DONE**
(`4f7737e`; split into a `_context`/`_preview`/`_execute` package per decision #5; the 2 split-trap
R0801 clusters resolved by genuine dedup -- 3 shared `_context` target-row helpers -- not a disable).
What remains: **`schemas/validation.py` module tm-lines is the SOLE remaining visible smell item**; and
`auth.py` (module-split per decision #5 + the `reauth`/`mfa_verify`/`mfa_confirm` return/branch
disables) + the `budget_variance` 3 tm-args disables (both suppressed, not visible).
**Working order was low-fork-batch-first** (developer direction 2026-06-06): clear the
function-decomposition + tm-args-param-object files FIRST, THEN consult on the package splits
(`carry_forward_service`, `schemas/validation`), the instance-attr dispositions, and the 2 criticals
(`auth.py`, `transfer_service.py`). STATUS: the low-fork batch is DONE -- the tm-args param-object files
(`tax_calculator`, `entry_service`) and `services/balance_calculator.py` (`calculate_balances`
tm-branches cleared; the `:121` block disable kept as justified) are complete, and the instance-attr
dispositions RESOLVED to documented disables (`98fcb91`+`cf76c2a`; the developer LEANED restructure but
the verified evidence overrode it -- see the instance-attr register + Next-by-live-density above). Two
files still carry **undisposed Phase-1 smell-disables**
...
unnecessary once the flag became an object attribute. The `too-many-instance-attributes` group is now
CLOSED -- the last 5 (`calendar_service` x2, `spending_trend_service` x2, `carry_forward_service` x1) are
documented disables (`cf76c2a`). **Note on the retirement_gap_calculator approach (trust-but-verify
...
module tm-lines is `schemas/validation.py` (Tier-3 -> package split per decision #5);
`services/carry_forward_service.py` was split in `4f7737e`.
...
| File | Residue |
|---|---|
| app/schemas/validation.py | line-too-long:12, missing-function-docstring:18, missing-class-docstring:1 -- **RESOLVED `c3d05de`+`8cda673`** (the package split eliminated the `too-many-lines` smell and auto-resolved 1 long line via per-module imports; `8cda673` then added all 19 docstrings + reflowed the remaining 11 long lines -> the `app/schemas/validation/` package is 10.00/10) |
...
| 2026-06-06 | `1cfbcdb` | 3 | **loan_payment_service.py -- extract dense sub-steps from two engine helpers (no disable; one moved) -- file DONE:** two tm-locals cleared by INTERNAL helper extraction (public signatures UNCHANGED -> the `balance_resolver.py:398` external caller + the in-module `load_loan_context` caller unaffected); FINANCIAL file, both targets separate from the OPEN P-1. `prepare_payments_for_engine` (16/15)->8: Step 2 (biweekly same-due-month redistribution -- the dense `while (y,m) in allocated_months` defensive cascade + `m += 1 / if m > 12` month-rollover + `calendar.monthrange` clamp, ~9 locals) extracted VERBATIM into `_redistribute_to_distinct_months(payments, payment_day) -> list[PaymentRecord]` (the full collision-key rationale + the Apr-10/Apr-24-both-due-May-1 worked example moved to its docstring); Step 1 (escrow `min(monthly_escrow, p.amount - contractual_pi)` excess-only subtraction -- P-1-adjacent) deliberately LEFT INLINE. `live_loan_transfer_amounts` (17/15)->12: the per-loan resolve-loop body (LoanParams -> `load_loan_context` -> LoanAnchorEvent -> `resolve_loan(LoanInputs(...))` -> `monthly_payment + monthly_escrow` PITI) extracted into `_resolve_loan_piti(loan_account_id, scenario_id, as_of) -> Decimal | None` (the two skip-this-loan `continue`s -> `return None`, mapped by the caller's `if piti is not None: piti_by_loan[id] = piti`); the deferred `from app.services import loan_resolver` import + its `import-outside-toplevel` disable + the one-directional-dependency comment MOVED into the helper (HEAD had 2 such deferred imports, still 2 -- moved not duplicated/lost; `live_loan` no longer references `loan_resolver`). Behavior bit-identical (reviewer byte-level-diff verified each helper body == the old inline block, escrow Step 1 == HEAD, the P-1 functions `compute_contractual_pi`/`compute_monthly_payment_baseline` not in the diff, `Decimal` PITI end-to-end). Both helpers private + typed. The extract-the-dense-self-contained-sub-step altitude (`_days_in_quarter`/`_project_one_account` precedent). **Leaving the P-1-adjacent escrow Step 1 inline ruled the CORRECT conservative call** (reviewer): extracting it for symmetry would entangle a behavior-preserving refactor with the open P-1 correctness question + make the eventual P-1 fix harder to review in isolation. Independent quality-pass (single fresh reviewer): behavior_equivalent=yes, **all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH** (A4 both genuinely-clarifying dense-sub-step extractions, C3 the moved import keeps the one-way dependency, E2 Decimal discipline). P-1 remains OPEN + untouched (correct). file 10.00/10, 0 smell messages; 0 disables added (82); 0 new R0801; useless-suppression 0; E/F 0. Score 9.92 held; visible 105->103; smell items 16->14 (9 8-symbol + 5 instance-attr). 27 loan_payment unit + integration targeted + **full suite 5771 passed.** | 9.92/10 | 103 |
| 2026-06-07 | `2cdde52` | 3 | **tax_calculator.py -- bundle W-4 inputs into W4Inputs (no disable) -- file DONE:** `calculate_federal_withholding` (IRS Pub 15-T) tripped BOTH `too-many-arguments` (9/5) and `too-many-locals` (21/15); both cleared. The 6 keyword-only W-4 inputs bundled into the new frozen `W4Inputs` dataclass -- a genuine cohesive named concept (the employee's W-4 / pre-tax adjustments, sourced + consumed together; the [[feedback_tm_args_param_object]] textbook public-fn R0913 case; `gross_pay`/`pay_periods`/`bracket_set` correctly stay OUT as the withholding context). `W4Inputs.__post_init__` does the coercion (`Decimal(str(...))` x4 money via `object.__setattr__` since frozen; `int(...)` x2 counts), so the function reads `w4.X` DIRECTLY -- the coerce-in-dataclass choice (developer-chosen) is what clears tm-locals (re-coercing in the function would reintroduce the 6 locals; a no-op for tm-locals). Signature `(gross_pay, pay_periods, bracket_set, w4=W4Inputs())` -- the immutable default preserves "all W-4 fields optional". Step 5 (dependent credits) -> `_dependent_credits`. tm-args 9->4, tm-locals 21->12. Validation stays in the function (reading `w4.X`) so the 5 errors raise at the same points (the negative-count guard still fires before Step 5). **37 call sites:** `paycheck_calculator` wraps the profile W-4 in `W4Inputs(...)` passing RAW `getattr(...) or 0` values (W4Inputs coerces -- the construct-from-strings discipline now in one place, byte-identical to the old `Decimal(str(...))`/`int(...)`); 9 W-4-arg test calls wrapped; ~27 no-W-4 test calls UNCHANGED. Independent quality-pass (single fresh reviewer) -- **verified behavior BY EXECUTION** on every branch (W4Inputs coercion across int/str/float; the 6 IRS steps byte-identical reading `w4.X`; `_dependent_credits` == the old `child_credit_total + other_credit_total` incl. real/None/missing `getattr(...,0) or 0` defaults; the 5 validation errors at the same points; the `w4=W4Inputs()` default proven immutable via `FrozenInstanceError`, no mutable-default hazard): behavior_equivalent=yes, **all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH** (A1/A2/A3 W4Inputs a genuine cohesive concept not a count-dodge; A4 `_dependent_credits` clarifying; D2 the frozen+`__post_init__`+`object.__setattr__` coercion canonical/sound; coerce-in-`__post_init__` ruled correct over coerce-in-function; the lost `*` keyword-only enforcement a non-issue, now guarded inside W4Inputs). file 10.00/10, 0 smell messages; 0 disables added (82); 0 new R0801; useless-suppression 0; E/F 0. Score 9.92->9.93; visible 103->101; smell items 14->12 (7 8-symbol + 5 instance-attr). 41 tax + 98 paycheck targeted + **full suite 5771 passed.** | 9.93/10 | 101 |
| 2026-06-07 | `d641e06` | 3 | **entry_service.py -- bundle create_entry's content fields into EntryDetails (no disable) -- file DONE; the low-fork batch's last file:** `create_entry` `too-many-arguments` + `too-many-positional-arguments` (6/5) cleared. Its 6 args exactly mirror the `TransactionEntry` columns -- the `build_recurring_transfer_template` entity-column-mirror situation (where a column-mirroring param object is the anti-pattern; no structural reduction was available here, and the project avoids tm-args disables). So, **developer-chosen (after I surfaced the fork):** bundle ONLY the cohesive CONTENT sub-group into a new frozen `EntryDetails(amount, description, entry_date, is_credit=False)` -- the add-purchase form's user inputs (submitted + validated together by `EntryCreateSchema`), "mirroring the FORM not the `TransactionEntry` table" -- keeping `transaction_id`/`user_id` explicit as routing/ownership context (a different lifecycle: URL + session, not the form) on a real seam. NO `__post_init__` (the fields are typed + used directly; the old function did no coercion). `create_entry(transaction_id, user_id, details: EntryDetails)`. tm-args 6->3, tm-pos 6->3. Behavior bit-identical: all 5 validation/guard paths byte-identical (the diff touches NOTHING above the `TransactionEntry` construction); only the `TransactionEntry(...)` kwargs + the `log_event` amount/is_credit read `details.X` (`amount=str(details.amount)` == the old `str(amount)`). **41 call sites:** the production route `**data` -> `EntryDetails(**data)` (EntryCreateSchema loads exactly amount/description/entry_date/is_credit; `amount` arrives as Decimal -- `as_string` governs dump not load; `is_credit` `load_default=False` always present == the dataclass default); 39 test calls wrapped (content kwargs into `details=EntryDetails(...)`, txn_id/user_id kept explicit -- script-wrapped 38, 1 manual for a comment-bearing call, AST-verified 0 unwrapped). Independent quality-pass (single fresh reviewer; verified line-by-line): behavior_equivalent=yes, **all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH** -- A1/A2/A3 `EntryDetails` a genuine cohesive concept not a count-dodge; the 4-content split the CORRECT altitude over (a) the all-6 column-mirror [which "would blur the ownership boundary every guard depends on"] and (b) a disable; E2 Decimal discipline intact through schema->dataclass->model. file 10.00/10, 0 smell messages; 0 disables added (82); 0 new R0801; useless-suppression 0; E/F 0. Score 9.93 held; visible 101->99; smell items 12->10 (5 8-symbol + 5 instance-attr). 208 entry (service + credit_workflow + log_events + mark_paid + routes) targeted + **full suite 5771 passed.** | 9.93/10 | 99 |
| 2026-06-07 | `681fb04` | 3 | **balance_calculator.py -- extract _detect_stale_anchor (no disable) -- `calculate_balances` tm-branches DONE (developer-consulted approach):** the FINANCIAL, Transfer-Invariant-#5-critical `calculate_balances` (13/12) cleared by extracting its self-contained phase 2 -- the stale-anchor detection scan (~6 branches) -- into `_detect_stale_anchor(periods, anchor_period_id, txn_by_period) -> bool`. Phase 1 (anchor coercion, `txn_by_period` grouping, the forward-roll loop producing `balances`) UNTOUCHED. 13->~7 branches. The two phases are genuinely separate concerns: phase 1 produces money (the balances OrderedDict driving the grid), phase 2 an independent informational boolean that does NOT affect the balances (the `TestStaleAnchorWarning` tests assert this). The helper uses an early `return True` on the first settled post-anchor txn -- the reviewer proved this EQUAL to the old flag + inner-break + outer-`if`-break (the loop had no work after setting the flag), a cleaner guard-clause idiom (D3). **TRANSFER INVARIANT #5 HOLDS (reviewer-confirmed):** neither function issues a query -- the helper reads only the in-memory `txn_by_period` dict + `periods` list passed in; no `db.session`/`.query`/`Transfer` except docstrings; the module imports stay `OrderedDict`/`Decimal`/`calculate_interest`/`is_projected` (no DB/Transfer import). The invariant binds the CALLER that assembles `transactions` (the balance_resolver producers); this refactor adds zero queries and does not weaken the boundary, and restates it correctly in the helper docstring. The deliberately-retained `:121` `too-many-arguments` disable on the sibling `calculate_balances_with_interest` STAYS -- those 6 args ARE cohesive + irreducible; the author correctly distinguished extract-where-the-smell-is-a-real-second-concern from keep-the-disable-where-inherent. Public contract unchanged; callers (`balance_resolver:488`/`:902` + the `_with_interest` wrapper) unaffected; the helper is referenced only at its def + single call (grep-confirmed). Independent quality-pass (single fresh reviewer; verified line-by-line vs HEAD, explicit invariant verdict): behavior_equivalent=yes, **all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH** (A4/C1/C2 a genuinely-clarifying real-seam extraction not count-shifting; D3 early-return cleaner; the line-too-long:11 a pre-existing Phase-4 residue, out of scope). `calculate_balances` smell cleared; 0 disables added (82); 0 new R0801; useless-suppression 0; E/F 0. Score 9.93 held; visible 99->98; smell items 10->9 (4 8-symbol + 5 instance-attr). 93 balance_calculator (+ hysa + entries) targeted + **full suite 5771 passed.** | 9.93/10 | 98 |
| 2026-06-07 | `98fcb91` | 3 | **calendar_service.py -- remove dead `MonthSummary.large_transactions` field (prep for the instance-attr disposition):** the field was computed in `get_month_detail` (a comprehension flattening `day_entries` filtered by `is_large`) and stored on `MonthSummary`, but read by NO production code -- every consumer (`routes/analytics.py`, `csv_export_service.py`) reads only `day_entries`. A repo-wide grep confirmed zero production/template readers (the surviving `large_transaction*` hits are the unrelated `large_transaction_threshold` setting column). It was a redundant pre-aggregation of data already available + already tested via `day_entries[*].is_large` -- the authoritative classification, still produced + still consumed by the CSV "Large" column. Removed the field, its comprehension, the constructor kwarg, 3 redundant test assertions (each affected test keeps its load-bearing `is_large` assertion, so large-txn classification stays fully covered), and the CSV test-fake default. MonthSummary 10->9 attrs. Independent review (code-reviewer): **ACCEPT (clean removal)** -- zero surviving consumers proven by grep, no coverage lost, no dangling import/constructor. No visible-message delta (R0902 still fires at 9/7, disabled in `cf76c2a`); **full suite 5771 passed.** | 9.93/10 | 98 |
| 2026-06-07 | `cf76c2a` | 3 | **5 `too-many-instance-attributes` classes DISPOSED -- documented disables (instance-attr group CLOSED):** `DayEntry` (10/7) / `MonthSummary` (9/7) [calendar_service], `ItemTrend` (11/7) / `TrendReport` (8/7) [spending_trend_service], `CarryForwardPlan` (10/7) [carry_forward_service]. A 5-analyst workflow (one reader per class + its consumers) + an independent reviewer found ALL FIVE flat irreducible value aggregates -- every field an irreducible column read flat by a row-rendering surface (CSV exports column-per-field, calendar/trends templates interleave, the carry-forward modal iterates `preview.plans`); NO consumer reads any sub-group as a unit. So a scoped+named+commented `disable=too-many-instance-attributes` + a Pylint-note docstring is correct, NOT a restructure: four of the five cannot reach the max=7 with a single nested object, and `TrendReport` (8/7, the only one-over case, probed hardest) reads its window fields scattered (`data_sufficiency` render-gate x3, `window_months` banner+CSV header+route filename, `window_periods` once) -- never as a `window` bundle, so nesting would be a pure count-dodge. The developer LEANED restructure; the verified evidence (field counts AST-checked, threshold arithmetic, consumer-access shape) overrode it -- faithful to [[feedback_tm_args_param_object]] "bundle only for a cohesive named concept". `AmortizationRow`/`PayoffScenarios`/`ProjectedBalance`/`RetirementGapAnalysis` precedent. Independent quality-pass: all 5 disables justified (0 restructure-available), field counts exact, scoping class-level, 0 useless-suppression; **1 REFINE applied** -- the DayEntry note's "1:1 column-per-field" claim was false (CSV emits 9 cols, omits `transaction_id`, folds `is_income`->Income/Expense + `is_paid`->Status), reworded factually true (also fixed a 2nd stale claim the reviewer missed: "the large-transaction filter reads is_large" referenced the comprehension removed in `98fcb91`; `is_large` is now read by the CSV "Large" column). Item-5 tradeoff ACCEPTED: the CarryForwardPlan note grew `carry_forward_service` 1027->1038 lines, a pre-existing out-of-scope C0302 (decision #5 package split). Inline disables 82->87 (5 documented instance-attr disables added); 0 new R0801; useless-suppression 0; E/F 0. Score 9.93 held; visible 98->93; smell items 9->4 (4 8-symbol, 0 instance-attr). Targeted calendar+csv (63) + **full suite 5771 passed.** Examined finding: `DayEntry.transaction_id` has no production reader, but -- unlike the derived `large_transactions` -- it is the row's identity/natural key, so the developer KEPT it as the DTO identity (decided 2026-06-07), not a dead-field removal. | 9.93/10 | 93 |
| 2026-06-07 | `53809b5` | 3 | **account_service.py -- bundle create_account inputs into AccountSpec (no disable) -- file DONE:** `create_account` `too-many-arguments` (6/5) cleared by bundling its six cohesive creation fields (`user_id`/`account_type_id`/`name`/`anchor_balance`/`anchor_period_id`/`notes`) into a new frozen `AccountSpec` -- the textbook public-fn param-object move ([[feedback_tm_args_param_object]]; W4Inputs/PayStubActuals/EntryDetails precedent). AccountSpec stays at **6 attrs** (under the R0902 limit, so NO instance-attr disable -- unlike the larger TransferSpec to come) and adds NO coercion of its own: the function body retains the full `anchor_balance` validation (float-reject / int-coerce / Decimal-check) verbatim, just re-sourced from `spec.`. `**extra_columns` kept as the open-ended passthrough (15 sites pass `is_active`/`sort_order`; folding them in -> 8 attrs -> R0902, so they stay external -- right altitude A5). **201 call sites:** the `/accounts` route (`crud.py`, which splatted `**data` carrying `name`+`account_type_id` dynamically) hand-edited to `data.pop()` the two required `AccountCreateSchema` fields into the spec before construction (zero field-leakage); the other 200 rewritten by a deterministic AST pass that sliced each arg VALUE verbatim via `ast.get_source_segment`, emitted `account_service.AccountSpec(...)` (zero new imports), and re-parsed each file. **Trust-but-verify correction:** the plan called this "a clean low-fork param-object" -- it is 201 call sites, NOT low-fork; surfaced to the developer, who chose the param-object over a documented disable anyway. Two independent rubric reviewers (behavior-equivalence + A-G design, run `wpprlm9ni`): behavior_equivalent=yes, **all ACCEPT (10 + 17 findings), 0 REFINE, 0 REVERT-OVERREACH** -- A1/A2/A3 AccountSpec a genuine cohesive concept not a count-dodge; A5 the `**extra_columns` split the right altitude; E1/E2 the validation byte-identical; the crud.py hand-edit pops-before-construct so no spec field can leak through `**extra_columns`. file 10.00/10, 0 smell messages; 0 disables added (87); 0 new R0801; useless-suppression 0; E/F 0. Score 9.93 held; visible 93->92; smell items 4->3 (3 8-symbol [validation.py + carry_forward tm-lines, update_transfer tm-locals], 0 instance-attr). **full suite 5771 passed.** | 9.93/10 | 92 |
| 2026-06-07 | `36a4fbb` | 3 | **transfer_service.py -- TransferSpec bundle + update_transfer decompose (invariant-critical) -- file DONE:** the sole transfer-creation path; two function smells cleared. **`create_transfer`** (too-many-arguments/positional/locals 12/5): the 12 cohesive creation fields bundled into a new frozen `TransferSpec` (developer-chosen B over a function disable, for consistency with `AccountSpec` + a cleaner factory call) -- one "transfer to create" request mirroring the `Transfer` row; carries a documented `too-many-instance-attributes` disable (12/7), the KEY contrast with `AccountSpec` (6 attrs, no disable). NO coercion in the dataclass (`_validate_positive_amount` stays in the function). Also extracted `_build_shadow(xfer, account_id, name, transaction_type_id)` -- the two near-identical shadow `Transaction` constructions now read the shared period/scenario/status/category/amount/due_date FROM the flushed parent `xfer` (genuine DRY; reviewer confirmed `Transfer.amount` is plain `Numeric(12,2)`, no TypeDecorator/server_default, so the post-flush read is byte-exact). **`update_transfer`** (too-many-locals 18/15 + a too-many-branches,too-many-statements disable): decomposed `_apply_status_change` (the audited F-047/F-048 status+paid_at block, moved verbatim) + `_apply_actual_amount` (the parse); too-many-locals cleared AND **the branches/statements disable REMOVED entirely** (both cleared by extraction; verified no useless-suppression); missing-param-doc on `**kwargs` fixed. All 4 new/extracted helpers type-hinted (the going-forward convention; folded-in REFINE). **65 call sites** wrapped in `TransferSpec(...)`: 51 via an AST pass (values sliced verbatim), 2 hand-edits (the commented `templates.py` recurrence path + the `_make_transfer` `**splat` helper), and **12 direct-import (`ast.Name`) sites in 5 test files** the AST `Attribute`-only pass missed -- the full suite (58 failures) caught them, fixed in a 2nd pass with `TransferSpec` added to those imports (lesson: detect `Name` calls too). Independent 3-reviewer panel (behavior-equivalence / Transfer-Invariants / A-G design, run `wy0sc35hx`): **behavior_equivalent=yes, invariants_hold=yes (Invariants 1-4 verified preserved), ACCEPT x2 + 1 REFINE [helper type hints, folded in], 0 REVERT-OVERREACH.** file 10.00/10; inline disables 87->86 (2 function disables removed, 1 `TransferSpec` object disable added); 0 new R0801; useless-suppression 0; E/F 0. Score 9.93 held; visible 92->90; smell items 3->2 (both the remaining package splits: `validation.py` + `carry_forward_service.py`). **full suite 5771 passed.** | 9.93/10 | 90 |
