...
removed; TRANSFER INVARIANTS 1-4 panel-verified). What remains: the package splits
(`carry_forward_service` + `schemas/validation` module tm-lines, the only 2 visible smell items left);
and `auth.py` (module-split per decision #5 + the `reauth`/`mfa_verify`/`mfa_confirm` return/branch
