...
What remains: the package splits (`carry_forward_service` + `schemas/validation` module tm-lines); and
the criticals (`auth.py` module-split + return/branch disables, `transfer_service` `create_transfer`
[-> `TransferSpec`, developer-chosen 2026-06-07] + `update_transfer` tm-locals) + the `budget_variance`
tm-args disables.
