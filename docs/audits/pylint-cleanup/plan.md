...
`duplicate-code` (R0801) clusters, zero `useless-suppression`, zero E/F; 113 visible messages.
Full suite 5770 passed.** Phase 3 (design smells) has TWENTY-FOUR files plus the form-mutation helper
family complete. The newest, **`app/routes/obligations.py` DONE** (`7a77db9`), cleared
...
Next by live density (re-measured 2026-06-06 after **`routes/obligations.py` DONE** `7a77db9`; 24 smell
items remain [19 of the 8-symbol set + 5 `too-many-instance-attributes`], down from 26): no single file
now dominates -- the densest are all 2-smell files, led by
...
(`DayEntry`/`MonthSummary` instance-attr + `_build_month_summary` tm-args/pos disable),
`services/spending_trend_service.py` (`ItemTrend`/`TrendReport` instance-attr + `_compute_item_trend`
tm-locals disable), `services/loan_payment_service.py` (`prepare_payments_for_engine` +
`live_loan_transfer_amounts` tm-locals). Two files still carry **undisposed Phase-1 smell-disables**
