...
`duplicate-code` (R0801) clusters, zero `useless-suppression`, zero E/F; 110 visible messages.
Full suite 5770 passed.** Phase 3 (design smells) has TWENTY-SIX files plus the form-mutation helper
family complete. The newest, **`app/routes/categories.py` DONE** (`5b32148`), cleared
...
Next by live density (re-measured 2026-06-06 after **`routes/categories.py` DONE** `5b32148`; 21
smell items remain [16 of the 8-symbol set + 5 `too-many-instance-attributes`], down from 22).
...
The remaining low-fork files: `routes/accounts/anchor.py` (`true_up` tm-return), `routes/entries.py`
(`update_entry` tm-return), `app/__init__.py` (`_register_blueprints` tm-locals),
