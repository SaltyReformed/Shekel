...
`duplicate-code` (R0801) clusters, zero `useless-suppression`, zero E/F; 109 visible messages.
Full suite 5770 passed.** Phase 3 (design smells) has TWENTY-SEVEN files plus the form-mutation helper
family complete. The newest, **`app/routes/accounts/anchor.py` DONE** (`ab16669`), cleared `true_up`'s
...
Next by live density (re-measured 2026-06-06 after **`routes/accounts/anchor.py` DONE** `ab16669`; 20
smell items remain [15 of the 8-symbol set + 5 `too-many-instance-attributes`], down from 21).
...
The remaining low-fork files: `routes/entries.py`
