...
cohesive named concept" rule. What remains: `services/account_service.py` (`create_account` tm-args -- a
clean low-fork param-object missed from the batch); the package splits (`carry_forward_service` +
`schemas/validation` module tm-lines); and the 2 criticals (`auth.py`, `transfer_service.update_transfer`)
+ the `budget_variance` tm-args disables.
