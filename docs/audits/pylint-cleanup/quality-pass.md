...
| 35 | `services/tax_calculator.py` | `2cdde52` | REVIEWED -- **ACCEPT** (single fresh reviewer; behavior_equivalent=yes verified BY EXECUTION -- reproduced W4Inputs coercion across int/str/float, the 6 IRS steps byte-identical reading `w4.X`, `_dependent_credits` == the old credit total incl. None/missing `getattr` defaults, the 5 validation errors at the same points, the `w4=W4Inputs()` default immutable via `FrozenInstanceError`; all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH -- A1/A2/A3 W4Inputs a genuine cohesive concept [the employee's W-4/pre-tax adjustments, the [[feedback_tm_args_param_object]] textbook public-fn case; gross/periods/brackets correctly stay out], A4 `_dependent_credits` clarifying, D2 the frozen + `__post_init__` + `object.__setattr__` coercion canonical/sound [no codebase precedent but the standard idiom], coerce-in-`__post_init__` ruled correct over coerce-in-function [the latter wouldn't clear tm-locals]; the lost keyword-only `*` enforcement a non-issue -- now guarded inside the dataclass) |
| 36 | `services/entry_service.py` | `d641e06` | REVIEWED -- **ACCEPT** (single fresh reviewer; behavior_equivalent=yes verified LINE-BY-LINE -- all 5 validation/guard paths byte-identical [the diff touches nothing above the `TransactionEntry` construction], only the entity kwargs + log_event read `details.X`, `EntryDetails` has NO coercion [neither did the old function], the production `EntryDetails(**data)` == the old `**data` [schema loads exactly the 4 keys, `amount` arrives Decimal, `is_credit` default matches], the 39 wraps preserve values incl. the comment-bearing case; all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH -- A1/A2/A3 `EntryDetails` a genuine cohesive concept "mirroring the FORM not the `TransactionEntry` table", the 4-content split the CORRECT altitude on a real seam [form content vs routing/ownership] over the all-6 column-mirror {"would blur the ownership boundary every guard depends on"} AND over a disable, E2 Decimal discipline intact schema->dataclass->model) |
| 37 | `services/balance_calculator.py` | `681fb04` | REVIEWED -- **ACCEPT** (single fresh reviewer; behavior_equivalent=yes verified LINE-BY-LINE vs HEAD -- `_detect_stale_anchor`'s early `return True` PROVEN equal to the old flag+inner-break+outer-`if`-break [the loop had no work after the flag], phase 1 byte-identical, the 2-tuple return + semantics unchanged; **Transfer Invariant #5 HOLDS** [explicit verdict: no `db.session`/`.query`/`Transfer` in either function except docstrings; the helper reads only the in-memory `txn_by_period`+`periods`; the invariant binds the caller, and this adds zero queries]; all ACCEPT, 0 REFINE, 0 REVERT-OVERREACH -- A4/C1/C2 a genuinely-clarifying real-seam extraction [phase 1 = money/balances, phase 2 = an independent informational boolean], D3 the early-return cleaner, the retained `:121` disable on `calculate_balances_with_interest` ruled justified+scoped [those 6 args ARE cohesive+irreducible -- extract-vs-disable correctly distinguished]; the line-too-long:11 a pre-existing Phase-4 residue) |
| 38 | `services/calendar_service.py` + `spending_trend_service.py` + `carry_forward_service.py` (5 instance-attr classes) | `cf76c2a` (+ dead-field `98fcb91`) | REVIEWED -- **ACCEPT** (5-analyst workflow + independent reviewer; all 5 -- DayEntry (10/7) / MonthSummary (9/7) / ItemTrend (11/7) / TrendReport (8/7) / CarryForwardPlan (10/7) -- ruled flat irreducible value aggregates with NO consumer reading any sub-group as a unit, so a documented disable is correct NOT a restructure [four can't reach max=7 with one nest; TrendReport 8/7's window fields read scattered -- gate x3/banner/CSV header/filename -- never as a bundle, so nesting = pure count-dodge]; the developer's restructure LEAN overridden by verified evidence -- field counts AST-checked, threshold arithmetic, consumer-access shape -- faithful to [[feedback_tm_args_param_object]]; `AmortizationRow`/`PayoffScenarios`/`ProjectedBalance`/`RetirementGapAnalysis` precedent; **1 REFINE applied** -- the DayEntry note's "1:1 column-per-field" claim was false [CSV emits 9 cols, omits `transaction_id`, folds `is_income`->Income/Expense + `is_paid`->Status], reworded factually true + a 2nd stale "large-transaction filter reads is_large" claim fixed [that filter was removed in `98fcb91`; `is_large` now read by the CSV "Large" col]; the dead-field `98fcb91` removal of `MonthSummary.large_transactions` a SEPARATE **ACCEPT** [no prod reader, grep-confirmed, redundant with + still tested via `day_entries[*].is_large`]; examined -- `DayEntry.transaction_id` has no prod reader but is the row's identity/natural key [unlike the derived `large_transactions`], so KEPT as the DTO identity, developer-decided 2026-06-07, not removed) |
...
| `account_service.py` (`53809b5`, run `wpprlm9ni`) | A1/A2/A3/A5 | `AccountSpec` (90-134) -- cohesive named concept or a count-dodge bag? Should this have been a documented disable (the developer's original lean; 201 call sites)? | ACCEPT | Two independent reviewers (behavior-equivalence + design) both ACCEPT, 0 non-ACCEPT findings (10 + 17). A1/A2/A3: the 6 fields are "the canonical inputs for creating an account" -- every consumer (`create_account`) reads all 6 and every caller co-assembles all 6; not stamp coupling; mirrors W4Inputs/PayStubActuals/EntryDetails. A5: stores only raw inputs, NO `__post_init__`/coercion (the function body keeps it), and the resolved `anchor_period_id` is computed in the function not stored. Stays at 6 attrs -> no R0902 disable needed (the key contrast with the 12-field TransferSpec). The developer chose the param-object over a disable for design consistency |
| `account_service.py` (`53809b5`) | A5/B1 | `**extra_columns` kept as an open-ended passthrough alongside the spec -- raw-inputs-mixed-with-passthrough smell, or right altitude? | ACCEPT | Right altitude: 15 sites genuinely pass `is_active`/`sort_order`, which are arbitrary `Account` columns, NOT part of the account-creation concept; folding them into AccountSpec would push it to 8 attrs and trip R0902. Cohesive inputs in the spec, open-ended columns through `**extra_columns` -- the correct seam |
| `account_service.py` / `crud.py` (`53809b5`) | E1/E2/E3 (behavior-equivalence) | Does the bundled `create_account` + the 201 rewritten call sites compute EXACTLY what the 6-arg version did? | ACCEPT | Behavior_equivalent=yes, verified line-for-line: the three-step `anchor_balance` validation (float-reject / int-coerce incl. the `not isinstance(bool)` guard / Decimal-check) byte-identical, just re-sourced from `spec.`; anchor-period resolution, both constructors, and the log line map 1:1 to HEAD. 8 random call sites spot-checked -- values verbatim, `is_active`/`sort_order` outside the spec. The `crud.py` hand-edit pops `account_type_id`/`name` (both `AccountCreateSchema`-required) BEFORE spec construction, so no spec field can leak through `**extra_columns`. Full suite 5771 passed |
