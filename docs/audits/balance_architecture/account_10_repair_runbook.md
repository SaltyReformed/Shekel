...
account's books open on 2026-04-05…"*.
...
**You may stop between acts — except between 5 and 6.** Every other boundary leaves the books
internally consistent. Between 5 and 6 the latest assertion is still 2026-07-16, so the dividend you
...
no longer contains its own settle day. Transfer 346 does: it moves to 2026-05-14 while staying in
the period starting 2026-05-21. That is ordinary in this app — a settle legitimately falls outside
its period, and 11 of 156 settled rows on an earlier production clone already did — but it is worth
knowing before you go looking for the row on the grid.
...
That is finding **N-414**. Both doors accept the write when reached; only the reach is missing.
Unarchive it from the cockpit's archived region, do 4c and 4d, then archive it again. The round trip
moves no money (measured: a byte-identical postings fingerprint either side).
...
- **An archived account's two cards still have no click path.** **N-414**.
...
- **The `$500.00` of expense act 4b books is not spending.** Recorded above; the root-cause
  alternative is to let the books open 2026-03-25, which needs the pay calendar to extend backward —
  the case `pay_calendar:C6` and ledger row `P10` park.
