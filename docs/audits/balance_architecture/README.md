...
| **in flight** | nothing OF THIS ARC. **The live branch state is NOT recorded here and must not be** -- read it from `git branch -vv`. This row asserted "no feature branches exist" on 2026-08-08 while block 10's `C1` was being built on `feat/pay-calendar`, and the same revision that says so was itself edited on that branch: a volatile value stored beside no reconciler, which is this arc's own root cause worn as a signpost. Production has moved TWICE since this arc last shipped, both from the recurrence arc: PR #85 (`5c33b462`) then PR #86 (`25f495ef`), so prod runs image `3438ac41aa78` at head **`c7f3a9d1e864`**, not `b5e3d9c1a7f2` | -- |
...
| N-123 | The pay-calendar WRITER refuses every payday from `today+1` to `today+13` and leaves a permanent hole on every one after `today+14`. **It is the pay-calendar arc's row `P3`, matched 2026-08-09** -- one defect in two ledgers, and the two arcs answer it OPPOSITELY (R-DB deletes the registration bootstrap payday; that arc's `C3` keeps it) | `$0.00` on both databases today; `$3,228.55` the moment a hole exists | OPEN | X-ad (whichever of X-ad / pay-calendar C3 runs first decides for both) |
...
| N-173 | For two thirds of settled Checking rows the settle date is a bookkeeping-session artifact: 88 of 135 share a click-minute with another row | 65.2% of the account | OPEN; this is the GENERATOR of N-172's churn. **Re-pointed from the ticked X-f1 at X-f1e3**: that step shipped the MECHANISM, and its backfill was the deleted derivation verbatim, so not one stored date improved | X-f6 |
