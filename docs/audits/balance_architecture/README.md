...
**NEXT: X-g3a** (the SHAPE, baseline-identical) then **X-g3b** (the cutover), then **X-g4** (the
deletion), then **X-c2c4** (the deletion
...
- [ ] **X-g** `feat(balance): a modeled asset is an event stream` -- **the from-scratch design, and
  ...
  * **X-g3 (DECOMPOSED, 2026-07-27, from its own trace)** -- ruling R-W's grid. `grid_balance_view`
    ...
    * [ ] **X-g3a** `refactor(grid): the accrual row is one of two modelled rows` -- the SHAPE,
      ...
      **Four things pinned so the build starts cold rather than re-deriving them.**
      ...
      emits `"interest"`. (4) **N-88's styling follows `investment/dashboard.html:93-100`
      verbatim** -- the chip takes its positive token only when the figure is
      `>= 0`, takes a negative value class when it is `< 0`, and renders an explicit `+` on the
      gain, so the SIGN carries the meaning and colour is never the only signal; the
      desktop `<tfoot>` and the Plan recap adopt the same rule rather than staying colourless, or
      the two form factors disagree (ruling R-P).
    ...
    * [ ] **X-g3b** `feat(balance): the grid renders the modelled balance` -- THE cutover, and the
      only commit here that moves a figure. The kind gate deletes and `grid_balance_view` assembles
      the account's real `ContributionInputs` through `_inputs._assemble_inputs([account], ctx)`
      sliced by `_contribution_inputs` (ruling R-AJ (a), (b)) -- the same two lines
      `_kind_correct._modelled_scalar` (`:174-177`) and `investment_growth_since_anchor` (`:316-323`)
      already run, so the app keeps ONE definition of the contribution feed. Every figure in the table above signed off on both databases; ruling R-K's
      four-term identity re-verified on 0 breaks of 59 for every non-loan account; both loans
      UNMOVED. Two tests INVERT BEHAVIOURALLY here (the field-contract updates are X-g3a's, above)
      ...
      `test_property_stays_cash_flow_no_accrual` (`tests/test_services/test_balance_at.py:2448`,
      `:2484`) each assert the grid is strictly BELOW the modelled map. `_assert_grid_view_reconciles`
      gains the fourth term. And the cross-page lock gains the grid as a surface for the modelled
