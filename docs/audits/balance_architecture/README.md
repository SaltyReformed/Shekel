...
| | | detail |
|---|---|---|
| **just landed** | **X-f2-c2** (`d23b55fd`) -- the reconcile panel settles ROWS, not just purchases: an envelope's close, bills and deposits, through the grid's own verb, stamped with the STATEMENT's day. Ahead of it **X-aq** (`9cabc206`) put the settle's amount rule IN that verb so every door books one figure, and a rename (`3e2c346e`) stopped the door claiming a scope it no longer has. Before them **X-f2-c1** (`24701c1d`): the reconcile step's reader and writer leave `entry_service` for `reconcile_service`, the reader returns a grouped value instead of ORM rows, purchases NEST under their parent, and all THREE of the panel's doors take the cash page's kind gate. No row kind joined the offer set and no figure moved. Before it, **X-f2-b** (`a41b5ebf`) gave a balance assertion a durable home and shipped to `dev` as **PR #90**; both leaves' records are in `archive/…2026-08-04.md` Section 1c under rule 5 | Section 5, `archive/…2026-08-04.md` 1c |
| **how it was measured** | **Six planted mutants for the leaf and three for X-aq, each killed by a distinct test -- and then two neutral reviews found six things the mutants could not.** The load-bearing ones: `_freshest_amount` OVERWRITING a hand-typed `actual_amount` (a human's figure is not a cache); the echo rule stated twice with two scopes; a `synchronize_session` justification a probe FALSIFIED (zero entries in the identity map at the statement); the block's kind DERIVED from its shape, which mis-sectioned production's `$1,958.87` deposit as a bill and would have forced X-f2-c3 to rewrite it inside a money commit; and a summary sentence overstating the panel's own effect by **$488.16** (`$634.08` is what the rows BOOK, `$145.92` is what the projection releases -- measured end to end against a read-only prod clone). Before this leaf: **nine mutations, and TWO of them the step's own controls did not catch.** `purchased_on` mutated to the statement day, and `description` emptied, both survived all 8,671 tests -- every line of the panel captioned with the wrong date, on the one screen whose job is to be walked against a paper statement, because every assertion graded ids and totals and none touched the value objects' other fields. An adversarial review found both; the grouping case asserts the values field for field now. The kind gate's own oracle is the STATUS CODE and nothing else: a loan's panel answered 200 with an EMPTY list, because a loan's `account_anchor_history` carries only its origination row -- a property of the data, not of the route, and one X-f2-c2 removes. X-f2-c's scope is likewise not an estimate: replayed over all 53 Checking assertion days on `shekel-prod-db`, **34 days would have had something to tick, 100 rows / $23,910.04, of which 8 rows / $5,442.89 are transfer shadows** | Section 5.0, X-f2-c |
...
| **blocked on you** | **TWO now, and one of them is this arc's.** **N-227**: an envelope's close is offerable from DAY ONE of its own period, because an envelope has no `due_date` and `attribution_date` falls back to the period's start -- so the offer set never drains (46 of 53 replayed production assertion days carry one) and closing early releases the unspent remainder while making every later purchase on that row inert. Should the bound be `period_end <= observed_on` for an envelope? On today's data that empties the panel, so it moves real figures either way. The other is not this arc's: The SEQUENCING ruling opened at X-an-a: `R6` cannot ship with X-an because it reads a `due_on` that `R5` creates behind X-f4. Everything X-f2-c2 was waiting on is RULED: the bill's amount (**R-FB**), the panel's shape (**R-FC**), income rows (**R-FD**), the settle's amount rule and where it lives (**R-FE**), and what makes a tick correctable (**R-FF**) | `implementation_plan_recurrence_redesign.md` section 0 |
| **next** | **X-ap** (the third settle door, MOVES MONEY), then **X-f2-c3** (transfer shadows, MOVES MONEY, and it OWNS **N-225** / **N-226**), then **X-f3** (own PR) -> X-f4 -> X-f5 -> X-f6, with **X-ar** unsequenced against them. X-aq and X-f2-c2 both SHIPPED on `feat/x-f2-c2-money`; whether they ride one PR or two is the developer's, because each moves money and a rollback of one should not carry the other. Superseded: X-aq puts the settle's amount rule in the VERB so every door books one figure, and it goes FIRST because otherwise the new panel ships displaying a figure the grid contradicts; it MOVES MONEY at the grid and takes its own commit. X-f2-c2's two zero-money commits are already done (`9f255bb4` the package split, `f4512a2f` R-FA's verb), both because folding a structural refactor into a money-moving leaf is what **R-EY** refused. Then **X-ap** (the third settle door, MOVES MONEY), **X-f2-c3** (transfer shadows, MOVES MONEY), **X-f3** (own PR) -> X-f4 -> X-f5 -> X-f6, with **X-ar** unsequenced against them. Block 2
...
* [ ] **X-f2** `feat(accounts): the true-up is a reconciliation` -- R-DH (f)'s second half, the
  ...
  leaves remain.
* [x] **X-aq** `9cabc206` a settle books the freshest derivation of the row's amount, in the
  VERB, so all its doors agree (**R-FE**). Figure-neutral on production today (0 rows, `$0.00`);
  **51 rows / `$4,897.50`** on a clone after one 3% raise applied without regenerating. A review
  then found its first draft OVERWRITING a hand-typed `actual_amount` -- a human's figure is not a
  cache -- and split the echo rule in two; both fixed at `41f678e5`. Opened **N-224**.
...
* [ ] **X-f2-c** the OUTSTANDING SET, widened (**R-EW**) -- the panel offers everything the statement
  ...
  * [x] **X-f2-c2** `d23b55fd` the TRANSACTION twin (**R-FA** / **R-FB** / **R-FC** / **R-FD** /
    **R-FF**): the envelope's close and bills, income included, settled through the grid's own verb
    on the STATEMENT day. Three commits before it (`9f255bb4`, `f4512a2f`, `3e2c346e`) and the two
    reviews' residue after (`41f678e5`). Closed **N-222** / **N-223**, re-pointed **N-221**; opened
    **N-225**..**N-228**.
