...
and recorded, not fixed: **N-45**.  **NEXT: X-c2** -- the cutover, and the only step where money
moves. Phases A and B complete (**A1** `f11382a0`, **A2** `c96c62be`,
...
- [ ] **X-c2** `refactor(balance): the cash seam reads the fold` -- the CUTOVER, and the only step
  ...
  **Deletion-list correction (X-c1, 2026-07-25):** `cash_ledger.period_subtotal` /
  `period_subtotals` / `PeriodSubtotal` belong on that list and the one-liner omitted them.  R-K
