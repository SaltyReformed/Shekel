...
| | | detail |
|---|---|---|
| **just landed** | **X-f1e2** (`bf15ed0c`, migration `b5e3d9c1a7f2`): `account_anchor_history.notes` is DROPPED and the origination assertion routes through the stager, so the table has ONE writer. Ruling **R-ES**. No figure moved -- both harnesses byte-identical. Production still runs merge `e5f27154`, migration head `d7c1f4a9e603` -- **none of the X-f1 cluster is deployed** | its Section 5 entry, and the commit |
| **how much of X-f1 is left** | **one leaf: X-f1e3**, a per-surface rendering decision that wants the design loop. Everything else in the cluster has shipped | Section 5's X-f1 entry, then `archive/…2026-08-04.md` Section 1a |
...
| **next** | **X-f1e3** -> X-f1 ticks -> X-an -> X-f2 -> **X-f3** (moves money, own PR) -> X-f4 -> X-f5 -> X-f6 | Section 5, in execution order |
| **why this shape** | the anchor half was redesigned from scratch by ruling **R-EB**; R-EQ then designed the duplicate rule that had only ever been re-keyed, R-ER put the day rule in the module that owns what an assertion is, X-f1e1 deleted the second DOOR, and X-f1e2 the second WRITER | Section 3.3, and R-EB / R-EQ / R-ER / R-ES in Section 4 |
| **the live lesson** | **unifying two writers made a clock-dependent rule run twice**, and the second application refuses what the first produced. Applying one rule in two places is not belt-and-braces when the rule reads a clock; the fix is a TYPE only the resolver mints | Section 8's "applied twice" bullet |
| **the ledger** | **90 rows**, up from 88: X-f1e2 closed N-198 and opened N-200 (a back-dated assertion can move `is_opening`), N-201 (`anchor_service.py` at 998/1000), N-202 (create-vs-rename deadlock, reproduced) | Section 6 |
| **resuming cold** | branch `feat/xf1-settle-day`; the test template is at **`b5e3d9c1a7f2`** and was rebuilt for this step, so rebuild only if you add a migration. Baseline: **7,858** green under both clock zones. The old advice to override `IDLE_TIMEOUT_MINUTES` is DEAD -- `TestingConfig` pins 720 itself. Two production clones sit on `shekel-dev-db`: **`shekel_xf1e`** at the OLD schema and **`shekel_res_mig`** at this migration's head, which is the pair the baseline diff was taken across | `app/config.py:447`; Section 7.2 for the harnesses |
...
* [ ] **X-f1** `feat(transactions): a settle carries the day the money moved` -- absorbs **S2-b**.
  ...
  * [ ] **X-f1e** `refactor(accounts): a balance is asserted at ONE door` -- developer ruling
    ...
    * [x] **X-f1e2** `refactor(accounts): an assertion is (account, day, balance), written at one
      door` -- ruling **R-ES**, commit `bf15ed0c`, migration `b5e3d9c1a7f2`. Closes **N-198**.
      The `notes` column is deleted and `account_service.create_account` appends its origination
      THROUGH `stage_anchor_true_up`, so the table has ONE writer. No figure moved: both baseline
      harnesses byte-identical, 7,858 green under both clock zones.
      **What the build added beyond the ruling, because two adversarial reviews found it**:
      routing through the stager made the day resolve TWICE, and the floor is clock-dependent, so
      a midnight roll -- or a schedule rebuild committing -- between the two applications refuses
      the day the first produced, after the account row is flushed. That is ruling R-ER's own
      defect one layer up. Both doors now resolve once and pass **`ObservationDay`**, a type only
      the resolver mints; its attribute is `civil_day` because `date.day` exists and a raw date
      silently satisfied `.day` with a day-of-month integer.
      Opened **N-200**, **N-201**, **N-202**.
