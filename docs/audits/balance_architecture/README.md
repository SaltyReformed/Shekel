...
* [ ] **X-bi** the DECOMPOSED parent of the MOVEMENT UNIFICATION (**R-IT**), split five ways by the
  ...
  * [ ] **X-bi-6** the DECOMPOSED parent of the transfer shadow rows' deletion (**R-BAL13**, superseding
    ...
    * [x] **X-bi-6-1b** `6773418b` -- the dashboard's bills / pulse, the calendar and the Spending report read
      legs through `cash_flow_set.set_transfer_legs_in_periods` (the ONE period-windowed load) into
      `PlanItems(rows, legs, items)`, each row producer beside its leg twin keyed by `cell_key`;
      `far_leg_clause` / `paycheck_rows_clause` deleted (no reader left); R-BAL86's "year-end" IS the Spending
      report (`year_end_summary_service` went at `3aecceb0`). Byte-identical over 844 day entries, 173 transfer
      entries and 33 spending reports. Record: the same file.
    ...
    * [ ] **X-bi-6-3** the ledger takes shape C (**R-BAL45**): a clearing `LedgerAccountKindEnum` member and
      ...
      migration re-books the 38 settled transfers, graded ledger-net per (real account, day) byte-identical,
    ...
    * [ ] **X-bi-6-4** the record half re-parents and the writers stop mirroring. TWO FORKS FOR THE DEVELOPER
      ...
      status means when two legs clear on different days. Then `transaction_entries` gains the link,
      `transaction_id` NULLABLE under an exactly-one-parent CHECK, the 38 movements backfilled,
      `covering_movements_by_leg`'s join moved once, the seam's Transfer arm, `transfer_service`'s mirror
      (`_build_shadow`, the five `_restore` clauses, `_validation._get_shadow_transactions`, `_status`'s pair
      day, `_apply_endpoint_move`), carry-forward and `transfer_recurrence` walking `budget.transfers`; a
      migration re-parented onto whichever chain lands first. Closes **N-304**, **BAL-503**. After CC-5.
    * [ ] **X-bi-6-5** the ROWS go: the migration deletes the 354 rows, drops `transactions.transfer_id`,
      ...
      `AmountOwnershipEnum.PARENT_TRANSFER` and `Transaction.transfer`; the exclusions and guards (the
      census markers on its `steps.md` row), the `is_transfer_shadow` fence and the `EVT_*` shadow events die
      with them; the nine leg twins fold into one valuation seam; CLAUDE.md's Transfer Invariants and the
      reviewer mirror rewritten. Graded byte-identical on both baselines. Closes **BAL-475**.
