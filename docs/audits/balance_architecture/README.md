...
* [ ] **X-bi** the DECOMPOSED parent of the MOVEMENT UNIFICATION (**R-IT**), split five ways by the
  ...
  * [ ] **X-bi-6** the DECOMPOSED parent of the transfer shadow rows' deletion (**R-BAL13**, superseding
    ...
    * [ ] **X-bi-6-4** the record half re-parents and the writers stop mirroring. `transfer_service/_endpoints.py:265-269`'s docstring ("the movement adds no failure its parent does not have", false since `credit_card:CC-5-4a-1`: a matched leg's payment member refuses where the parent holds none, BAL-503's clause) is this leaf's to correct. TWO FORKS FOR THE DEVELOPER
      ...
      status means when two legs clear on different days (Fork B's third case is **BAL-528**: whether a `$0.00`-closed transfer is a timed bill). Then `transaction_entries` GAINS the link and `transaction_id` goes NULLABLE under an exactly-one-parent CHECK; the 38 movements are backfilled; `transfer_legs._covering_movements_query`, the ONE join, MOVES onto the link, and so does the ledger's own reach through the shadows since 6-3 (`posting_service._transfer_family_movements`); the status seam GAINS a Transfer arm (today `_seam.py` skips a Transfer whole); `transfer_service`'s mirror GOES (`_build_shadow`, the five `_restore` clauses, `_validation._get_shadow_transactions`, `_status`'s pair day, `_apply_endpoint_move`); carry-forward and `transfer_recurrence` WALK `budget.transfers`; a migration re-parented onto whichever chain lands first. Closes **N-304**, **BAL-503**, **BAL-528**. After `credit_card:CC-5-1` (shipped `4045a9b1`; R-BAL86's "after CC-5-1").
    * **X-bi-6-2 is ABSORBED into X-bi-6-4** (**R-BAL106**, 2026-09-22), its wait on
      `bank_import:X-f6b-2` lifted: 6-4's leaf 6-4c makes statement match and the reconcile panel
      offer LEGS keyed (transfer, account)
      (`statement_match/{_candidates,_offers,_moving,_variance,_destinations}`,
      `reconcile_service/{_transfers,_rows,_assemble,_transactions}`), settling through
      `settle_transfer`; the held bank-import branch resolves the conflict at its own merge.
...
* [ ] **X-au-m** `fix(transfers): an owner-priced pair states its figure ONCE` -- Transfer Invariant
  ...
  **except on a DERIVE-MODE LOAN PAYMENT, the one case where a leg's derivation is NOT its parent's
  ...
  typed, parent reads the contract's `$1,499.10` and so do both legs.
