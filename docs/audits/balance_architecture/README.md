...
| **blocked on you** | nothing. X-f1e2's ruling was TAKEN 2026-08-05 (**R-ES**): the `notes` column is deleted and the origination assertion routes through the stager, so the assertion table has one writer | R-ES in Section 4, X-f1e2's Section 5 entry |
...
* [ ] **X-f1** `feat(transactions): a settle carries the day the money moved` -- absorbs **S2-b**.
  ...
  * [ ] **X-f1e** `refactor(accounts): a balance is asserted at ONE door` -- developer ruling
    ...
    * [ ] **X-f1e2** the assertion's ONE WRITER, and the label nothing reads -- owns **N-198**.
      **RULED 2026-08-05 (R-ES), taken on the trace's measurements**: the `notes` column is
      DELETED, and `account_service.create_account` appends its origination assertion THROUGH
      `anchor_service.stage_anchor_true_up` instead of constructing the row, so
      `AccountAnchorHistory` has exactly one writer. What forced it: **no code in `app/` reads the
      column** (AST census of every `.notes` access -- five hits, all on transactions, transfers
      and tax checkpoints), 76 of 78 production rows are NULL, and the 2 labelled rows are the only
      row their account carries. The app already decides which assertion is the OPENING
      positionally (`cash_ledger/_events.py:370`, `is_opening = index == 0` over
      `(observed_on, created_at, id)`) and maps it to typed posting kinds, so a provenance label
      would be a second definition of the same word that a back-dated assertion can put out of step
      with the first. `anchor_service.py:744` already says the cash table "carries one kind of fact
      and needs no such split"; the column contradicted it. **The loan twin's `source_id` is NOT
      touched** -- that one has readers (`loan_loaders.py:150`, and the drift card's label).
      **The unification is the half that buys the structure**: today the origination write skips
      the owner's write lock, ruling R-EQ's did-this-change compare and the uniform log line, so
      "one door writes an assertion" is a convention. Routing it through the stager also takes the
      advisory lock EARLIER on the create path than the posting sync does today, which improves
      **N-193**'s position rather than worsening it. The stager's return is CHECKED and a `False`
      raises: on a just-flushed account the compare cannot find a governing row, so a decline means
      the E-19 / CRIT-01 invariant broke, and this is where it is established.
