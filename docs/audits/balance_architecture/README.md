  ...
  - **X-c2b (DECOMPOSED, 2026-07-26)** `the cash seam reads the fold` -- THE cutover. All three cash
    ...
    **34 test files**. Mixing them makes a plumbing slip read exactly like a fold slip. So the step
    ...
    - [ ] **X-c2b2** `refactor(balance): the cash seam reads the fold` -- THE cutover, and the only
      ...
      the `stale_anchor_warning` flag + banner (nothing left to warn about once the fold counts the
      settled rows) and the net-worth trend's cash history gate (N-44 -- measured 2026-07-26:
      periods 1-7 carry NO Checking or Money Market balance today, so removing the gate one commit
      early would understate six history points by ~`$6,460`). Every moved figure individually
    ...
    - [ ] **X-c2b3** `refactor(balance): the replaced cash producers delete` -- pure deletion, no
      ...
      `_subtotal_from_transactions`, `_calculator._detect_stale_anchor`, `BalanceResult` (N-50),
      `_kernel.load_account_period_transactions` and `_calculator.calculate_balances_with_interest`
      (N-53), each with its test file. `_layer_interest` survives as the accrual rule;
      `_cash_engine.balances_for` survives to X-c2c returning a bare `OrderedDict`.
