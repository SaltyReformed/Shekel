...
(**N-70**).  **NEXT: X-c2c, and it is larger than its one-liner reads -- see the step.**
...
- **X-c2 (DECOMPOSED, 2026-07-25)** `the cash seam reads the fold` -- the CUTOVER, and the only
  ...
  **Decomposed on the developer's ruling (2026-07-25), because tracing measured a coupling that
    ...
    **34 test files**. Mixing them makes a plumbing slip read exactly like a fold slip. So the step
    ...
    - [x] **X-c2b3** `refactor(balance): the replaced cash producers delete`
      ...
      **Three defects found and fixed, all X-c2b2 residue rather than this step's:** **N-67** (the
      ...
      fixture).  One recorded, not fixed: **N-70**.
  ...
  - [ ] **X-c2c** `refactor(balance): the last cash producer deletes` -- the investment and
    appreciation bases are WINDOWED to anchor-forward off the fold, so their own ruled pre-anchor
    semantics (the reverse growth projection; the flat anchor carry) survive untouched (N-43);
    the reservation's `as_of` window deletes (R-M); and `_cash_engine`, `_calculator` and
    `cash_ledger.load_balance_transactions` delete WHOLE (N-46).
    **Two additions from X-c2b3's ruling and its trace.** (a) `_calculator._detect_stale_anchor`
    and the `calculate_balances` arity change land HERE, with the module -- see X-c2b3 for the
    measurement that moved them.  (b) The step is LARGER than its one-liner reads: `_calculator`'s
    entire test surface is `test_balance_calculator.py` + `test_balance_calculator_entries.py`
    (4,431 lines, 80 tests of hand-computed financial assertions -- 52-period penny accuracy,
    the entries-aware reduction, transfer shadows, `effective_amount` behaviour), plus ~20 call
    sites in suites ABOUT other things (hostile QA, workflows, transfers, carry-forward, interest
    accrual) that use it as a convenient balance oracle.  Every one of those has to be re-pointed
    at the seam or shown covered there BEFORE the module goes, or the step trades a producer for a
    coverage hole -- which is the one thing a deletion step must not do.
...
| N-70 (X-c2b3) | **`balance_predicates.attribution_year` is dead, and has been since plan step F2.**  ZERO production callers in `app/` or `scripts/` (verified against `0ad4f936` as well, so this step did not orphan it); it survives on `tests/test_utils/test_balance_predicates.py` alone -- the dead-code-alive-for-its-own-tests shape C3b4 / D2a / F2 / E1e / N-46 each deleted.  Its docstring names its consumers as "the year-end spending section and the transfers section", and F2 deleted that whole package.  Found while deleting its file-mate `account_period_scope_clause` (N-66); RECORDED rather than deleted because the two were orphaned by different steps -- this one is F2 residue, not this arc's, and X-c2b3 deletes what X-c2b2 replaced | ~25 lines kept alive for its own test | recorded, NOT fixed -- out of the balance arc's scope (plan Section 9 / CLAUDE.md rule 6) | own commit (F2 residue) |
