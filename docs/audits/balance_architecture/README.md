...
scenarios only.  **NEXT: X-c2b3, the pure deletion.** Phases A and B complete (**A1** `f11382a0`, **A2** `c96c62be`,
  ...
  **Decomposed on the developer's ruling (2026-07-25), because tracing measured a coupling that
  ...
  - **X-c2b (DECOMPOSED, 2026-07-26)** `the cash seam reads the fold` -- THE cutover. All three cash
    ...
    **34 test files**. Mixing them makes a plumbing slip read exactly like a fold slip. So the step
    ...
    - [ ] **X-c2b3** `refactor(balance): the replaced cash producers delete` -- pure deletion, no
      behaviour: `_daily_series`, `_cash_engine.balance_as_of_date` + `_project_to_period_before`,
      `cash_ledger.period_subtotal` / `period_subtotals` / `PeriodSubtotal` +
      `_subtotal_from_transactions`, `_calculator._detect_stale_anchor`, `BalanceResult` (N-50) and
      `_kernel.load_account_period_transactions` (N-53), each with its test file.
      **Two list corrections from X-c2b2 as built:** `calculate_balances_with_interest` is ALREADY
      gone (it went with the interest-layering rule at b2 -- `duplicate-code` refuses two copies of
      the walk, and a caller-less copy is the dead-code shape this arc deletes), and
      `_layer_interest` does not "survive" here because it MOVED to `balance_at/_interest.py`, which
      is what makes X-c2c's "`_calculator` deletes WHOLE" true rather than contradictory.
      `_cash_engine.balances_for` and `_calculator.calculate_balances` survive to X-c2c.
...
* **A static guard that greps for a NAME cannot tell code from prose.**
  ...
  behavioural assertion where one exists.
