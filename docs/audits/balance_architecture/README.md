...
`d3e3d82a` on branch `fix/one-partition-implementation` -- **UNPUSHED, no PR, one commit ahead of
`origin/dev`**; `anchor_settle_partition.md` Section 14). The developer ruled
...
**NEXT, in order:** push `fix/one-partition-implementation` and open its PR to `main` (CI has
never seen it -- CI does not run on `dev` pushes, and this branch has had no PR), then **X-d**, PULLED FORWARD past nine steps because it is the
...
**The ledger stands at 53 rows.** **X-f's build opened N-130 (the production defect), N-131 / N-132
