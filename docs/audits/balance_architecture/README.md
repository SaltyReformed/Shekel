...
- [ ] **X-c2** `refactor(balance): the cash seam reads the fold` -- the CUTOVER, and the only step
  ...
  (fold-seeded, accrual forward of the latest assertion) and the grid footer on ruling R-K read the
  fold; the investment and appreciation bases are WINDOWED to anchor-forward so their own ruled
  pre-anchor semantics survive untouched (N-43). Closes **old X1** (settled counted from its
  instant), **old X2** (the Projected-only premise, `_detect_stale_anchor` -- nothing left to detect
  -- and the scalar/daily fork) and **old X3** (pre-anchor: the fold replays EVERY anchor, so a past
  date reads the anchor in force THEN, killing both the scalar's fabrication and the map's omission
  -- B-18) in ONE proven move, because the fold subsumes all three. Deletes what it replaces in the
  same commit (the C3b4 / D2a / E1e lesson): `_cash_engine`, `_calculator`'s cash walk,
  `_daily_series`, the stale-anchor flag and its banner, the reservation's `as_of` window (R-M), and
  the net-worth trend's cash history gate (N-44). Every moved figure on the six affected surfaces
  individually explained and signed off (Section 7.1).
