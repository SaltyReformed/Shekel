...
| | | detail |
|---|---|---|
| **just landed** | **X-bi-6-4a-1 `4d660606`, X-bi-6-4a-2 `9b4e66c9` and X-bi-6-4b `78be0ff4` -- A PAID TRANSFER'S MONEY IS READ AND BOOKED AS A LEG OF ITS TRANSFER** (**R-BAL106**, **R-BAL90**, **R-BAL139**, **R-BAL140**; no migration): the cash fold, the savings metric, the ledger oracle, the posting writer, the loan family and the contributions reach a settled transfer's covering movement through one `transfer_legs` join; a `$0.00` close is no timed bill, and a `$0.00` loan payment is dated by the installment it skips. `transfer_legs` became a package in a pure move (`bf565491`). The previous landing, **X-cv `832c30bd`** -- entrypoint step 3 is all-or-nothing. | Section 5, X-bi-6-4; ../../plans/rulings.md R-BAL88..R-BAL91, R-BAL106..R-BAL108, R-BAL139, R-BAL140 |
  ...
  * [ ] **X-bi-6** the DECOMPOSED parent of the transfer shadow rows' deletion (**R-BAL13**, superseding
    ...
    * [ ] **X-bi-6-4** the DECOMPOSED parent of the record half's re-parent onto `budget.transfers` and the mirror's end, its forks ruled 2026-09-20 (**R-BAL88** the movement's two side-keyed links, **R-BAL89** the first evidence pays a transfer and writes both sides, **R-BAL90** a `$0.00` close is no timed bill) and split 2026-09-22 into four leaves, readers first (**R-BAL106**); each leaf its own session and review; ticks with 6-4d. Every reader moves onto a LEG-shaped call whose interval body, the one reach through a shadow, lives in `transfer_legs` (reads) or `transfer_service` (writes), so 6-4d moves each body once. **BAL-538** (a period-lock refusal naming a transfer's shadow, which has no door) is owned here until a leaf takes it; the two acts that do remove such a payment are the hard delete's removal act (`transfer_service/_delete.py:118`) and a `$0.00` re-record's (`status_seam/_covering.py:640`).
      ...
      * [ ] **X-bi-6-4c** the DECOMPOSED parent of statement match, the reconcile panel, carry-forward and the side's day reading transfers (**R-BAL106**), cut into four leaves 2026-09-24 and ruled the same day (**R-BAL141**, **R-BAL142**, **R-BAL143**, **R-BAL145**); ticks with its last leaf. **BAL-532** (a recurring transfer's occurrence delete leaves a reverted leg's payment and match under hidden shadows) is owned here until a leaf takes it, carrying `credit_card:CC-5-4a-4`'s `transfer_id IS NULL` carve-out in its hidden-row trigger, which retires only when that trigger's arrival arm also refuses a movement under a deleted TRANSFER and its hiding arm reads `budget.transfers`.
        * [ ] **X-bi-6-4c-1** statement match offers LEGS (the absorbed `X-bi-6-2`): a fourth `RowKind.LEG`, a Projected leg on the screen's account keyed (transfer, account) (**R-BAL87**); `_candidates` excludes shadows and offers legs through 6-4c-2's leg loader, its owner, live, status and period gates read off the TRANSFER; `_valuation` prices a leg through 6-4c-2's leg-shaped price; `_moving` dispatches to `settle_transfer` / `update_transfer`; `_accept`, `_accepted_view` and `_acts` read the leg's record, valued by a LEG-shaped twin of `status_seam.covered_cash_leg`; the `(kind, row_id)` token plumbing accepts LEG. Byte-identical; after `credit_card:CC-5-4a-4` is on dev and 6-4c-2.
        * [ ] **X-bi-6-4c-2** the reconcile panel, carry-forward and recurrence walk transfers: an offer tells a leg from a row by `transfer_legs.cell_key` (**R-BAL87**; no second `("row", id)` / `("leg", id)` spelling), a transfer arm loading Projected legs from a NEW owner- and period-scoped `transfer_legs` loader with no scenario, the tick posting `transfer_ids` / `transfer_amount-<id>` (**R-BAL145**) and settling through `settle_transfer` with a leg-shaped clearing door; carry-forward loads the source period's Projected transfers and moves each by `update_transfer`, labelled by its from-side leg (**BAL-546**, declared with its order); `transfer_recurrence._rows_holding_owner_records` asks ONE `transfer_legs` helper whose body is today's query. It introduces the leg price and the clearing door 6-4c-1 reuses.
