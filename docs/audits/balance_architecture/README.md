...
> **PRODUCTION IS CURRENT.** Prod and `main` are both at `c4a19e7b2d80` (deployed 2026-08-01).
> The residue moved no rendered figure (0 of 15,682 seam leaves), as measured.
...
**S1-c IS BUILT AND IN PROGRESS on branch `feat/entry-posting-date`, and ruling R-M was RE-RULED
in the course of it** (`anchor_settle_partition.md` Section 12, ruled 2026-08-01). Building
...
Measured on a production clone: **no figure moves**, no date is invented, and R-DH (f)'s split
lands exactly on the hand-computed `-$427.22` / `-$160.05` halves. **`pylint app/` 10.00/10; the
suite is mid-conversion at 7,424 passed / 157 failed** and the branch must not be PR'd until green
-- the remaining work and the ruling each conversion must satisfy are Section 13.

**NEXT, in order:** finish S1-c's test conversion (Section 13), then step 3's fence -- much of
which S1-c already built (`cash_ledger.is_inside_assertion` is the one predicate; what remains is
the checker) -- then **S2-b** (the TRANSACTION half of step 2, `transactions.settled_on`, plus the
...
**The ledger stands at 52 rows.** **X-f's build opened N-130 (the production defect), N-131 / N-132
