...
- [ ] **X-af** `test(periods): the fixtures build their window on the USER's clock` -- closes the
  MERGE-GATE half of **N-137**; the app half is deliberately NOT here and is **N-138**'s.
  **BUILT AND GREEN 2026-08-02 on `fix/one-clock-for-the-users-day`, sequenced ahead of PR #76
  because it is a merge-gate blocker rather than a feature** (the box is ticked when it MERGES, not
  when it is written). `main` itself failed 8 tests under CI's `TZ=Pacific/Kiritimati` outside the
...
**The ledger stands at 54 rows.** **X-f's build opened N-130 (the production defect), N-131 / N-132
