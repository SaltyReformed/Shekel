...
- [ ] **X-g** `feat(balance): a modeled asset is an event stream` -- **the from-scratch design, and
  ...
  * [ ] **X-g3** -- ruling R-W's grid. `grid_balance_view` renders the modeled balance and
    generalises the conditional Interest row to a "Growth"/accrual row on R-O's own non-zero rule,
    on BOTH form factors (R-P).

    **"Render plumbing only" is WRONG and this entry says so before the step starts** (measured
    2026-07-27 on `shekel_f3_final`, after X-g2b shipped). The grid's rendered balance MOVES: it is
    the kind-blind cash-flow figure today and becomes the modelled one, which is finding N-76's
    whole point -- Roth `$27,432.35 -> $33,477.26`, Trad IRA `$11,714.31 -> $14,295.64`, Empower
    **`$31,070.06 -> $48,846.91`** at the last projected period. The seam already answers those
    (they are `balance_map`'s figures), but the GRID has never shown them, so this is a
    money-moving render step and needs the every-figure sign-off, not a plumbing one.

    **Ruling R-W's identity is INCOMPLETE as stated, and the missing term is the LARGER one.** R-W
    writes `balance[p] - balance[p-1] == net[p] + reconciliation[p] + accrual[p]`. A modelled asset
    has TWO modelled tiers, and `AssetPeriodFigures` reports both:
    `... == net + reconciliation + accrual + contribution`. On the real Empower 401(k) over the
    horizon the accrual is `$8,152.58` and the CONTRIBUTION is **`$9,624.27`** -- the employer's
    flat 5% of `$3,631.74` a period, which is a live feed on real data (X-g1's own measurement).
    Render only the accrual and the grid's identity breaks by the larger half.

    **So the step has at least three forks to trace and rule before it builds**, and none of them is
    in R-W: (1) ONE "Growth" row summing both modelled tiers, or TWO rows (contribution is money the
    user PUT IN, accrual is what the market did -- the same distinction ruling R-AC keeps apart in
    the /investment chip, and the grid already has a precedent for a two-part explanation in R-K's
    net + reconciliation); (2) the row's LABEL per kind -- "Interest" is right for an HYSA and wrong
    for a 401(k), and R-O's one-conditional-row-convention argues against two labels for one row;
    (3) `grid_balance_view` passes `ContributionInputs.absent()` today, so it must assemble the real
    per-account inputs for an INVESTMENT -- which is `_assemble_inputs` inside a producer that has
    never needed it, and is where the step's cost actually is.
