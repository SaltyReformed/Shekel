# Budget App -- Project Roadmap v4.3
...
| 7        | Data Export                   | CSV export, PDF reports, full data backup                                                                                   |
| 8        | Multi-user (far future)       | Kid accounts, registration flow, account sharing model; not actively planned                                                |
...
### 4.11 Salary Profile Button Placement -- PARTIAL

- **Resolution (partial):** View Breakdown and View Projection buttons moved to a prominent
  position near the top of the salary profile edit page (`/salary/{id}/edit`). However, the
  main salary listing page (`/salary`) still displays both inline action icons and redundant
  full buttons below the salary profile. The `/salary` page fix is deferred to Section 5A,
  task 5A.3.
...
non-seasonal variable expenses, and then inflation adjustments. This section also includes
computation engines for budget variance analysis, annual expense aggregation, and spending
trend detection. These engines build the analytical services; the display and visualization
layers for tasks 6.5, 6.6, and 6.7 are built in Section 8 (Visualization and Reporting
Overhaul).
...
## 8. Visualization and Reporting Overhaul (Priority 6)

**Goal:** Overhaul the app's visualization and reporting capabilities. This phase begins with
fixing known chart bugs, then redesigns the charts page, builds new summary and reporting
views, and wires up display layers for the computation engines built in Section 6 (Smart
Features).

This section consolidates visualization and reporting work from multiple sources:

- Chart bug fixes from `fixes_improvements.md` production feedback (tasks 8.0a, 8.0b).
...
- Display and rendering layers for the budget variance analysis (6.5), annual expense
  calendar (6.6), and spending trend detection (6.7) engines built in Section 6. Those
  sections build the computation services; this section builds the UI that presents their
  output.

**Prerequisite:** Sections 5, 5A, and 6 are complete. Section 7 (Notifications) is
recommended but not strictly required; if notifications are in place, the summary dashboard
(8.1) can display active alerts. If not, the alerts section is a placeholder.
...
| implementation_plan_section5.md       | Roadmap v4.1, Section 5   | Defunct; predates transfer rework, Section 4, account param arch, audit, and Section 5A. Must be rewritten before starting Section 5. |
...
| Salary button duplication             | fixes_improvements.md     | Partially completed as task 4.11 (`/salary/{id}/edit` fixed); `/salary` page fix moved to Section 5A.3 |
| Grid estimated vs. actual             | fixes_improvements.md     | Moved to Section 5A.1 as prerequisite to Section 5                     |
| Grid transaction sort display         | fixes_improvements.md     | Moved to Section 5A.2 (category item sub-headers)                      |
| Category editing and add flow         | fixes_improvements.md     | Moved to Section 5A.4 (category management overhaul)                   |
| CRUD deactivate/delete inconsistency  | fixes_improvements.md     | Moved to Section 5A.5 (unified delete/archive pattern)                 |
