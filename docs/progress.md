...
The `budget.scenarios` model exists with `is_baseline` flag, but no clone/compare logic is implemented. UI spec is in `docs/scenario_ui_requirements.md`.
...
Note: Inflation field exists on paycheck deductions but global projection adjustment is not implemented.
...
### Phase 8B: Audit & Structured Logging — NOT STARTED

- [ ] system.audit_log table with row-level tracking
- [ ] PL/pgSQL trigger function on 23+ financial tables
- [ ] Structured JSON application logging with request_id correlation
- [ ] Request duration tracking (log at INFO if >500ms)
- [ ] Event categorization (Auth, Business, Error, Performance)
- [ ] Retention cleanup script (365-day default)

Note: python-json-logger dependency is installed and basic logging config exists, but full structured logging is not wired up.
