...
**Scope:** Tasks 4.1 through 4.10 from `docs/project_roadmap_v4.md`.
...
- **Server handler:** `transactions.get_quick_edit` in `app/routes/transactions.py:66`
- **Response:** HTMX fragment -- renders `grid/_transaction_quick_edit.html` with a single amount input and an expand button.
...
- **Server handler:** `transactions.update_transaction` in `app/routes/transactions.py:87`
...
- **Dependencies:** `_update_schema` (Marshmallow validation), `_get_owned_transaction()`.

### GI-3: Quick Edit Expand Button -> Full Edit Popover

- **Trigger element:** `<button class="txn-expand-btn" data-txn-id="<id>">` (in `grid/_transaction_quick_edit.html`, line 20-26). Click handled by delegated JS in `grid_edit.js:279`.
- **HTTP request:** JS `fetch('/transactions/<id>/full-edit')` (not HTMX -- uses vanilla fetch in `grid_edit.js:74`).
- **Server handler:** `transactions.get_full_edit` in `app/routes/transactions.py:76`
- **Response:** HTML fragment -- renders `grid/_transaction_full_edit.html`. Injected into `#txn-popover` via JS.
...
- **Trigger element:** `<form hx-patch="/transactions/<id>" hx-target="#txn-cell-<id>" hx-swap="innerHTML">` (in `grid/_transaction_full_edit.html`, line 16-19)
- **HTTP request:** PATCH `/transactions/<txn_id>` with form data (estimated_amount, actual_amount, status_id, notes).
- **Server handler:** `transactions.update_transaction` in `app/routes/transactions.py:87` (same as GI-2).
...
- **Trigger element:** `<button hx-post="/transactions/<id>/mark-done" hx-target="#txn-cell-<id>" hx-swap="innerHTML">` (in `grid/_transaction_full_edit.html`, line 75-81 for expenses, line 112-120 for income as "Received")
- **HTTP request:** POST `/transactions/<txn_id>/mark-done`
- **Server handler:** `transactions.mark_done` in `app/routes/transactions.py:123`
- **Response:** HTMX fragment -- renders `grid/_transaction_cell.html` with updated status badge.
- **DOM update:** Targets `#txn-cell-<id>` with `innerHTML` swap.
- **Side effects:** `HX-Trigger: gridRefresh`. This triggers `window.location.reload()` via the `gridRefresh` listener in `app.js:41-43`. Full page reload, not partial.
- **Dependencies:** Status lookup by name (`"done"` or `"received"`). `_get_owned_transaction()`.

### GI-6: Mark Credit Button (in Full Edit Popover)

- **Trigger element:** `<button hx-post="/transactions/<id>/mark-credit" hx-target="#txn-cell-<id>" hx-swap="innerHTML">` (in `grid/_transaction_full_edit.html`, line 84-91)
- **HTTP request:** POST `/transactions/<txn_id>/mark-credit`
- **Server handler:** `transactions.mark_credit` in `app/routes/transactions.py:157`
- **Response:** HTMX fragment -- renders `grid/_transaction_cell.html`.
...
- **Trigger element:** `<button hx-delete="/transactions/<id>/unmark-credit" hx-target="#txn-cell-<id>" hx-swap="innerHTML">` (in `grid/_transaction_full_edit.html`, line 93-99)
- **HTTP request:** DELETE `/transactions/<txn_id>/unmark-credit`
- **Server handler:** `transactions.unmark_credit` in `app/routes/transactions.py:174`
- **Response:** HTMX fragment -- renders `grid/_transaction_cell.html`.
...
- **Server handler:** `transactions.cancel_transaction` in `app/routes/transactions.py:191`
- **Response:** HTMX fragment -- renders `grid/_transaction_cell.html`.
...
- **Trigger element:** `<div class="txn-cell txn-empty-cell" hx-get="/transactions/new/quick?category_id=X&period_id=Y&txn_type_name=Z" hx-trigger="click" hx-target="closest td" hx-swap="innerHTML">` (in `grid/_transaction_empty_cell.html`, line 10-18)
- **HTTP request:** GET `/transactions/new/quick?category_id=<id>&period_id=<id>&txn_type_name=<income|expense>`
- **Server handler:** `transactions.get_quick_create` in `app/routes/transactions.py:209`
...
- **Trigger element:** `<tfoot id="grid-summary" hx-get="/grid/balance-row?periods=N&offset=O&account_id=A" hx-trigger="balanceChanged from:body" hx-swap="outerHTML">` (in `grid/_balance_row.html`, line 6-11)
- **HTTP request:** GET `/grid/balance-row?periods=N&offset=O[&account_id=A]`
- **Server handler:** `grid.balance_row` in `app/routes/grid.py:210`
- **Response:** HTMX fragment -- renders `grid/_balance_row.html` (the entire `<tfoot>`).
...
- **Dependencies:** `balance_calculator.calculate_balances()`, all periods, all transactions, all transfers. `account` and `low_balance_threshold` context variables.

### GI-11: Anchor Balance Click -> Edit Form

- **Trigger element:** `<div id="anchor-display" hx-get="/accounts/<id>/anchor-form" hx-trigger="click" hx-swap="outerHTML">` (in `grid/_anchor_edit.html`, line 31-42)
...
- **Trigger element:** `<form hx-patch="/accounts/<id>/true-up" hx-target="this" hx-swap="outerHTML">` (in `grid/_anchor_edit.html`, line 6-9)
- **HTTP request:** PATCH `/accounts/<account_id>/true-up` with form data `anchor_balance`.
- **Server handler:** `accounts.true_up` in `app/routes/accounts.py:453`
...
- **Trigger element:** `<button hx-get="/accounts/<id>/anchor-display" hx-target="closest form" hx-swap="outerHTML">` (in `grid/_anchor_edit.html`, line 22-26)
...
- **Server handler:** `transactions.carry_forward` in `app/routes/transactions.py:415`
...
- **Trigger element:** `<form class="txn-quick-edit" data-mode="create" hx-post="/transactions/inline" hx-target="closest td" hx-swap="innerHTML">` (in `grid/_transaction_quick_create.html`, line 11-16)
- **HTTP request:** POST `/transactions/inline` with form data (category_id, pay_period_id, scenario_id, transaction_type_id, estimated_amount).
- **Server handler:** `transactions.create_inline` in `app/routes/transactions.py:318`
...
- **Trigger element:** `<form hx-post="/transactions" hx-swap="none" data-modal-auto-close>` (in `grid/grid.html`, line 301-303)
...
- **Server handler:** `transactions.create_transaction` in `app/routes/transactions.py:366`
- **Response:** HTMX fragment -- renders `grid/_transaction_cell.html`.
- **DOM update:** `hx-swap="none"` -- no DOM swap. Instead, the `htmx:afterRequest` handler in `app.js:96-106` detects `data-modal-auto-close`, hides the modal, and calls `location.reload()`.
...
- **Trigger element:** Arrow buttons and quick-select buttons in the grid header (in `grid/grid.html`, line 26-49). These are standard `<a href="...">` links, NOT HTMX requests.
- **HTTP request:** GET `/?periods=N&offset=O` (full page load).
- **Server handler:** `grid.index` in `app/routes/grid.py:32`
- **Response:** Full page.
...
- **Trigger element:** Keyboard events on the document (in `app.js:346-553`).
...
- **DOM update:** Adds/removes `cell-focused` class on `<td>` elements. The `getDataRows()` function filters out rows with classes `section-banner-income`, `section-banner-expense`, `spacer-row`, `group-header-row`.
- **Side effects:** After HTMX swaps, `htmx:afterSwap` handler (in `app.js:523-530`) restores focus position.
- **Dependencies:** `.grid-table` must exist. Data rows must NOT have the excluded classes.

### GI-19: Transfer Cell Click -> Quick Edit

- **Trigger element:** `<div class="txn-cell" hx-get="/transfers/<id>/quick-edit" hx-trigger="click" hx-target="#xfer-cell-<id>" hx-swap="innerHTML">` (in `transfers/_transfer_cell.html`, line 9-13)
- **HTTP request:** GET `/transfers/<xfer_id>/quick-edit`
- **Server handler:** `transfers.get_quick_edit` in `app/routes/transfers.py`
- **Response:** HTMX fragment -- transfer quick edit form.
- **DOM update:** Targets `#xfer-cell-<id>` with `innerHTML` swap.
- **Side effects:** None.
- **Dependencies:** Transfer ownership check.

### GI-20: Quick Edit Escape -> Revert to Display

- **Trigger element:** Escape key in a quick edit input (handled by `grid_edit.js:214-265`).
...
- **Server handler:** `transactions.get_cell` (`app/routes/transactions.py:56`) or `transactions.get_empty_cell` (`app/routes/transactions.py:290`).
...
- **Trigger element:** F2 key pressed while focused in a quick edit/create input (handled by `grid_edit.js:182-211`).
- **HTTP request:** JS `fetch('/transactions/<id>/full-edit')` or `fetch('/transactions/new/full?...')` (same as GI-3).
- **Server handler:** `transactions.get_full_edit` or `transactions.get_full_create`.
...
- **`<thead class="table-dark">`:** One row. First column is `<th class="row-label-col sticky-col">Item</th>`. Subsequent columns are pay period headers. Each period header contains two `<div>` elements:
  - `<div class="fw-bold">{{ period.start_date.strftime('%m/%d') }}</div>` -- start date in MM/DD format.
  - `<div class="small text-light-emphasis">{{ period.end_date.strftime('%m/%d/%y') }}</div>` -- end date in MM/DD/YY format.
  - For past periods (excluding current), a carry forward button is also rendered inside the `<th>`.

- **`<tbody>`:** Contains three major sections separated by spacer rows:
  1. **INCOME section:** Starts with a banner row `<tr class="section-banner-income">` with colspan. Then rows are grouped by category. Each category group has an optional group header row `<tr class="group-header-row">` showing `category.group_name` with colspan. Under each group, individual transaction rows are rendered as `<tr>` with:
  ...
  2. **EXPENSES section:** Same structure as income, with banner `<tr class="section-banner-expense">`.
  3. **TRANSFERS section** (conditional): Only rendered if any transfers exist. Banner `<tr class="section-banner-transfer">`. Transfer rows are grouped by template name, not by category.

- **`<tfoot id="grid-summary">`:** Included via `{% include "grid/_balance_row.html" %}`. Contains 4-5 summary rows: Total Income, Total Expenses, Net Transfers (conditional), Net (Income - Expenses), and Projected End Balance.

**Key row-label behavior:** The row label cell shows `category.item_name` (e.g., "Electricity") with `category.display_name` (e.g., "Home: Utilities > Electricity") as the `title` attribute (hover tooltip). The transaction template name (e.g., "AEP Electric") is NOT shown in the row label. It appears only when the user clicks a cell and opens the full edit popover (line 9-10 of `_transaction_full_edit.html`), or as the `title` attribute on the `.txn-cell` div (line 18 of `_transaction_cell.html`).

**Row grouping:** Categories are iterated in `group_name, item_name` order (set by the query in `grid.py:130-134`). The template checks if `category.group_name != ns.current_group` to decide whether to insert a group header row. This means multiple categories under the same group (e.g., "Home") share one group header.

**CSS sizing:** The `.row-label-col` has `min-width: 160px; max-width: 200px`. The `.row-label` has `font-size: 0.8rem`. Group header rows have `font-size: 11px` with `text-transform: uppercase`.
...
During step 5 (mark cleared expenses as done), the user scans the grid to find specific expenses. If two transactions share the same category item name, the user cannot distinguish them at a glance in the row label column. They must hover over each cell to see the template name in the tooltip, then click the correct one. This adds friction to the most time-sensitive part of the workflow. With approximately 10-15 active line items, this affects maybe 2-4 items that share categories, but those items require extra cognitive effort every payday.
...
1. **Total Income** (`<tr class="summary-row fw-bold">`) -- sums projected income per period, excluding done/received/credit/cancelled.
2. **Total Expenses** (`<tr class="summary-row fw-bold">`) -- sums projected expenses per period, same exclusions.
3. **Net Transfers** (`<tr class="summary-row fw-bold">`) -- conditional, only rendered when `xfer_by_period` has data. Shows net transfer effect per period.
4. **Net (Income - Expenses)** (`<tr class="summary-row fw-bold">`) -- shows the net cash flow per period. Negative values get a warning icon.
5. **Projected End Balance** (`<tr class="balance-row-summary fw-bold">`) -- the running balance. Uses `balance-negative` and `balance-low` CSS classes for visual warnings.

The tfoot is sticky (`position: sticky; bottom: 0; z-index: 2;` from `app.css:331-335`). The summary rows use `background-color: var(--shekel-summary-bg)` to prevent transparency during scroll. The balance row has special styling: `font-size: 15px; color: var(--shekel-accent);` (blue accent color).

The grid wrapper has `max-height: calc(100vh - 160px)` and `overflow: auto`. This means the tfoot sticks to the bottom of the scrollable area.

With 4-5 rows of approximately 30-35px each (including padding and borders), the footer consumes approximately 120-175px of vertical space. On a 1080p display (~960px usable after browser chrome and navbar), that leaves roughly 620-680px for the grid header and transaction rows. At approximately 28-30px per transaction row, this fits approximately 20-22 transaction rows before vertical scrolling within the grid wrapper.

**A2. What is wrong and why it matters.**

The footer occupies 120-175px of sticky space at the bottom of the grid viewport. While the Projected End Balance row is essential (it's the primary number the user checks during the payday workflow), the individual Total Income, Total Expenses, and Net Transfer rows are informational. They rarely change the user's actions. The Net row is useful but could be combined with the balance row. The 4-5 rows consume space that could show 3-5 more transaction rows without scrolling.

**A3. Impact on payday workflow.**

During step 7 (check projections), the user primarily looks at the Projected End Balance row. The other summary rows are secondary. If the grid has enough transactions to require scrolling, the user must scroll to see all transactions while the footer consumes fixed space at the bottom. Reducing the footer frees vertical space for the actual transaction data the user needs to reconcile.
...
1. **`grid/_transaction_cell.html:34-38`:** Checks `t.status.name in ('done', 'received')` to show a checkmark badge. The `title` and `aria-label` attributes use `t.status.name|capitalize` (renders as "Done" or "Received").
2. **`grid/_transaction_full_edit.html:44-48`:** Status dropdown uses `s.name|capitalize` as option text. Shows "Projected", "Done", "Received", "Credit", "Cancelled", "Settled".
3. **`grid/_transaction_full_edit.html:74-81`:** "Done" button text: `<i class="bi bi-check-circle"></i> Done` (hardcoded string). Only shown for expenses where `txn.status.name != 'done'`.
4. **`grid/_transaction_full_edit.html:112-120`:** "Received" button text: `<i class="bi bi-check-circle"></i> Received` (hardcoded string). Only shown for income where `txn.status.name == 'projected'`.
5. **`grid/_transaction_full_create.html:52-56`:** Status dropdown uses `s.name|capitalize`.
6. **`transfers/_transfer_cell.html:34-35`:** Same pattern as transaction cell -- checks `xfer.status.name in ('done', 'received')`.
7. **`transfers/_transfer_full_edit.html:71`:** "Done" button text hardcoded.

**Internal usage of status name strings (these must NOT change):**

1. **`app/services/balance_calculator.py:27`:** `SETTLED_STATUSES = frozenset({"done", "received"})` -- used to exclude settled transactions from balance calculation.
2. **`app/services/balance_calculator.py:339, 388`:** String checks `if status_name in ("credit", "cancelled", "done", "received")`.
3. **`app/routes/transactions.py:137-139`:** `Status.filter_by(name="received")` and `Status.filter_by(name="done")` for mark_done logic.
4. **`app/routes/transfers.py:508`:** `Status.filter_by(name="done")` for transfer mark-done.
5. **`app/services/recurrence_engine.py:40`:** `IMMUTABLE_STATUSES = frozenset({"done", "received", "credit", "cancelled"})`.
6. **`app/services/transfer_recurrence.py:28`:** Same immutable statuses frozenset.
7. **`app/services/credit_workflow.py`:** Likely uses status name checks.
8. **`app/services/chart_data_service.py:390`:** `Status.filter_by(name="done")`.
9. **`app/models/transaction.py:99`:** `self.status.name in ("done", "received")` for effective_amount property.
10. **`grid/_balance_row.html:18, 34, 51, 78`:** String checks in Jinja2 for filtering totals.
11. **`grid/grid.html:140, 204, 264`:** `txn.status.name != 'cancelled'` filter.
12. **`app/routes/savings.py:399`:** `txn.status.name in ("done", "received")`.

**Test assertions that reference "done"/"Done":**
- `tests/test_services/test_transfer_recurrence.py:300, 721, 743`: Queries and asserts on `name="done"`.
- `tests/test_services/test_recurrence_engine.py:240`: Queries `name="done"`.
- `tests/test_scripts/test_integrity_check.py:456-457`: Queries `name="done"`.
- `tests/test_services/test_credit_workflow.py:263+`: Multiple tests query `name="done"`.
...
Account creation is in `app/routes/accounts.py`. The creation route (`create_account`, line 99-150) handles the post-creation flow:

1. **Mortgage accounts:** Redirect to `mortgage.dashboard` (line 146). This takes the user to a separate page for entering mortgage parameters (interest rate, term, origination date, etc.).
2. **Auto loan accounts:** Redirect to `auto_loan.dashboard` (line 148).
3. **HYSA accounts:** Auto-create a default `HysaParams` record (line 136-137), then redirect to the accounts list (line 150). The user must then find the new account card on the savings dashboard and click the small graph icon button to configure APY.
4. **Investment/retirement accounts:** Redirect to the accounts list. The user must find the account card and click the small graph-up icon to configure return rates.
5. **Checking/savings:** Redirect to the accounts list. No parameters needed.
...
On the savings dashboard (`app/templates/savings/dashboard.html`), each account card has small icon buttons in the top-right corner (line 88-113). For HYSA, there's a `<a class="btn btn-sm btn-outline-info" title="HYSA Detail">` with a graph icon. For mortgage, a house icon. For investment/retirement, a graph-up icon. These buttons link to the respective detail/parameter pages. There is no visual indicator that an account needs parameter configuration.
...
All datasets use `borderWidth: 2`, `fill: false`, `tension: 0.3`, `pointRadius: 0` (`chart_balance.js:39-43`). There is no dash pattern differentiation. When the chart has many lines (checking, savings, HYSA, mortgage, retirement), some colors may be difficult to distinguish, especially for lines that are close together or small in amplitude.
...
The chart route `charts.balance_over_time` (`app/routes/charts.py:42-65`) accepts optional `start` and `end` query parameters. The `chart_data_service.get_balance_over_time()` function uses a helper `_get_periods()` to determine the period range. Looking at the template, the chart loads via `hx-get="{{ url_for('charts.balance_over_time', **request.args) }}"` on page load.

There is an account checkbox filter in `_balance_over_time.html` (line 4-25) that re-fetches the chart with selected accounts. There is NO time frame control. The chart always shows whatever the default period range is (likely all periods from the anchor forward, based on the `_get_periods` helper).
...
**Before state:** Each unique `category` object produces one `<tr>` row. The row label cell shows `category.item_name`. Multiple transactions sharing a category are distinguished only by hover tooltip on the cell.

**After state:** Each unique transaction template gets its own row. The row label shows the transaction template name (e.g., "AEP Electric") instead of the category item name. Category group headers remain for visual grouping.
...
- `app/templates/grid/grid.html`: Replace the category-based iteration with row_key iteration. The row label becomes `<th class="sticky-col row-label">{{ row_key.transaction_name }}</th>` with `title="{{ row_key.display_name }}"`. Group header rows remain.
...
**Changes required:**
- `app/templates/grid/_transaction_cell.html`: Add a small transaction name label above or below the amount. Something like `<div class="txn-name-label small text-muted text-truncate" style="font-size: 0.65rem;">{{ t.name }}</div>` before the amount display.
...
2. **Total vertical height with 15 active line items:** Measure the grid height including headers, section banners, group headers, and transaction rows. Option A adds more rows (one per template) but removes the ambiguity. Option B keeps the same row count but adds vertical space within each cell.
...
- **GI-5 through GI-8 (Status changes):** Target `#txn-cell-<id>` -- SAFE. All use `gridRefresh` which reloads the page.
...
- **GI-18 (Keyboard navigation):** `getDataRows()` filters out rows with classes `section-banner-income`, `section-banner-expense`, `spacer-row`, `group-header-row`. If Option A removes some group header rows, fewer rows are excluded. If Option A adds `section-banner-transfer` class to new rows, they'd be excluded. **REQUIRES VERIFICATION** that the new row structure does not add any rows with excluded classes that should be navigable, and does not remove the excluded classes from rows that should be skipped. The transfer section already uses `section-banner-transfer` which is NOT in the exclusion list -- this is a bug in the current code (transfer banner rows are navigable). However, this is a pre-existing issue, not caused by this change.
- **GI-19 (Transfer cell click):** NOT AFFECTED by transaction row changes.
...
**B6. Edge cases:**
- **Zero transactions:** Grid shows section banners and group headers but no transaction rows. Option A: no template-specific rows appear. Option B: same as current (no cells with name labels).
- **30+ transactions:** Option A adds more rows, requiring more scrolling. The sticky tfoot ensures the balance row is always visible. The sticky thead ensures headers are visible. The vertical space concern is the primary argument for Option B.
...
- **Narrow browser window:** Mobile CSS already constrains `.sticky-col` to `min-width: 90-130px`. Long names will truncate more aggressively but remain functional.
- **Year boundary periods:** Not affected -- this is a row structure change, not a date change.
...
Condense the footer from 4-5 rows to 2 rows:

1. **Row 1: Net Cash Flow** -- Combines the current "Net (Income - Expenses)" row with the Net Transfers information. Shows the combined net figure. This replaces separate Total Income, Total Expenses, and Net Transfers rows.
2. **Row 2: Projected End Balance** -- Unchanged from current. This is the primary data the user needs.

The Total Income, Total Expenses, and Net Transfers data can be made accessible via a tooltip or a collapsible detail row that expands on click. This preserves the data for users who want it while freeing vertical space.

**Template changes:**
- `app/templates/grid/_balance_row.html` -- Remove the separate income/expense/transfer total rows. Replace with a single "Net Cash Flow" row that computes `income - expenses + transfers_in - transfers_out`. Keep the existing Projected End Balance row. Add a tooltip on the Net Cash Flow cell showing the breakdown (e.g., "Income: $X, Expenses: $Y, Transfers: $Z").

**CSS changes:**
- No new CSS needed. Existing `.summary-row` and `.balance-row-summary` styles apply.

**B5. HTMX regression analysis:**
- **GI-10 (Balance row refresh):** This is the primary affected interaction. The balance row endpoint returns the entire `<tfoot>`. Changing the number of `<tr>` elements in the tfoot is safe because the swap is `outerHTML` on `#grid-summary`. The new `<tfoot id="grid-summary">` replaces the old one entirely. The `hx-trigger="balanceChanged from:body"` attribute on the new tfoot element ensures future triggers work.
- All other interactions: NOT AFFECTED. The tfoot change is self-contained.

**B6. Edge cases:**
- **Zero transactions:** All summary rows show zeros or empty. With 2 rows instead of 4-5, the footer is smaller but still correct.
- **No transfers:** The "Net Transfers" component is already conditional. In the condensed version, if no transfers exist, the Net Cash Flow is simply income - expenses.
- **Negative net:** Warning icon behavior is preserved in the condensed row.

**B7. Files to modify:**
- `app/templates/grid/_balance_row.html` -- Modified. Condense to 2 rows.
- `app/routes/grid.py` -- No changes (context variables unchanged).
- `tests/test_routes/test_grid.py` -- Modified. Update assertions for new footer structure.
...
Note: `today` is already passed as a context variable from `grid.index` (line 171 of `grid.py`).
...
2. **`app/templates/grid/_transaction_full_edit.html:44-48`:** (Status dropdown options)
...
3. **`app/templates/grid/_transaction_full_edit.html:80`:** (Button text)
   - BEFORE: `<i class="bi bi-check-circle"></i> Done`
   - AFTER: `<i class="bi bi-check-circle"></i> Paid`

4. **`app/templates/grid/_transaction_full_create.html:52-56`:** (Status dropdown)
   - BEFORE: `{{ s.name|capitalize }}`
   - AFTER: `{{ s.display_label or s.name|capitalize }}`

5. **`app/templates/transfers/_transfer_cell.html:35`:**
   - BEFORE: `title="{{ xfer.status.name|capitalize }}"`
   - AFTER: `title="{{ xfer.status.display_label or xfer.status.name|capitalize }}"`

6. **`app/templates/transfers/_transfer_full_edit.html:71`:** (Button text)
   - BEFORE: `<i class="bi bi-check-circle"></i> Done`
   - AFTER: `<i class="bi bi-check-circle"></i> Paid`
...
- `app/routes/transfers.py` -- Uses `Status.filter_by(name="done")`. KEEP UNCHANGED.
- `app/services/recurrence_engine.py` -- Uses `IMMUTABLE_STATUSES` frozenset. KEEP UNCHANGED.
- `app/services/transfer_recurrence.py` -- Same. KEEP UNCHANGED.
...
- `app/templates/grid/_balance_row.html` -- Uses `txn.status.name not in (...)` for logic. KEEP UNCHANGED. These are filter conditions, not display.
- `app/templates/grid/grid.html:140, 204, 264` -- Uses `txn.status.name != 'cancelled'`. KEEP UNCHANGED.
- `scripts/seed_ref_tables.py` -- Seeds by name. KEEP UNCHANGED. The migration handles display_label.
- `app/__init__.py:367` -- Seeds by name in test mode. KEEP UNCHANGED.
...
- `app/templates/transfers/_transfer_full_edit.html` -- Modified. Use display_label for button text.
...
In `create_account()` (line 133-150), extend the redirect logic:
...
- **Assertion:** Response status 200. Response contains `<tfoot id="grid-summary">`. Response contains balance amounts. Response contains the `hx-trigger="balanceChanged from:body"` attribute.
...
- **Assertion:** After all steps, GET `/grid/balance-row` returns correct balances. The balance calculation matches hand-computed expected values.
...
- **Category:** Route
- **Setup:** `seed_user`, `auth_client`, seeded transaction templates with distinct names.
- **Action:** GET `/`.
- **Assertion:** Response HTML contains the transaction template name (e.g., "AEP Electric") visible in a `<th class="sticky-col row-label">` element.
...
- **Setup:** Same as above.
- **Action:** GET `/`.
- **Assertion:** Response HTML contains a `.txn-name-label` element with the transaction name inside a `<td class="text-end cell">`.
...
- **Action:** GET `/`.
- **Assertion:** Grid renders without errors. Section banners appear. No transaction rows. Summary rows show zeros.
...
- **Action:** GET `/`. Inspect that data rows do NOT have excluded CSS classes (`section-banner-income`, `section-banner-expense`, `spacer-row`, `group-header-row`).
- **Assertion:** Transaction rows are navigable by keyboard (no excluded classes). Banner/spacer/group rows have the correct excluded classes.
- **Why:** Keyboard navigation depends on CSS class filtering.

#### Task 4.2 Tests

**C-4.2-1:** `test_balance_row_has_two_summary_rows`
- **Category:** Route
- **Setup:** `seed_user`, `auth_client`, seeded periods/transactions.
- **Action:** GET `/grid/balance-row?periods=6&offset=0`.
- **Assertion:** Response `<tfoot>` contains exactly 2 `<tr>` elements (Net Cash Flow and Projected End Balance). Response does NOT contain separate "Total Income" or "Total Expenses" rows.
- **Why:** Verifies footer condensation.

**C-4.2-2:** `test_condensed_footer_net_calculation`
- **Category:** Route
- **Setup:** `seed_user`, `auth_client`, seeded transactions with known amounts.
- **Action:** GET `/grid/balance-row`.
- **Assertion:** The "Net Cash Flow" row shows the correct value (income - expenses +/- transfers).
- **Why:** Verifies the combined net calculation is correct.

**C-4.2-3:** `test_balance_row_htmx_refresh_after_condensation`
- **Category:** Route (HTMX regression)
- **Setup:** `seed_user`, `auth_client`, seeded data.
- **Action:** GET `/grid/balance-row`. Verify response has `id="grid-summary"` and `hx-trigger="balanceChanged from:body"`.
- **Assertion:** The `<tfoot>` has the correct HTMX attributes for future refreshes.
- **Why:** Ensures the self-referencing HTMX trigger still works after the tfoot structure changes.
...
**D1. What could break:**
- The keyboard navigation in `app.js:356-364` (`getDataRows()`) filters rows by CSS class. If Option A changes the class names on rows, keyboard navigation could skip or include wrong rows.
...
- C-4.1-5 (keyboard navigation test) checks CSS classes.
...
- **4.1 + 4.2:** Both change vertical space. Option A adds rows; 4.2 removes footer rows. Net effect depends on the specific counts. Test with realistic data (15 items) after both are applied.
...
**D1. What could break:**
- The balance row refresh (GI-10) swaps the entire `<tfoot>`. If the condensed tfoot structure has different CSS or missing elements that the JS relies on, the refresh could produce visual artifacts.
- Tests that assert on specific summary row text ("Total Income", "Total Expenses") will fail.

**D2. Mitigation:** C-4.2-1 through C-4.2-3 cover the structure, calculation, and HTMX refresh.
...
| 4.2 (Footer Condensation) | Moderate | 2-4 hours | Multi-element template change with calculation logic consolidation. HTMX self-referencing tfoot. |
| 4.3 (Date Format) | Simple | 30-60 min | Single template conditional. Two test cases. |
| 4.4 (Terminology) | Moderate | 3-5 hours | Migration, model change, 6+ template files, test updates across the codebase. Wide ripple effect but each change is small. |
...
1. **Commit #1: Payday workflow regression test suite** -- MUST be first. Establishes safety net.
...
10. **Task 4.2 (Footer Condensation)** -- Moderate. Done after 4.1 because the vertical space budget depends on the row structure chosen in 4.1. The footer condensation should be evaluated with the new layout to ensure the combined effect works well.

**Justification:**
- Safety-first: Non-grid tasks (4.5, 4.6, 4.9, 4.10, 4.7/4.8) come first while the grid is untouched.
- The regression test suite (commit #1) protects all subsequent changes.
- Text-only grid changes (4.4, 4.3) come before structural changes (4.1, 4.2).
- 4.1 is the riskiest and comes after all simpler changes are proven.
- 4.2 depends on 4.1's layout choice and comes last.

### E3. Total Estimated Time Range

**Minimum:** 19.75 hours (sum of low estimates + regression test suite ~2h)
**Maximum:** 34.5 hours (sum of high estimates + regression test suite ~4h)
**Expected:** ~25 hours
...
- `app/templates/transfers/_transfer_full_edit.html` -- Modified. Use display_label.
...
**F4.** Open the grid. Click a projected expense cell. Verify the full edit popover shows "Paid" button (not "Done"). Verify the status dropdown shows "Paid" option. Mark the expense, verify the badge tooltip says "Paid". Open the grid fresh -- verify income still shows "Received" button.
...
**F4.** Open the app. Perform the complete payday workflow (true-up balance, mark paycheck received, carry forward unpaid, mark expenses done, mark one credit, check projections). Verify every step completes without errors and the final balance is correct.
...
**F1.** `feat(grid): condense footer to net cash flow and projected balance rows`

**F2.**
- `app/templates/grid/_balance_row.html` -- Modified. Condense to 2 rows.
- `tests/test_routes/test_grid.py` -- Modified. Update footer assertions.
- `tests/test_routes/test_grid_regression.py` -- Modified. Update C-0-6 if it asserts on specific summary rows.

**F3.** C-4.2-1, C-4.2-2, C-4.2-3 + full payday workflow regression suite.

**F4.** Open the app. Perform the complete payday workflow. Verify the condensed footer shows correct Net Cash Flow and Projected End Balance. Verify the balance row refreshes correctly after edits (HX-Trigger: balanceChanged).

**F5.** Screenshot before/after showing the footer reduction. Note the freed vertical space.
