...
**What ``X-ao-2`` deleted matters as much as what it moved.**  The half-moved
...
#: Every arc's rulings, since ``balance:X-ao-2`` finished the lift.
...
#: to the exact three arcs ``X-ao-2`` had to protect.  Two independent
...
#: in both directions, correct while it lasted and DELETED by ``X-ao-2`` with
...
#:
#: Two spellings, both taken from the corpus rather than imagined:
#:
...
#:
#: **A CITATION is deliberately not matched.**  ``ruling **R-R35**`` and
#: ``**Ruling R-AP, taken AGAINST the recommendation**`` name a ruling without
#: stating one, and both are live text in arc documents today; the
#: discriminator is the DATE, because a declaration carries one and a citation
#: does not.
RULING_DECLARATION_RX = re.compile(
    r"\bR-[A-Z]+\d*,\s*(?:ruled|archived)\b"
    r"|\*\*Ruling\s+R-[A-Z]+\d*\s*\(\d{4}-\d{2}-\d{2}"
)
...
#: ``## Rulings``, ``## The rulings``, ``## Locked developer rulings`` and
#: ``## 4. The rulings`` match while ``## 0. Why this arc splits, and the one
#: ruling still owed`` and ``### Phase X -- the anchor half (ruling R-EB)`` do
#: not.  All five are live headings; the anchoring is what tells them apart.
#:
...
#: ``X-ao-2``: its section was headed ``## 4. Decisions that govern the
...
RULINGS_HEADING_RX = re.compile(
    r"^#{2,4}\s+(?:\d+[a-z]?\.\s+)?(?:the\s+|locked\s+developer\s+)*rulings?\b",
    re.IGNORECASE,
)
...
#: The LIFTED rows that exceed :data:`RULINGS_ROW_CAP`, BY KEY.
#:
...
#: own and the developer ruled its destination on 2026-08-27: a SHIPPED
#: ruling's deliberation goes to the as-built record of the step that shipped
#: it, which is what this registry's own first sentence already says and what
#: rule 5's archive is for; an OPEN one's goes to its live step specification.
#: That is ``X-ao-2b``, and it deletes this set.
#:
#: **It is a SET and not a COUNT, and that is a repair rather than a
#: refactor.**  ``LIFTED_ROWS_OVER_CAP = 23`` could not tell 23 rows from a
#: different 23: trimming one row while another swelled past the cap left the
#: total at 23 and the arm green, which is a gate that has stopped measuring
#: what it names.  Keyed, both directions fail -- a row over the cap that is
...
LIFTED_ROWS_OVER_CAP = frozenset({
    ...
    "balance:R-GV",
    "bank_import:R-FU", "bank_import:R-FV", "bank_import:R-FW",
    "bank_import:R-FX", "bank_import:R-FY", "bank_import:R-GA",
...
def row_width_violations() -> list[str]:
    ...
    **Two directions, because the count this replaced only had one.**  A new
    row over the cap is a failure, and so is a row in
    :data:`LIFTED_ROWS_OVER_CAP` that is no longer over it: the second is what
    tells ``X-ao-2b`` it has finished a row, and under a bare count a trim and
    a swell cancelled out and the arm stayed green.

    Returns:
        One message per over-cap row not in the debt, widest first, then one
        per debt entry that is no longer over the cap.
    """
    ...
    problems.extend(
        ...
        f"{RULINGS_ROW_CAP}-character cap. The debt is a SET so that finishing "
        f"a row is visible: drop this key"
        for key in sorted(LIFTED_ROWS_OVER_CAP - set(over))
    )
    return problems
...
def _registry_side_violations(declared: list[str]) -> list[str]:
    ...
    **The fourth arm used to admit an arc that had not moved yet.**  That
    ...
    problems.extend(
        ...
        f"nothing says where its rulings live. Since balance:X-ao-2 there is "
...
def _document_side_violations() -> list[str]:
    ...
    Four arms, over EVERY arc document rather than over a map of the ones that
    have not moved.  A map is migration state and it was right while the
    ...
    * **A rulings SECTION is a POINTER**: it names ``rulings.md`` and carries
      no table row and no list item.  This is the arm for an ID-LESS block,
      which is the shape ``credit_card``'s eight locked rulings had and which
      no id- or table-based arm can see.

    **A fifth arm was DELETED rather than kept**, and the mutation run is why:
...
def migration_violations() -> list[str]:
    ...
    **Three of these arms exist because a draft did not close the hole its own
    ...
    contributed.  ``X-ao-2`` then measured a third: the table arm could not see
