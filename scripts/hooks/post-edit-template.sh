...
# shellcheck disable=SC2312 # hook_repo_root forks git but its exit status is printf's, and its own || absorbs a git failure, so it cannot fail; the cd itself is the guard, a bad/empty path trips the || block and exits 2.
cd "$(hook_repo_root)" || {
