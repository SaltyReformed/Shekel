...
caller names (or a day before every plan date when it names none), and the plan
is appended exactly as the seam appends it.  With no recorded payment, the
projection boundary is the day after that assertion, so no plan charge is
clamped, every projection keeps its own date, and the hypothetical extra
accrues at every charge -- which is precisely what ``_split_plan`` did from its
seed.  ``fold_forward``'s origination gate (``0.00`` before ``owed_from``) is the
empty prefix before the assertion.
...
from app.services.balance_at._loan_stream import merged_stream
from app.services.balance_at._plan_fold import (
    required_extra,
    timeline_payoff_date,
)
from app.services.balance_at._plan_records import LoanForwardPlan
...
def timeline_from_plan(
    ...
    Args:
        ...
        owed_from: The day that assertion is dated; the loan owes ``0.00``
            before it.
        plan: The hand-built :class:`~app.services.balance_at._plan_records.LoanForwardPlan`.
    ...
    return replay_loan_stream(
        merged_stream(facts, plan), extra_per_period=extra_monthly,
    )
...
def split_plan(
    seed: Decimal, plan: LoanForwardPlan, extra_monthly: Decimal = _ZERO,
) -> list[PaymentOutcome]:
...
def plan_interest_in_year(
    seed: Decimal, plan: LoanForwardPlan, year: int,
) -> Decimal:
