...
class TestGridItemSubHeaders:
    """Tests for category item sub-header rows in the grid.

    Verifies that the grid renders lightweight sub-header rows showing
    the Category Item Name between the group header and transaction rows,
    making the visual hierarchy Group -> Item -> Transaction explicit.
    """

    @staticmethod
    def _get_current_period(seed_user):
        """Return the current period for the seed user."""
        return pay_period_service.get_current_period(seed_user["user"].id)

    def test_item_subheader_renders_for_multiple_items(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """Two items in the same expense group each get their own
        sub-header row with the item-subheader-row CSS class.
        """
        with app.app_context():
            projected = db.session.query(Status).filter_by(
                name="Projected",
            ...
            # Create two items in the same group.
            cat_ins = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrAuto",
                item_name="SubHdrCarIns",
            )
            cat_gas = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrAuto",
            ...
            db.session.add_all([cat_ins, cat_gas])
            db.session.flush()

            for cat, name in [(cat_ins, "Ins Bill"), (cat_gas, "Gas Fill")]:
            ...
            assert "item-subheader-row" in html
            assert "SubHdrCarIns" in html
            assert "SubHdrGas" in html

    def test_item_subheader_renders_for_single_item(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """A group with only one item still gets a sub-header for
        visual context.
        """
        with app.app_context():
            projected = db.session.query(Status).filter_by(
            ...
            cat = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrHousing",
                item_name="SubHdrMortgage",
            )
            db.session.add(cat)
            db.session.flush()
            ...
            assert "SubHdrMortgage" in html
            assert "item-subheader-row" in html

    def test_item_subheaders_appear_in_both_sections(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """Sub-headers render in both the income and expense sections."""
        with app.app_context():
            projected = db.session.query(Status).filter_by(
                name="Projected",
            ).one()
            ...
            cat_inc = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrEarnings",
                item_name="SubHdrFreelance",
            )
            cat_exp = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrUtils",
                item_name="SubHdrElectric",
            )
            ...
            # Both item names appear as sub-headers.
            assert "SubHdrFreelance" in html
            assert "SubHdrElectric" in html

            # Verify sub-headers appear in their respective sections
            # by checking position relative to section banners.
            income_pos = html.find("INCOME")
            expense_pos = html.find("EXPENSES")
            freelance_pos = html.find("SubHdrFreelance")
            electric_pos = html.find("SubHdrElectric")
            assert income_pos < freelance_pos < expense_pos, (
                "Income sub-header must appear in the income section"
            )
            assert expense_pos < electric_pos, (
                "Expense sub-header must appear in the expense section"
            )

    def test_group_change_resets_item_tracking(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """When two groups each have an item with the same name, both
        groups render their own sub-header (current_item resets on
        group change).
        """
        with app.app_context():
            projected = db.session.query(Status).filter_by(
            ...
            # Two groups, each with an item named "SubHdrSameItem".
            cat_a = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrGroupA",
                item_name="SubHdrSameItem",
            )
            cat_b = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrGroupB",
                item_name="SubHdrSameItem",
            )
            db.session.add_all([cat_a, cat_b])
            db.session.flush()

            for cat, name in [(cat_a, "Txn A"), (cat_b, "Txn B")]:
                db.session.add(Transaction(
                    pay_period_id=current.id,
                    scenario_id=seed_user["scenario"].id,
                    account_id=seed_user["account"].id,
            ...
            assert "SubHdrGroupA" in html
            assert "SubHdrGroupB" in html
            # Count sub-header rows that contain "SubHdrSameItem".
            # The item name also appears in row title attributes, so
            # count the specific sub-header markup pattern instead.
            import re
            subheader_matches = re.findall(
                r'class="item-subheader-row"[^>]*>.*?SubHdrSameItem',
                html, re.DOTALL,
            )
            assert len(subheader_matches) == 2, (
                f"Expected 2 sub-header rows for same item name in "
                f"different groups, got {len(subheader_matches)}"
            )

    def test_subheader_content_matches_category_item(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """The sub-header text is the exact category item_name."""
        with app.app_context():
            projected = db.session.query(Status).filter_by(
                name="Projected",
            ).one()
            ...
            cat = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrTransport",
                item_name="SubHdrCarPayment",
            )
            db.session.add(cat)
            db.session.flush()
            ...
            # The sub-header contains the item name, not the group
            # name or transaction name.
            assert "SubHdrCarPayment" in html
            # Verify it is inside a sub-header row.
            idx = html.find("SubHdrCarPayment")
            # Look backward from the item name for the class attribute.
            preceding = html[max(0, idx - 200):idx]
            assert "item-subheader-row" in preceding

    def test_subheader_does_not_duplicate_for_same_item(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """Two transactions under the same item produce only one
        sub-header, not two.
        """
        with app.app_context():
            projected = db.session.query(Status).filter_by(
            ...
            cat = Category(
                user_id=seed_user["user"].id,
                group_name="SubHdrDupTest",
                item_name="SubHdrDupItem",
            )
            db.session.add(cat)
            db.session.flush()

            # Two transactions under the same category item.
            for name in ["DupTxnAlpha", "DupTxnBeta"]:
                db.session.add(Transaction(
                    pay_period_id=current.id,
                    scenario_id=seed_user["scenario"].id,
                    account_id=seed_user["account"].id,
            ...
            # The item sub-header row appears exactly once (the item
            # name also appears in transaction row title attributes, so
            # count the specific sub-header markup pattern).
            import re
            subheader_matches = re.findall(
                r'class="item-subheader-row"[^>]*>.*?SubHdrDupItem',
                html, re.DOTALL,
            )
            assert len(subheader_matches) == 1, (
                f"Expected 1 sub-header row for item with 2 "
                f"transactions, got {len(subheader_matches)}"
            )

    def test_subheader_order_matches_sort_order(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """Sub-headers appear in alphabetical order by item_name
        within a group, matching the sort key.
        """
        with app.app_context():
            projected = db.session.query(Status).filter_by(
                name="Projected",
            ...
            # Create items whose sort order is known: Alpha < Beta < Gamma.
            cats = []
            for item_name in ["SubHdrGamma", "SubHdrAlpha", "SubHdrBeta"]:
                cat = Category(
                    user_id=seed_user["user"].id,
                    group_name="SubHdrSortGrp",
                    item_name=item_name,
                )
                db.session.add(cat)
                cats.append(cat)
            ...
            # Verify alphabetical ordering within the group.
            pos_alpha = html.find("SubHdrAlpha")
            pos_beta = html.find("SubHdrBeta")
            pos_gamma = html.find("SubHdrGamma")
            assert pos_alpha < pos_beta < pos_gamma, (
                "Item sub-headers must appear in alphabetical order"
            )

    def test_subheader_has_correct_css_class(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """The sub-header tr element has the item-subheader-row class."""
        with app.app_context():
            projected = db.session.query(Status).filter_by(
                name="Projected",
            ).one()
            ...
            # Use a seed category to keep setup minimal.
            db.session.add(Transaction(
                pay_period_id=current.id,
                scenario_id=seed_user["scenario"].id,
                account_id=seed_user["account"].id,
            ...
            assert 'class="item-subheader-row"' in html

    def test_subheader_does_not_break_subtotals(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """Subtotals remain correct despite the presence of sub-header
        rows in the grid.
        """
        with app.app_context():
            projected = db.session.query(Status).filter_by(
            ...
            # Sub-headers are present.
            assert "item-subheader-row" in html
            # Subtotals are correct.
            assert "$3,000" in html
            assert "$1,200" in html

    def test_grid_with_no_transactions_no_subheaders(
        self, app, auth_client, seed_user, seed_periods,
    ):
        """An empty grid (no transactions) renders no sub-header rows."""
        with app.app_context():
            resp = auth_client.get("/?periods=3")
            assert resp.status_code == 200
            html = resp.data.decode()

            assert "item-subheader-row" not in html
