    ...
    **Every case here stages a payday that is NOT one stored period-length
    ...
    def test_a_row_filed_in_ANOTHER_owners_paycheck_is_NOT_offered(
        ...
        **The row is AD-HOC** (``template=False``) since plan step
        balance:X-cf: a row of a definition is generated by the engine now,
        which refuses a period outside the owner's calendar before any INSERT
        (``RecurrenceWindowError``), one tier above the key this case grades.
        The key is over ``(pay_period_id, user_id)`` on every row, and an
        ad-hoc envelope is exactly what ``_create._create_envelope`` writes.
        """
        with app.app_context():
