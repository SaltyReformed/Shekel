...
def _rule(cadence, starts_on=_A_FIRST_OCCURRENCE, *, owner_id=_USER_ID,
    ...
    **It states the SIX cadence columns and nothing else** (plan step R7c-c).
    ...
    off, and four of the six are ``NOT NULL``.  A cadence is one of the
    baseline oracle's own constants -- so a rule built for this file carries
    exactly what the write door would write for the frozen shape built from
    that same constant.
    ...
    def test_rule_occurrences_equals_read_rules_placements(self, app):
        ...
        with app.app_context():
            ...
            for cadence in BASELINE_CADENCES:
                rule = _rule(
                    cadence, starts_on=date(2026, 6, 15),
                )
...
class TestItSwallowsNothingElse:
    ...
    def test_an_unmodelled_unit_still_raises(self, app):
        ...
        **The unreadable column moved at plan step R7c-c**: this planted a
        ``pattern_id`` the closed pattern set's enum did not name, and that
    ...
    def test_a_non_positive_interval_still_raises(self, app):
        ...
        with app.app_context():
            calendar = build_calendar()
            rule = _rule(EVERY_N_PERIODS, interval_n=0)
