...
def test_the_cache_stores_on_membership_not_truthiness():
    ...
    never the value's truthiness, because those derivations have legitimately
    falsy answers: an empty plan (a not-yet-configured or retired loan) and a
    ``None`` payoff (a loan that never clears).  A truthiness
    ...
    directly on the shared primitive, so it holds for every cache it fills.
    """
