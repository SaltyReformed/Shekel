...
(:func:`app.services.balance_at._plan.fold_forward`) against balances computed BY
...
def test_a_planned_payment_pays_its_principal_down():
    ...
    plan = [_payment(date(2026, 2, 1), "1000.00")]
...
def test_an_overdue_records_clamp_defers_its_paydown_to_the_effective_date():
    ...
    plan = [_payment(date(2026, 1, 15), "1000.00", effective=date(2026, 1, 21))]
...
def test_extra_cash_pays_the_loan_down_faster():
    ...
    seed = Decimal("100000.00")
    base = fold_forward(
        seed, _ORIGINATION, [_payment(date(2026, 2, 1), "1000.00")],
        [date(2026, 2, 1)],
    )
    with_extra = fold_forward(
        seed, _ORIGINATION, [_payment(date(2026, 2, 1), "1500.00")],
        [date(2026, 2, 1)],
    )
...
def test_a_date_before_origination_owes_nothing():
    ...
    plan = [_payment(date(2026, 5, 1), "1500.00")]
