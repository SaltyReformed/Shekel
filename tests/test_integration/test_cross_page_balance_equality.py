...
rendering surface MUST return the identical Decimal:
  ...
  2. /savings                 -- ``savings_dashboard_service`` +
                                ``GET /savings``.
  3. /accounts checking detail -- the balance ``GET /accounts/<id>/checking``
...
def _accounts_checking_value(ctx):
    """Read the /accounts checking-detail balance the ROUTE actually rendered.
    ...
    ``GET /accounts/<id>/checking`` request and reads the balance the template
    rendered, off the ``data-current-balance`` hook the balance ``<span>``
    carries (the raw Decimal, not the formatted ``money()`` string, so the read
    is immune to display-format changes).  This is the L6 route-render lock: a
    reader that reconstructs the seam call would still match even if the route
    drifted onto a different producer or a template bug mis-rendered the figure,
    because the reader hardcodes the correct call.  Reading the rendered
    attribute makes the /accounts surface a genuinely distinct check (it was
    formerly a byte-identical twin of :func:`_grid_value`'s ``cash_balance_map``
    call) AND catches a route that stopped rendering the seam's value.
    ...
    """
    resp = ctx["client"].get(f"/accounts/{ctx['account_id']}/checking")
    assert resp.status_code == 200, (
        f"/accounts/{ctx['account_id']}/checking returned {resp.status_code}; "
        "the checking-detail route must render the symptom-tuple data without "
        "raising (the empty-state / falsy guards must not be zeroing the balance)"
    )
    match = re.search(r'data-current-balance="([^"]*)"', resp.text)
    assert match is not None, (
        "the checking-detail response carried no data-current-balance attribute "
        "-- the balance <span> hook this route-render lock reads is gone; either "
        "the template dropped it or the route rendered the empty-state branch"
    )
...
class TestCrossPageBalanceEquality:
    ...
    def test_all_surfaces_equal(
        ...
        reader drives the real ``GET /accounts/<id>/checking`` and asserts the
        ...
        service reader's by the same seam the checking route now proves is
        wired.
        """
        with app.app_context():
            ...
            # The accounts-checking reader drives the real route, so give it
            ...
            # silently zeroing the balance).  ``GET /accounts/<id>/checking``
            # is exercised AND value-checked by ``_accounts_checking_value``
            # above, so it is not re-fetched here.
...
class TestSeamInjectionLock:
    ...
    def test_invariant_fails_if_seam_reintroduced(
        ...
        with app.app_context():
            ...
            # The accounts-checking reader drives the real route (it returns
            # the correct 160.00 here); the divergence under test is injected
            # into /savings below, not checking.
            ctx["client"] = auth_client
    ...
    def test_route_render_lock_catches_a_checking_route_misrender(
        self,
        app,
        seed_cross_page_account,
        auth_client,
        ...
        """L6: the accounts-checking reader catches a ROUTE that mis-renders.
        ...
        call.  This control proves the accounts-checking reader is genuinely
        different: it reads what ``GET /accounts/<id>/checking`` actually
        ...
        with app.app_context():
            ...
            def _misrender(_balances, _current_period, _anchor):
                """Force ONLY the checking route to render a divergent balance."""
            ...
            assert "'accounts_checking'" in str(excinfo.value), (
                ...
                "message did not name 'accounts_checking' -- the checking "
