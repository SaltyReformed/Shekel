...
def update_settings():
    ...
    if errors:
        ...
        # Validation-error re-render of the settings dashboard with the
        # non-active sections zeroed out -- the standard Flask "re-render
        # the shared template with empty collections + 422" shape it
        # shares with the parallel ``settings`` section handler.  The
        # context differs only by ``active_section`` and the domain
        # form_data; a shared builder would just relocate the kwargs list
