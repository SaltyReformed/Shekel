...
def is_standing_loan_payment(
    ...
    **The ONE identity behind every per-bound rule the recurrence form applies
    ...
    reads that memo through :func:`loan_standing_payment`, so the composed
    door, the form's two locks, the three refusals and the forward plan read
    ONE producer and the lookup runs once per pass however many of them ask.
    ...
    **Not the same question as "is this a loan payment".**
    ...
    if account is None:
        return False
    standing = loan_standing_payment(account, ctx)
