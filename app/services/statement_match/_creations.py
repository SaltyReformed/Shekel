...
class PurchaseDestination:
    ...
    :func:`~._destinations.destinations_for`'s, and it mirrors every guard
    ...
    **It carried its paycheck as THREE fields until pay-calendar plan step
    ...
    Attributes:
        ...
        name: The row's OWN name, unlabelled -- what a merchant rule's stated
            envelope name is compared against at that answer's FIRST firing
            (plan step ``bank_import:X-f6a-4``, finding **N-327**), so a line
            files into a placed envelope of that name already here instead of
            minting another beside it; the answer then names the definition
            (leaf 7b-3 of ``balance:X-bi-7b``) and never compares a name
            again.
        ...
        period: The paycheck it is budgeted under, as the calendar DERIVED it
            ...
            asserted: :func:`~._destinations.destinations_for` scopes its scan by
        ...
        template_id: The DEFINITION this row is one of -- generated by a
            recurring one, or placed by a rule-less one (a grid one-off's, a
            bank-born envelope's; plan step ``balance:X-bi-7b``).  Required:
            a destination is built from a row that ``tracks_purchases``, and
            since the family's cutover (``balance:X-bi-7d-2``) that is the
            DEFINITION's flag, so no destination lacks one.
            **It is the row's identity ACROSS pay periods, and that is what a
            merchant rule is keyed on** (plan step ``bank_import:X-f6a-3d``):
            a rule cannot name ``transaction_id`` -- an envelope belongs to
            one period, and the 24 unexplained Amazon lines on the
            developer's own statement fall in ten of them -- and it cannot
            name the NAME either, because template 22 generated a row called
            ``Kayla`` in one period and ``Kayla's Spending Money`` in the
            other 60.  (A legacy link-less row held ``None`` here until the
            cutover: it could hold a purchase but never be a RULE's
            destination, having nothing period-independent to remember.)
        is_placed: Whether the row is a RULE-LESS definition's
            (``Transaction.is_placed``: a grid one-off's, a bank-born
            envelope's).  It answers :attr:`names_no_cadence`, the
            convergence key of :func:`~._placement._new_envelope_placement`:
            a new-envelope answer's FIRST firing converges on a same-named
            envelope no cadence made -- a placed one -- already in the
            line's period, and names its DEFINITION where it has one
            (:func:`~._naming.name_the_filed_definition`), so an envelope
            the owner made at the grid under that name is not doubled beside
            (finding **N-327**); a recurring definition's row is a different
            answer's and is never converged on.  Leaf 7b-1 carried ``recurs``
            here for that reader and for :func:`~._stating.rule_naming`; leaf
            7b-3 (``X-f6c``'s shape, ruling **R-BAL24**) made a rule-less
            definition a rule's destination in its own right, and the field
            became this one.
    """
    ...
    @property
    def names_no_cadence(self) -> bool:
        """Return whether no rule generated this row: it was PLACED.

        The convergence key of :func:`~._placement._new_envelope_placement`
        (the ``recurs`` 7b-1 read, spelled off the fields this value
        carries).  It read ``is_placed or template_id is None`` until the
        family's cutover (plan step ``balance:X-bi-7d-2``) minted every bare
        row a definition; a row the matcher created names one, so the
        link-less arm selected the legacy shape alone and went with it.
        """
...
def envelope_answer_key(new_envelope: NewEnvelope) -> "tuple[str, int]":
    """Return what identifies ONE new-envelope answer within one press.
    ...
    **The one spelling of "the same envelope this answer means"**, and it lives
    ...
    modules: :class:`~._container.MintedEnvelopes` writes it as a press mints
    definitions, and :func:`~._leftovers._marked_joining` reads it to say which
    ...
    Both terms are load-bearing: the NAME and the CATEGORY are what the
    owner's answer states, and two answers spelling one word under two
    categories are two budget lines.  **The PERIOD left the key at leaf 7b-3
    of ``balance:X-bi-7b``** (ruling **R-BAL24**): an answer mints ONE
    rule-less DEFINITION the first time it fires in a press, and every later
    line of that answer -- in that paycheck or any other -- is a row of it,
    placed where its paycheck holds none; which ROW a line joins is the
    registry's second key, ``(template_id, pay_period_id)``, and a March swipe
    still cannot file into an April row.
    ...
    Returns:
        ``(name, category_id)``.
    """
    return (new_envelope.name, new_envelope.category_id)
...
#: The prefix of what that control submits for the PLACE arm -- *a row of
#: this rule-less definition, placed in the line's period* (plan step
#: ``balance:X-bi-7b`` leaf 7b-3, ruling **R-BAL24**) -- and the ONE spelling
#: of it: :func:`place_token` writes it for
#: :attr:`~._placement.Placement.select_value` and :func:`parse_place_token`
#: reads it for the schema, so the wire value is stated once for both
#: directions.
PLACE_PREFIX: str = "place:"


def place_token(template_id: int) -> str:
    """Return what the destination select submits to place a row of *template_id*."""
    return f"{PLACE_PREFIX}{template_id}"


def parse_place_token(value: object) -> "int | None":
    """Return the definition a PLACE token names, or ``None`` for any other value.

    Args:
        value: The submitted destination value.

    Returns:
        The ``template_id`` the token carries, or ``None`` when *value* is not
        a PLACE token at all.  A token whose id part is not a row id is
        ``None`` too, so the field refuses it as it refuses any other
        malformed value.
    """
    if not isinstance(value, str) or not value.startswith(PLACE_PREFIX):
        return None
    return parse_row_id(value[len(PLACE_PREFIX):])


@dataclass(frozen=True)
class PlaceIn:
    """The destination field's PLACE arm, as the schema hands it to the door.

    Attributes:
        template_id: The rule-less definition to place a row of, in the
            line's own period.
    """
...
class PurchaseCreation:
    ...
    **Exactly one destination arm is set**, and :mod:`._create` refuses every
    other shape rather than preferring one -- a door that silently picked an
    arm would record something nobody asked for.  THREE arms since leaf 7b-3
    of ``balance:X-bi-7b`` (ruling **R-BAL24**): an existing envelope, a new
    one, or a row of a rule-less definition PLACED in the line's period.
    ...
    **It names no OWNER and no ACCOUNT** (plan step ``bank_import:X-f6a-3c-2``).
    ...
    Attributes:
        ...
        template_id: A rule-less definition to PLACE a row of, in the line's
            period, or ``None`` -- the arm a TEMPLATE merchant answer comes to
            where the paycheck holds none of its rows (leaf 7b-3).
    """
...
class CreatedPurchase:  # pylint: disable=too-many-instance-attributes
    ...
    #: The DEFINITION the envelope is a row of (``transactions.template_id``;
    #: every envelope names one, and the type says so since the family's
    #: cutover, ``balance:X-bi-7d-2``).  Carried out so
    #: :meth:`~._container.MintedEnvelopes.remember` can key the press's
    #: registry on the definition the act minted or placed under (leaf 7b-3
    #: of ``balance:X-bi-7b``, ruling **R-BAL24**).
    template_id: int
    #: The NEW-ENVELOPE answer this act made TEMPLATE -- the merchant's
    #: stored answer, when it named exactly the envelope filed into
    #: (:func:`~._naming.name_the_filed_definition`) -- or ``None``.
    #: Carried out so the registry learns a definition the first firing
    #: CONVERGED on as well as one it minted: without it a second line of
    #: that answer in another paycheck of the same press minted a second
    #: definition and re-flipped the rule onto it (found by both of 7b-3's
    #: adversarial reviews).
    answer_named: "NewEnvelope | None" = None
