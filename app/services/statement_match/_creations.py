    ...
    **It carried its paycheck as THREE fields until pay-calendar plan step
    ...
    Attributes:
        ...
        template_id: The recurring definition this row was generated from, or
            ...
            remember about it.
        recurs: Whether that definition has a RULE (``Transaction.recurs``,
            plan step ``balance:X-bi-7b``).  A one-off is a rule-less
            definition plus its placed row since that step, so
            ``template_id`` alone no longer says *generated by a cadence*:
            an envelope the owner made at the grid carries a definition too.
            The two readers that used to ask ``template_id is None`` for
            "an envelope no cadence made" -- :func:`~._stating.rule_naming`
            and :func:`~._placement._new_envelope_placement` -- ask this
            instead, so a grid one-off's offered rule stays NEW-ENVELOPE and
            the cross-statement suggestion still converges on it (finding
            **N-327**).  ``False`` for a link-less row.  The family's third
            leaf (``X-f6c``) makes a rule-less definition itself a rule's
            destination and these readers stop asking.
    """
