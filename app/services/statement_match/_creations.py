    ...
    **It carried its paycheck as THREE fields until pay-calendar plan step
    ...
    Attributes:
        ...
        template_id: The DEFINITION this row is one of -- generated by a
            ...
            bank-born envelope's; plan step ``balance:X-bi-7b``) -- or
            ``None`` for a legacy link-less row until the family's cutover.
        ...
        is_placed: Whether the row is a RULE-LESS definition's
            ...
            envelope's).  With ``template_id`` it answers
            :attr:`names_no_cadence`, the convergence key of
            :func:`~._placement._new_envelope_placement`: a new-envelope
            answer's FIRST firing converges on a same-named envelope no
            cadence made -- placed, or legacy link-less -- already in the
    ...
    def names_no_cadence(self) -> bool:
        """Return whether no rule generated this row: placed, or legacy link-less.

        The convergence key of :func:`~._placement._new_envelope_placement`
        (the ``recurs`` 7b-1 read, spelled off the two fields this value
        carries), and the set the family's cutover collapses to ``is_placed``.
        """
        return self.is_placed or self.template_id is None
...
class CreatedPurchase:  # pylint: disable=too-many-instance-attributes
    ...
    #: or ``None`` for a legacy link-less envelope until the family's cutover.
