    ...
    **It carried its paycheck as THREE fields until pay-calendar plan step
    ...
    Attributes:
        ...
        template_id: The DEFINITION this row is one of -- generated by a
            ...
            bank-born envelope's; plan step ``balance:X-bi-7b``).  Every
            purchase-tracking row names one since the family's cutover
            (``balance:X-bi-7d-2``); the field stays optional because the
            value is built from a row and the row's column is.
            **It is the row's identity ACROSS pay periods, and that is what a
            ...
            other 60.  ``None`` means this row can hold a purchase but can
            never be a RULE's destination, because there is nothing
            period-independent to remember about it.
...
class CreatedPurchase:  # pylint: disable=too-many-instance-attributes
    ...
    #: every envelope names one since the family's cutover, ``X-bi-7d-2``).
    #: Carried out so :meth:`~._container.MintedEnvelopes.remember` can key
    #: the press's registry on the definition the act minted or placed under
    #: (leaf 7b-3 of ``balance:X-bi-7b``, ruling **R-BAL24**).
    template_id: int | None = None
