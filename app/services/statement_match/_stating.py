...
def rule_naming(
    ...
    * a row of a DEFINITION -- generated by a recurring one, or placed by a
      rule-less one (a grid one-off's, a bank-born envelope's) -- is named by
      :attr:`RuleAnswer.TEMPLATE`, which
      :func:`~._placement._template_placement` resolves to that definition's
      row in whatever period the line falls in, PLACING one where the
