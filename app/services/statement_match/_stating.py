...
def rule_naming(
    ...
    **Every destination a line can be filed into names a DEFINITION since the
    one-definition cutover** (plan step ``balance:X-bi-7d-2``): a
    :class:`~._creations.PurchaseDestination` is built from a row that
    ``tracks_purchases``, which is the definition's flag and ``False`` for a
    row with none, so the answer is :attr:`RuleAnswer.TEMPLATE` -- generated
    by a recurring definition or placed by a rule-less one (a grid one-off's,
    a bank-born envelope's) -- which
    :func:`~._placement._template_placement` resolves to that definition's
    row in whatever period the line falls in, PLACING one where the paycheck
    holds none of a rule-less definition (ruling **R-BAL24**, leaf 7b-3 of
    ``balance:X-bi-7b``): the identity across periods ruling **R-GA** says a
    rule needs.

    It carried a second arm until that cutover: a LEGACY link-less row --
    no definition to name -- was named by :attr:`RuleAnswer.NEW_ENVELOPE`
    with its own name and category, which
    :func:`~._placement._new_envelope_placement` then converged on (measured
    2026-08-30 on the developer's own account: 223 of his 256 offerable
    destinations carried a template and 33 did not, every one of the 33
    minted by a new-envelope answer).  The cutover minted each of those a
    definition and the arm went with them; ``PurchaseDestination.template_id``
    is ``int`` since, so the shape cannot be spelled.  A NEW envelope is
    :func:`rule_creating`'s answer, from a :class:`~._creations.NewEnvelope`
    rather than a destination row.
    ...
    **It was keyed on ``recurs`` from leaf 7b-1 to leaf 7b-2**, because a
    ...
    UNRESOLVED in every other paycheck until 7b-3 made such a definition a
    rule's destination in its own right; the key went with that.
