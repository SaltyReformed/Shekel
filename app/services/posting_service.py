...
def reverse_transfer_postings_before_delete(xfer: Transfer) -> None:
    ...
    **BUILT AND NOT YET WIRED, and saying so is the point** (plan step X-d,
    finding **N-145**).  ``transfer_service.delete_transfer`` still calls
    :func:`sync_transfer_postings` with ``settled=False``, which asserts inside
    the retirement window this function exists to avoid; moving it over needs
    ~9 more lines in a module already at 999 of its 1000-line ceiling, so the
    seam that makes room is a developer ruling rather than a refactor to slip
    in.  Until then this has no caller, and a reader must not infer from its
    presence that the transfer delete path is already correct.

    The transfer twin of :func:`reverse_postings_before_delete`, and the
    delete-side reconcile ``transfer_service.delete_transfer`` will run FIRST,
