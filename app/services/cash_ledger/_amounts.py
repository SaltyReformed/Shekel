...
from ._days import MovedOn, ReconciledThrough
...
def _entry_checking_impact(
    ...
    **Which bucket a debit falls in is DERIVED, and that is ruling R-DH (d)**
    ...
    That nullability is why the stored column is promoted through
    :meth:`~app.services.cash_ledger.MovedOn.recorded` rather than the plain
    constructor (ruling R-DJ): the door that admits ``None`` is the one this
    call site needs, and saying so distinguishes "not seen yet" from a day the
    schema guarantees.
    ...
    for entry in entries:
        ...
        elif reconciled_through.covers(MovedOn.recorded(entry.settled_on)):
