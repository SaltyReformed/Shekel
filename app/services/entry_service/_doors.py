...
def create_entry(
    ...
    # A DEBIT purchase cannot move what the credit entries sum to; see
    # ``sync_entry_payback``'s ``moves_credit_total`` (X-au-i, finding N-323).
    sync_entry_payback(transaction_id, owner_id,
                       moves_credit_total=details.is_credit)
...
def update_entry(entry_id: int, user_id: int, **kwargs) -> TransactionEntry:
    ...
    # Read off the SUBMITTED fields, not off the entry: the loop above has
    # already mutated it, so asking it now answers about the new state.  Only
    # these two can move the credit sum (X-au-i, finding N-323).
    sync_entry_payback(entry.transaction_id, owner_id,
                       moves_credit_total=bool(_COST_BEARING_FIELDS & set(valid_updates)))
...
def delete_entry(entry_id: int, user_id: int) -> int:
    ...
    transaction_id = entry.transaction_id
    # Read BEFORE the delete below; afterwards the row is gone (X-au-i).
    deleted_a_credit = entry.is_credit
    ...
    sync_entry_payback(transaction_id, owner_id,
                       moves_credit_total=deleted_a_credit)
    _resync_after_entry_change(txn)
