"""What a PURCHASE write may not do: the entry doors' refusals, in one place.

**Split out of :mod:`._doors` at plan step X-au-i**, the leaf whose own change
pushed that module past ``max-module-lines``.  The cut is by KIND rather than by
convenience: a door decides what to write, and these decide what may not be
written at all.  Every one is asked before any column is mutated, so a refusal
leaves the row untouched, and every one is a rule the popover already obeys by
not rendering the control -- these are the crafted-request and stale-form
backstops behind those.

They stay PRIVATE to the ``entry_service`` package: ``_doors`` is their only
caller and each is one half of a rule whose other half is a template.

Boundary discipline (``CLAUDE.md`` Architecture): plain data and ORM rows in,
``None`` or a raise out; no Flask import, no writes.
"""
...
#: The fields whose change moves MONEY on a purchase, shared by the refusals
#: here and by ``_doors.update_entry``'s payback sync -- one statement of "what
#: a cost-bearing edit is", not two.
