...
def compute_tracks_section(balance_ctx: BalanceContext) -> dict:
    ...
    **This tier carried a ``DebtTrack`` wrapper until plan step X-u** (ruling
    ...
    Args:
        balance_ctx: The render's read pass, built by the route.  **TAKEN
            rather than built here** (pay-calendar plan step C2-f2e, ledger
            rows **P56** and **P61**): this tier opened a pass of its own while
            :func:`~._pulse.compute_pulse_section` opened another, so ``/``
            held two and derived the owner's pay calendar TWICE per render
            where ``/grid``, ``/savings`` and ``/retirement`` each derive it
            once.  This tier reads no account and no period, so the pass is all
            it needs -- which is why it takes the context where the two
            producers beside it take a
            :class:`~._section.DashboardSection`.
    ...
    # second pass from.  **And the pass arrives from the ROUTE since C2-f2e**,
    # which is the same argument one level up: this module held the last
    # ``BalanceContext.build`` under ``app/services/dashboard_service/``, so the
    # page it renders half of opened two.
