...
#: :func:`app.services.pay_period_service.generate_pay_periods` authors
#: ``end_date = start_date + (cadence_days - 1)``; at a cadence of 1 those two
#: are the same day and the INSERT dies as a ``CheckViolation`` -- an
...
#: REPRESENTATION, not of the schedule: an authored ``end_date`` cannot hold a
...
#: correctly, because neither of those authors an end.
#:
